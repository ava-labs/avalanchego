...
func TestFeeHistory(t *testing.T) {
	var cases = []struct {
		...
		{false, 2, 10, 20, nil, 0, 0, errBeyondHistoricalLimit},         // query behind historical limit
		{false, 32, 1000000000, 0, nil, 0, 0, errBeyondHistoricalLimit}, // no blocks remaining after truncation
	}
	for i, c := range cases {
		config := Config{
