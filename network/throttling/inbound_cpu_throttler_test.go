...
func TestCPUThrottler(t *testing.T) {
	...
	cpuTargeter.EXPECT().TargetCPUUsage(vdrID).Return(0.5, 0.5).Times(1)
	cpuTracker.EXPECT().Utilization(vdrID, gomock.Any()).Return(float64(0.9)).Times(1)
	cpuTargeter.EXPECT().TargetCPUUsage(nonVdrID).Return(0.5, 0.5).Times(1)
	...
	cpuTargeter.EXPECT().TargetCPUUsage(vdrID).Return(float64(0), float64(0)).Times(1)
	cpuTracker.EXPECT().Utilization(vdrID, gomock.Any()).Return(float64(1)).Times(1)
	cpuTargeter.EXPECT().TargetCPUUsage(nonVdrID).Return(float64(0), float64(0)).Times(1)
	...
	cpuTargeter.EXPECT().TargetCPUUsage(vdrID).Return(float64(0.5), float64(0.5)).Times(1)
	cpuTracker.EXPECT().Utilization(vdrID, gomock.Any()).Return(float64(0)).Times(1)
	cpuTargeter.EXPECT().TargetCPUUsage(nonVdrID).Return(float64(0.5), float64(0.5)).Times(1)
