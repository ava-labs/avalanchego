...
func (m *metrics) initialize(registerer prometheus.Registerer) error {
	...
	errs.Add(
		...
		m.appRequest.initialize(message.AppRequest, registerer),
		m.appResponse.initialize(message.AppResponse, registerer),
		m.appGossip.initialize(message.AppGossip, registerer),
	)
	return errs.Err
}
