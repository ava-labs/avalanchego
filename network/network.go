...
func (n *network) GetAcceptedFrontier(validatorIDs ids.ShortSet, chainID ids.ID, requestID uint32, deadline time.Duration) []ids.ShortID {
	...
	n.log.Debug("compression: op,len,compLen,time GetAcceptedFrontier,%d,%d,%d", lenMsg, lenCompMsg, time.Since(now).Nanoseconds())
...
func (n *network) AcceptedFrontier(validatorID ids.ShortID, chainID ids.ID, requestID uint32, containerIDs []ids.ID) {
	...
	lenMsg := len(msg.Bytes())
	lenCompMsg := len(compMsg)
	n.log.Debug("compression: op,len,compLen,time AcceptedFrontier,%d,%d,%d", lenMsg, lenCompMsg, time.Since(t1).Nanoseconds())
...
func (n *network) GetAncestors(validatorID ids.ShortID, chainID ids.ID, requestID uint32, deadline time.Duration, containerID ids.ID) bool {
	...
	lenMsg := len(msg.Bytes())
	lenCompMsg := len(compMsg)
	n.log.Debug("compression: op,len,compLen,time GetAncestors,%d,%d,%d", lenMsg, lenCompMsg, time.Since(t1).Nanoseconds())
...
func (n *network) MultiPut(validatorID ids.ShortID, chainID ids.ID, requestID uint32, containers [][]byte) {
	...
	lenMsg := len(msg.Bytes())
	lenCompMsg := len(compMsg)
	n.log.Debug("compression: op,len,compLen,time MultiPut,%d,%d,%d", lenMsg, lenCompMsg, time.Since(t1).Nanoseconds())
	if peer == nil || !peer.connected.GetValue() || !peer.compatible.GetValue() || !peer.Send(msg, true, true) {
...
func (n *network) PushQuery(validatorIDs ids.ShortSet, chainID ids.ID, requestID uint32, deadline time.Duration, containerID ids.ID, container []byte) []ids.ShortID {
	...
	lenMsg := len(msgBytes)
	lenCompMsg := len(compMsg)
	n.log.Debug("compression: op,len,compLen,time PushQuery,%d,%d,%d", lenMsg, lenCompMsg, time.Since(t1).Nanoseconds())
...
func (n *network) PullQuery(validatorIDs ids.ShortSet, chainID ids.ID, requestID uint32, deadline time.Duration, containerID ids.ID) []ids.ShortID {
	...
	lenMsg := len(msgBytes)
	lenCompMsg := len(compMsg)
	n.log.Debug("compression: op,len,compLen,time PullQuery,%d,%d,%d", lenMsg, lenCompMsg, time.Since(t1).Nanoseconds())
...
func (n *network) Chits(validatorID ids.ShortID, chainID ids.ID, requestID uint32, votes []ids.ID) {
	...
	lenMsg := len(msgBytes)
	lenCompMsg := len(compMsg)
	n.log.Debug("compression: op,len,compLen,time Chits,%d,%d,%d", lenMsg, lenCompMsg, time.Since(t1).Nanoseconds())
