	...
	tests := []struct {
		...
		},
		{
			...
			requestFunc: func(t *testing.T, router *Router, client *Client, sender *common.MockSender, handler *mocks.MockHandler, wg *sync.WaitGroup) {
				...
				require.NoError(t, client.AppRequest(context.Background(), set.Of[ids.NodeID](nodeID), request, callback))
			},
		},
		{
		...
		},
		{
			...
			requestFunc: func(t *testing.T, router *Router, client *Client, sender *common.MockSender, handler *mocks.MockHandler, wg *sync.WaitGroup) {
				...
				require.NoError(t, client.AppGossipSpecific(context.Background(), set.Of[ids.NodeID](nodeID), request))
			},
		},
	}
...
func TestAppRequestDuplicateRequestIDs(t *testing.T) {
	...
	require.NoError(client.AppRequest(context.Background(), set.Of[ids.NodeID](nodeID), []byte{}, nil))
	requestSent.Wait()

	// force the router to use the same requestID
	router.requestID = 0
	timeout.Add(1)
	err = client.AppRequest(context.Background(), set.Of[ids.NodeID](nodeID), []byte{}, nil)
	requestSent.Wait()
