...
func TestExportTxSemanticVerify(t *testing.T) {
	...
	if err := rawTx.SignSECP256K1Fx(vm.codec, apricotCodecVersion, [][]*crypto.PrivateKeySECP256K1R{{keys[0]}}); err != nil {
		t.Fatal(err)
	}
	...
	if err := rawTx.UnsignedTx.SemanticVerify(vm, utx.UnsignedTx, rawTx.Creds, 0); err != nil {
		t.Fatal(err)
	}
}
...
func TestExportTxSemanticVerifyUnknownCredFx(t *testing.T) {
	...
	if err := rawTx.UnsignedTx.SemanticVerify(vm, utx.UnsignedTx, []verify.Verifiable{nil}, 0); err == nil {
...
func TestExportTxSemanticVerifyInvalidAssetID(t *testing.T) {
	...
	if err := rawTx.SignSECP256K1Fx(vm.codec, apricotCodecVersion, [][]*crypto.PrivateKeySECP256K1R{
		{
			keys[0],
		},
		{
...
func TestExportTxSemanticVerifyInvalidFx(t *testing.T) {
	...
	if err := rawTx.UnsignedTx.SemanticVerify(vm, utx.UnsignedTx, []verify.Verifiable{&avax.TestVerifiable{}}, 0); err == nil {
...
func TestExportTxSemanticVerifyInvalidTransfer(t *testing.T) {
	...
	if err := rawTx.SignSECP256K1Fx(vm.codec, apricotCodecVersion, [][]*crypto.PrivateKeySECP256K1R{{keys[1]}}); err != nil {
		t.Fatal(err)
	}
