...
func TestInvalidGenesis(t *testing.T) {
	...
	vm := &VM{StateMigration: NoMigration{}}
...
func TestVMLinearizeStateMigration(t *testing.T) {
	...
	require.NoError(vm.Linearize(context.Background(), ids.ID{}))
	...
	vm = &VM{
		Config:         config,
		StateMigration: &fooStateMigration{CommitFrequency: 1},
	}
