...
func validCreateAssetTx(t *testing.T) (*CreateAssetTx, codec.Manager, *snow.Context) {
	...
	ctx := NewContext(t)
	if err := tx.SyntacticVerify(ctx, c, assetID, &config.TxFees{}, 1); err != nil {
...
func TestCreateAssetTxSyntacticVerifyUnsortedInitialStates(t *testing.T) {
	...
	if err := tx.SyntacticVerify(ctx, c, ids.Empty, &config.TxFees{}, 2); err == nil {
...
func TestCreateAssetTxSyntacticVerifyInitialStates(t *testing.T) {
	...
	if err := tx.SyntacticVerify(ctx, c, assetID, &config.TxFees{}, 3); err == nil {
...
func TestCreateAssetTxSyntacticVerifyBaseTx(t *testing.T) {
	...
	var baseTx BaseTx
	tx.BaseTx = baseTx
	if err := tx.SyntacticVerify(ctx, c, assetID, &config.TxFees{}, 2); err == nil {
