...
var (
	...
	eUpgradeDynamicFeesConfig = fee.DynamicFeesConfig{
		GasPrice:            fee.GasPrice(10 * units.NanoAvax),
		FeeDimensionWeights: fee.Dimensions{1, 1, 1, 1},
		MaxGasPerSecond:     fee.Gas(1_000_000),
		LeakGasCoeff:        fee.Gas(1),
	}

	customDynamicFeesConfig *fee.DynamicFeesConfig
)
...
func GetDynamicConfig(isEActive bool) (fee.DynamicFeesConfig, error) {
	if !isEActive {
		return fee.DynamicFeesConfig{}, ErrDynamicFeeConfigNotAvailable
	}
...
func ResetDynamicConfig(ctx *snow.Context, customFeesConfig *fee.DynamicFeesConfig) error {
