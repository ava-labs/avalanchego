...
func (service *Service) CreateAsset(r *http.Request, args *CreateAssetArgs, reply *AssetIDChangeAddr) error {
	...
	amountsSpent, ins, keys, err := service.vm.Spend(
		...
		kc,
		map[ids.ID]uint64{
			service.vm.feeAssetID: service.vm.TxFees.CreateAsset,
		},
	)
	if err != nil {
		return err
	...
	if amountSpent := amountsSpent[service.vm.feeAssetID]; amountSpent > service.vm.TxFees.CreateAsset {
		outs = append(outs, &avax.TransferableOutput{
			Asset: avax.Asset{ID: service.vm.feeAssetID},
			Out: &secp256k1fx.TransferOutput{
				Amt: amountSpent - service.vm.TxFees.CreateAsset,
...
func (service *Service) SendMultiple(r *http.Request, args *SendMultipleArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	amountWithFee, err := safemath.Add64(amounts[service.vm.feeAssetID], service.vm.TxFees.Base)
	if err != nil {
		return fmt.Errorf("problem calculating required spend amount: %w", err)
	}
...
func (service *Service) Mint(r *http.Request, args *MintArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	amountsSpent, ins, keys, err := service.vm.Spend(
		...
		feeKc,
		map[ids.ID]uint64{
			service.vm.feeAssetID: service.vm.TxFees.Operation,
		},
	)
	if err != nil {
		return err
	...
	outs := []*avax.TransferableOutput{}
	if amountSpent := amountsSpent[service.vm.feeAssetID]; amountSpent > service.vm.TxFees.Operation {
		outs = append(outs, &avax.TransferableOutput{
			Asset: avax.Asset{ID: service.vm.feeAssetID},
			Out: &secp256k1fx.TransferOutput{
				Amt: amountSpent - service.vm.TxFees.Operation,
				OutputOwners: secp256k1fx.OutputOwners{
					Locktime:  0,
					Threshold: 1,
					Addrs:     []ids.ShortID{changeAddr},
...
func (service *Service) Import(_ *http.Request, args *ImportArgs, reply *api.JSONTxID) error {
	...
	if amountSpent := amountsSpent[service.vm.feeAssetID]; amountSpent < service.vm.TxFees.Import {
		var localAmountsSpent map[ids.ID]uint64
		localAmountsSpent, ins, keys, err = service.vm.Spend(
			utxos,
			kc,
			map[ids.ID]uint64{
				service.vm.feeAssetID: service.vm.TxFees.Import - amountSpent,
			},
		)
		if err != nil {
			return err
	...
	amountsSpent[service.vm.feeAssetID] -= service.vm.TxFees.Import
...
func (service *Service) Export(_ *http.Request, args *ExportArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	if assetID == service.vm.feeAssetID {
		amountWithFee, err := safemath.Add64(uint64(args.Amount), service.vm.TxFees.Export)
		if err != nil {
			return fmt.Errorf("problem calculating required spend amount: %w", err)
		}
		amounts[service.vm.feeAssetID] = amountWithFee
	} else {
		amounts[service.vm.feeAssetID] = service.vm.TxFees.Export
