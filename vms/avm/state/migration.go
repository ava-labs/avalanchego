...
func (f *FooMigration) Migrate(
	...
	// TODO check all iterator errors
	itr := utxoDB.NewIteratorWithStart(lastMigratedUTXO[:])
	for itr.Next() {
		utxo := &avax.UTXO{}
	...
	if err := itr.Error(); err != nil {
		return nil, fmt.Errorf("iterating db: %w", err)
	}
	...
	itr = txDB.NewIteratorWithStart(lastMigratedTx[:])
	for itr.Next() {
	...
	itr = blockDB.NewIteratorWithStart(lastMigratedBlk[:])
	for itr.Next() {
	...
	lastMigratedBlkHeight, err := next.GetLastMigratedBlockHeight()
	if err != nil {
		return nil, fmt.Errorf("getting last migrated block height: %w", err)
	}

	itr = blockIDDB.NewIteratorWithStart(database.PackUInt64(lastMigratedBlkHeight))
	for itr.Next() {
...
func (m *migrationState) GetLastMigratedBlockHeight() (uint64, error) {
