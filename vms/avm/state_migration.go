...
var (
	...
	_ StateFactory    = (*LegacyStateFactory)(nil)
	_ StateFactory    = (*GForkStateFactory)(nil)
...
type LegacyStateFactory struct{}

func (LegacyStateFactory) New(
	_ context.Context,
	_ logging.Logger,
	_ block.Parser,
...
type GForkStateFactory struct {
	CommitFrequency int

	updates int
}

func (g *GForkStateFactory) New(
	ctx context.Context,
	log logging.Logger,
	parser block.Parser,
	metrics prometheus.Registerer,
	...
	next, err := newGForkState(ctx,
		baseDB,
		prefixdb.New([]byte("v1.14.x"), baseDB),
		parser,
		metrics)
	if err != nil {
...
func (g *GForkStateFactory) commit(db versiondb.Commitable) (bool, error) {
	g.updates++
