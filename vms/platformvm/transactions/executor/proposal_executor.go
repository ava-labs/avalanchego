...
func (pe *proposalExecutor) executeAdvanceTime(
	...
	chainTimestamp := parentState.GetTimestamp()
	...
	CurrentStakerStates := parentState.CurrentStakerChainState()
	PendingStakerStates := parentState.PendingStakerChainState()
	currentSupply := parentState.GetCurrentSupply()

	newlyCurrentStakerStates, newlyPendingStakerStates, updatedSupply, err := pe.updateStakerSet(
		CurrentStakerStates,
		PendingStakerStates,
		currentSupply,
		txTimestamp,
	...
	onCommitState := state.NewVersioned(parentState, newlyCurrentStakerStates, newlyPendingStakerStates)
	onCommitState.SetTimestamp(txTimestamp)
	onCommitState.SetCurrentSupply(updatedSupply)

	// State doesn't change if this proposal is aborted
	onAbortState := state.NewVersioned(parentState, CurrentStakerStates, PendingStakerStates)

	return onCommitState, onAbortState, nil
}

func (pe *proposalExecutor) updateStakerSet(
	currentStakerStates txstate.CurrentStakerState,
	pendingStakerStates txstate.PendingStakerState,
	currentSupply uint64,
	proposedChainTime time.Time,
) (
	txstate.CurrentStakerState,
	txstate.PendingStakerState,
	...
	for _, tx := range currentStakerStates.Stakers() {
		...
		default:
			return nil,
				nil,
				0,
				fmt.Errorf("expected tx type *unsigned.AddValidatorTx or *unsigned.AddDelegatorTx but got %T", staker)
		}
	}
