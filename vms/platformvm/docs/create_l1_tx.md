...
- Any existing [PoA validators](https://github.com/ava-labs/avalanchego/blob/master/vms/platformvm/txs/executor/subnet_tx_verification.go) continue until their staking period expires but no new ones can be added, as the subnet owner is set to empty on conversion.
...
- VMID is used to look up and run the VM. We decided to remove it from the transaction type and instead add it as a field in the node config: [subnets.Config](https://github.com/ava-labs/avalanchego/blob/master/subnets/config.go#L21) as a mapping subnetID -> VMID.
