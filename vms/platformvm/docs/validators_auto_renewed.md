...
Mutable auto-renewed validator state is stored separately from the core `Staker` record in `state.StakingInfo`, defined in `state/metadata_validator`:
...
Creates a new primary network auto-renewed validator. Defined in `txs/add_auto_renewed_validator_tx`.

Verification rules (in `txs/executor/staker_tx_verification`):
...
On execution (in `txs/executor/standard_tx_executor`):
...
Updates the auto-renew configuration. Defined in `txs/set_auto_renewed_validator_config_tx`.
...
Issued by the block builder at the end of each cycle. Defined in `txs/reward_auto_renewed_validator_tx`.
...
This is a proposal transaction with commit and abort paths, handled in `txs/executor/proposal_tx_executor`. It has no inputs or outputs; reward and stake-return UTXOs are created by proposal execution. The standard executor rejects it, and `RewardValidatorTx` is rejected for auto-renewed validators.

## Cycle End Processing

When a cycle ends, the block builder (in `block/builder/builder`) issues a `RewardAutoRenewedValidatorTx` as a proposal block. The builder selects this reward transaction type when the next staker to reward was created by `AddAutoRenewedValidatorTx`; other permissionless stakers continue to use `RewardValidatorTx`.

When an auto-renewed validator exits, proposal execution returns the initial
stake outputs as UTXOs that reference the original `AddAutoRenewedValidatorTx`.
...
3. Withdrawn rewards are paid as reward UTXOs on the commit path. The configured withdrawal from `AutoCompoundRewardShares` is paid as one reward UTXO, and any additional overflow from the `MaxValidatorStake` cap is paid as a separate reward UTXO.
...
On commit, all rewards (`PotentialReward` + `AccruedValidationRewards` + `DelegateeReward` + `AccruedDelegateeRewards`) are withdrawn as reward UTXOs. Principal is returned. Validator is removed.
...
RPC consumers need this metadata to distinguish auto-renewed validators from fixed-term validators and to show the validator's next-cycle behavior. Wallets, staking dashboards, and operators can use it to display whether the validator will renew or exit at the next cycle boundary, what percentage of rewards will be restaked, and which authority can update the configuration.
