...
- `CreateL1Chain` (new): Since the new [`CreateL1Tx`](https://github.com/ava-labs/avalanchego/blob/create-l1-tx/vms/platformvm/txs/create_l1_tx.go) has no `SubnetID` field, a new `CreateL1Chain(subnetID ids.ID, tx *txs.CreateL1Tx)` method is a helper function that will mirror [`CreateChainTx`](https://github.com/ava-labs/avalanchego/blob/master/vms/platformvm/txs/create_chain_tx.go) but takes `subnetID` explicitly and derives the `BlockchainID` via `tx.BlockchainID(subnetID)`. Both methods build the same `chains.ChainParameters` struct and call `QueueChainCreation`.
- [`createSubnet`](https://github.com/ava-labs/avalanchego/blob/master/vms/platformvm/vm.go#L328) type switch: Since [`GetChains`](https://github.com/ava-labs/avalanchego/blob/create-l1-tx/vms/platformvm/state/state.go#L1170) in the proposed implementation will now return either `*txs.CreateChainTx` or `*txs.CreateL1Tx`, a type switch should be added: `CreateChainTx` calls [`CreateChain`](https://github.com/ava-labs/avalanchego/blob/master/vms/platformvm/config/internal.go#L93C20-L93C31), `CreateL1Tx` calls [`CreateL1Chain`](https://github.com/ava-labs/avalanchego/blob/create-l1-tx/vms/platformvm/config/internal.go#L115).
- `state.AddL1Chain` and `Diff.Apply`: the already existing `AddChain` casts the tx to `*txs.CreateChainTx` to extract the `subnetID`. Since `CreateL1Tx` will have no `SubnetID` field, `AddL1Chain` would take `subnetID` as an explicit parameter instead. `Diff.Apply` would also be updated with a type switch to route `CreateChainTx` to `AddChain` and all other types to `AddL1Chain`.
