...
func (c *Client) getAutoRenewedValidatorConfigOwners(ctx context.Context, txIDs ...ids.ID) (map[ids.ID]fx.Owner, error) {
...
func (c *Client) GetOwners(ctx context.Context, subnetIDs []ids.ID, validationIDs []ids.ID, autoRenewedValidatorTxIDs []ids.ID) (map[ids.ID]fx.Owner, error) {
	...
	autoRenewedOwners, err := c.getAutoRenewedValidatorConfigOwners(ctx, autoRenewedValidatorTxIDs...)
	if err != nil {
		return nil, err
	}

	owners := make(map[ids.ID]fx.Owner, len(subnetOwners)+len(deactivationOwners)+len(autoRenewedOwners))
	maps.Copy(owners, subnetOwners)
	maps.Copy(owners, deactivationOwners)
	maps.Copy(owners, autoRenewedOwners)
	return owners, nil
}
