...
func (vs *versionedStateImpl) GetTx(txID ids.ID) (*transactions.SignedTx, status.Status, error) {
...
func (vs *versionedStateImpl) AddTx(tx *transactions.SignedTx, status status.Status) {
	txID := tx.ID()
