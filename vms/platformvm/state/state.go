...
func (s *state) syncGenesis(genesisBlk blocks.Block, genesis *genesis.State) error {
	...
	for _, vdrTx := range genesis.Validators {
		...
		staker, err := NewCurrentStaker(vdrTx.ID(), tx, tx.StartTime(), tx.StakingPeriod(), potentialReward)
		if err != nil {
			return err
		}
...
func (s *state) loadCurrentValidators() error {
	...
	for validatorIt.Next() {
		...
		staker, err := NewCurrentStaker(txID, stakerTx, stakerTx.StartTime(), stakerTx.StakingPeriod(), metadata.PotentialReward)
		if err != nil {
			return err
		}
	...
	for _, delegatorIt := range []database.Iterator{delegatorIt, subnetDelegatorIt} {
		for delegatorIt.Next() {
			...
			staker, err := NewCurrentStaker(txID, stakerTx, stakerTx.StartTime(), stakerTx.StakingPeriod(), potentialReward)
			if err != nil {
				return err
			}
