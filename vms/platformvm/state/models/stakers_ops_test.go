...
func TestSimpleStakerOpsForStakersStorage(t *testing.T) {
...
func TestSimpleStakerOpsForState(t *testing.T) {
	properties := gopter.NewProperties(nil)

	properties.Property("some current validator ops", prop.ForAll(
		func(s state.Staker) string {
			store, err := buildChainState()
			if err != nil {
				return fmt.Sprintf("unexpected error while creating chain state, err %v", err)
			}
...
func TestSimpleStakerOpsForDiff(t *testing.T) {
	properties := gopter.NewProperties(nil)
	...
	properties.Property("some current validator ops", prop.ForAll(
		func(s state.Staker) string {
			...
			if err != nil {
				return fmt.Sprintf("unexpected error while creating chain base state, err %v", err)
			}
