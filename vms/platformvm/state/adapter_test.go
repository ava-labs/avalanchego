...
func TestStakingState(t *testing.T) {
	...
	// [State.PutCurrentValidator] reads the node's primary network entry.
...
func TestStakingStatePreservesPendingPriority(t *testing.T) {
	...
	typedDelegatorTx, err := NewTx[platform.Delegator](delegatorTx)
	require.NoError(t, err)
	require.NoError(t, typedState.PutPendingDelegator(typedDelegatorTx))
...
// [NewTx] must reject a transaction of the wrong staking role. Passing a
// mismatched (txID, body) pair to the staking APIs is a compile error, so
// construction is the only path that needs runtime coverage.
func TestNewTxRejectsCrossRoleTx(t *testing.T) {
	now := time.Now().Truncate(time.Second)
	validator := platform.Validator{
		NodeID: defaultValidatorNodeID,
		Start:  uint64(now.Add(time.Hour).Unix()),
	...
	_, err := NewTx[platform.ValidatorTx](delegatorTx)
	require.ErrorIs(t, err, errUnexpectedStaker)

	_, err = NewTx[platform.Delegator](validatorTx)
	require.ErrorIs(t, err, errUnexpectedStaker)

	_, err = NewTx[platform.Delegator](nil)
	require.ErrorIs(t, err, errUnexpectedStaker)
}
