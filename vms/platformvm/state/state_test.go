...
func TestReplacementValidatorPreservesDelegatorWeight(t *testing.T) {
	// Verifies that replacing a validator (remove + re-add in the same block) preserves delegator weight.
	...
	var (
		...
		validatorWeight1 uint64 = 10
		validatorWeight2 uint64 = 20
		delegatorWeight  uint64 = 5
	)

	// Create the first validator (weight=10).
	unsignedTx1 := createPermissionlessValidatorTx(t, subnetID, txs.Validator{
		NodeID: nodeID,
		End:    uint64(endTime1.Unix()),
		Wght:   validatorWeight1,
	})
	tx1 := &txs.Tx{Unsigned: unsignedTx1}
	require.NoError(tx1.Initialize(txs.Codec))
	...
	// Create a delegator (weight=5) for the same node.
	unsignedDelegatorTx := createPermissionlessDelegatorTx(subnetID, txs.Validator{
		NodeID: nodeID,
		End:    uint64(endTime1.Unix()),
		Wght:   delegatorWeight,
	})
	delegatorTx := &txs.Tx{Unsigned: unsignedDelegatorTx}
	require.NoError(delegatorTx.Initialize(txs.Codec))
	delegatorStaker, err := NewCurrentStaker(delegatorTx.ID(), unsignedDelegatorTx, startTime, 0)
	require.NoError(err)

	// Create the replacement validator (weight=20).
	unsignedTx2 := createPermissionlessValidatorTx(t, subnetID, txs.Validator{
		NodeID: nodeID,
		End:    uint64(endTime2.Unix()),
		Wght:   validatorWeight2,
	})
	tx2 := &txs.Tx{Unsigned: unsignedTx2}
	require.NoError(tx2.Initialize(txs.Codec))
	...
	// Block 0: Add the validator and delegator, then commit.
	state.AddTx(tx1, status.Committed)
	...
	d.PutCurrentDelegator(delegatorStaker)
	require.NoError(d.Apply(state))

	state.SetHeight(0)
	require.NoError(state.Commit())

	// Sanity-check: manager weight = validator (10) + delegator (5) = 15.
	require.Equal(validatorWeight1+delegatorWeight, state.validators.GetWeight(subnetID, nodeID))

	// Block 1: Replace the validator (remove old, add new) in the same block.
	// The delegator is NOT touched — it stays.
	state.AddTx(tx2, status.Committed)
	...
	// The manager weight must be newValidator (20) + delegator (5) = 25.
	// If we naively used diff.added.Weight we'd get 20, losing the delegator.
	require.Equal(validatorWeight2+delegatorWeight, state.validators.GetWeight(subnetID, nodeID))
}
