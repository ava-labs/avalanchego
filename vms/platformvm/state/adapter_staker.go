...
// NewCurrentValidator returns a current validator built from the registering
// transaction tx. The period bounds are the caller's: a bounded registration
// carries its own end time, while a restaking registration's end time is
// computed by execution from the configured period.
func NewCurrentValidator(
	tx Tx[platform.ValidatorTx],
	start, end time.Time,
	weight, potentialReward uint64,
) (CurrentValidator, error) {
	stakingPeriod, err := newCurrentValidatorStakingPeriod(tx.ID(), tx.Body(), start, end, weight)
	if err != nil {
...
// NewCurrentDelegator returns a current delegator built from the registering
// transaction tx.
func NewCurrentDelegator(
	tx Tx[platform.Delegator],
	startTime, endTime time.Time,
	weight, potentialReward uint64,
) CurrentDelegator {
	return CurrentDelegator{
		stakingPeriod:   newCurrentStakingPeriod(tx.ID(), tx.Body(), startTime, endTime, weight),
		potentialReward: potentialReward,
	}
}
