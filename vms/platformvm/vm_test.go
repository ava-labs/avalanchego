...
func defaultVM(t *testing.T, f fork) (*VM, database.Database, *mutableSharedMemory) {
	...
	require.NoError(err)
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(testSubnet1))
	vm.ctx.Lock.Lock()
...
func TestTransferSubnetOwnershipTx(t *testing.T) {
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(transferSubnetOwnershipTx))
	vm.ctx.Lock.Lock()
...
func TestBaseTx(t *testing.T) {
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(baseTx))
	vm.ctx.Lock.Lock()
