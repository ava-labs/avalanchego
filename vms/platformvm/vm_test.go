...
func defaultVM(t *testing.T) *VM {
	...
	err := vm.Initialize(ctx, db, genesisBytes, msgChan, nil)
	assert.NoError(t, err)

	tx, err := vm.newCreateSubnetTx(
		2, // threshold; 2 sigs from keys[0], keys[1], keys[2] needed to add validator to this subnet
		[]ids.ShortID{ // control keys are keys[0], keys[1], keys[2]
...
func TestGenesis(t *testing.T) {
	...
	assert.NoError(t, err, "couldn't get genesis block")
	assert.Equal(t, choices.Accepted, genesisBlock.Status(), "genesis block should be accepted")
	...
	for _, utxo := range genesisState.UTXOs {
		...
		assert.NoError(t, err, "couldn't get genesis block")
		assert.Len(t, utxos, 1, "expected each address to have one UTXO")

		out, ok := utxos[0].Out.(*secp256k1fx.TransferOutput)
		assert.True(t, ok, "expected utxo output to be type *secp256k1fx.TransferOutput")
