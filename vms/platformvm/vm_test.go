...
func defaultVM() (*VM, database.Database, *common.SenderTest, *mutableSharedMemory) {
	...
	if err != nil {
		...
		panic(err)
	} else if lastAcceptedID, err := vm.LastAccepted(); err != nil {
		panic(err)
	} else if err := vm.SetPreference(lastAcceptedID); err != nil {
		panic(err)
	}
...
func TestAddValidatorCommit(t *testing.T) {
	...
	if _, ok := commit.Block.(*stateless.CommitBlock); !ok {
...
func TestAddValidatorReject(t *testing.T) {
	...
	if _, ok := abort.Block.(*stateless.AbortBlock); !ok {
		t.Fatal(errShouldPrefCommit)
	}
...
func TestRejectedStateRegressionInvalidValidatorReward(t *testing.T) {
	...
	{
		onAccept, found := vm.stateVersions.GetState(addValidatorProposalCommit0.ID())
		assert.True(found)
		pendingStakers := onAccept.PendingStakers()
	...
	{
		onAccept, found := vm.stateVersions.GetState(advanceTimeProposalCommit0.ID())
		assert.True(found)
	...
	{
		onAccept, found := vm.stateVersions.GetState(addValidatorProposalCommit1.ID())
		assert.True(found)
		pendingStakers := onAccept.PendingStakers()
	...
	{
		onAccept, found := vm.stateVersions.GetState(advanceTimeProposalCommit1.ID())
		assert.True(found)
		currentStakers := onAccept.CurrentStakers()
