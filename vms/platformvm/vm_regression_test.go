...
func TestAddDelegatorTxOverDelegatedRegression(t *testing.T) {
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(addValidatorTx))
	vm.ctx.Lock.Lock()
	...
	// trigger block creation
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(addFirstDelegatorTx))
	vm.ctx.Lock.Lock()
	...
	// trigger block creation
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(addSecondDelegatorTx))
	vm.ctx.Lock.Lock()
	...
	// trigger block creation
	vm.ctx.Lock.Unlock()
	err = vm.issueTx(addThirdDelegatorTx)
...
func TestAddDelegatorTxHeapCorruption(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			vm.ctx.Lock.Unlock()
			require.NoError(vm.issueTx(addThirdDelegatorTx))
			vm.ctx.Lock.Lock()
			...
			vm.ctx.Lock.Unlock()
			require.NoError(vm.issueTx(addFourthDelegatorTx))
			vm.ctx.Lock.Lock()
...
func TestAddDelegatorTxAddBeforeRemove(t *testing.T) {
	...
	vm.ctx.Lock.Unlock()
	err = vm.issueTx(addSecondDelegatorTx)
	require.ErrorIs(err, executor.ErrOverDelegated)
	vm.ctx.Lock.Lock()
}
...
func TestRemovePermissionedValidatorDuringPendingToCurrentTransitionNotTracked(t *testing.T) {
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(createSubnetTx))
	vm.ctx.Lock.Lock()
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(addSubnetValidatorTx))
	vm.ctx.Lock.Lock()
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(removeSubnetValidatorTx))
	vm.ctx.Lock.Lock()
...
func TestSubnetValidatorBLSKeyDiffAfterExpiry(t *testing.T) {
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(primaryTx))
	vm.ctx.Lock.Lock()
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(subnetTx))
	vm.ctx.Lock.Lock()
	require.NoError(buildAndAcceptStandardBlock(vm))
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(primaryRestartTx))
	vm.ctx.Lock.Lock()
	require.NoError(buildAndAcceptStandardBlock(vm))
...
func TestPrimaryNetworkValidatorPopulatedToEmptyBLSKeyDiff(t *testing.T) {
	...
	vm.ctx.Lock.Unlock()
	require.NoError(vm.issueTx(primaryTx1))
	vm.ctx.Lock.Lock()
	require.NoError(buildAndAcceptStandardBlock(vm))
