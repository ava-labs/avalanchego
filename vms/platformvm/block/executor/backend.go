...
func (b *backend) getOnAbortState(blkID ids.ID) (optionsState, bool) {
	state, ok := b.blkIDToState[blkID]
	if !ok || state.onAbortState == nil {
		return optionsState{}, false
	}
	return state.optionsState, true
}

func (b *backend) getOnCommitState(blkID ids.ID) (optionsState, bool) {
	state, ok := b.blkIDToState[blkID]
