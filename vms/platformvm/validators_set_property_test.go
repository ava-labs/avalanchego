...
func TestGetValidatorsSetProperty(t *testing.T) {
	...
	properties.Property("check GetValidatorSet", prop.ForAll(
		...
		gen.SliceOfN(10, gen.OneConstOf(
			startPrimaryWithBLS,
...
func buildCustomGenesis() ([]byte, error) {
	...
	// we need at least a validator, otherwise BuildBlock would fail, since it won't find
	// next staker to promote/evict from stakers set. Contrary to what happens with production code
	// we push such validator at the end of times, so to avoid interference with our tests
