...
func TestMempoolMetering(t *testing.T) {
	...
	highTxFeesPaid := big.NewInt(1)
	require.NoError(err)
	wantGasUsage, err := wantComplexity.ToGas(weights)
	require.NoError(err)
	wantHighTxGasPrice := new(big.Rat).SetFrac(
		highTxFeesPaid,
		big.NewInt(int64(wantGasUsage)),
	)
	require.Equal(wantHighTxGasPrice, gotTx.GasPrice)
}

// Txs must pay a fee
func TestMempoolMetering_FeeGreaterThanZero(t *testing.T) {
	require := require.New(t)
