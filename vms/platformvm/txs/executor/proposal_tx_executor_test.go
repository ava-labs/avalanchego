...
func TestProposalTxExecuteAddDelegator(t *testing.T) {
	...
	tests := []test{
		...
		},
		{
			...
			endTime:       uint64(currentTimestamp.Add(MaxFutureStartTime + genesistest.MinStakingDuration + time.Second).Unix()),
			nodeID:        nodeID,
			rewardAddress: rewardAddress,
			feeKeys:       []*secp256k1.PrivateKey{genesistest.Keys[0]},
			setup:         nil,
...
func TestProposalTxExecuteAddSubnetValidator(t *testing.T) {
...

	{
		...
		tx, err := env.txBuilder.NewAddSubnetValidatorTx(
			genesistest.Weight, // weight
			uint64(genesistest.ValidateStartTime.Unix())+1,                                     // start time
			uint64(genesistest.ValidateStartTime.Add(genesistest.MinStakingDuration).Unix())+1, // end time
			nodeID,           // node ID
			testSubnet1.ID(), // subnet ID
