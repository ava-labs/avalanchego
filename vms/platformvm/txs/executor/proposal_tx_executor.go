...
func (e *proposalTxExecutor) setOnCommitStateContinuousValidatorRestake(validatorTx txs.ValidatorTx, validator *state.Staker) error {
	...
	if restakingRewards > 0 {
		newAccruedRewards, err = safemath.Add(validator.AccruedRewards, restakingRewards)
		if err != nil {
			return err
		}

		newWeight, err = safemath.Add(validator.Weight, restakingRewards)
		if err != nil {
			return err
		}
	}
	...
	if restakingDelegateeRewards > 0 {
		newAccruedDelegateeRewards, err = safemath.Add(validator.AccruedDelegateeRewards, restakingDelegateeRewards)
		if err != nil {
			return err
		}
...
func (e *proposalTxExecutor) createOverflowUTXOs(
	...
	totalRestakingRewards, err := safemath.Sub(newWeight, oldWeight)
	if err != nil {
		return 0, 0, err
	}

	// Calculate how much room we have to grow before hitting max stake.
	restakingAvailability, err := safemath.Sub(e.backend.Config.MaxValidatorStake, oldWeight)
	if err != nil {
		return 0, 0, err
	}

	restakableValidationReward := reward.ProportionalAmount(rewards, restakingAvailability, totalRestakingRewards)
	excessValidationReward := rewards - restakableValidationReward

	restakableDelegateeReward := reward.ProportionalAmount(delegateeReward, restakingAvailability, totalRestakingRewards)
	excessDelegateeReward := delegateeReward - restakableDelegateeReward
