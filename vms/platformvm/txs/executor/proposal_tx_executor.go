...
func (e *ProposalTxExecutor) AdvanceTimeTx(tx *txs.AdvanceTimeTx) error {
	...
	currentStakerStates := e.ParentState.CurrentStakers()
	pendingStakerStates := e.ParentState.PendingStakers()
	...
	e.OnCommit = state.NewDiff(e.ParentState, newlyCurrentStakerStates, newlyPendingStakerStates)
	...
	e.OnAbort = state.NewDiff(e.ParentState, currentStakerStates, pendingStakerStates)
	e.PrefersCommit = !txTimestamp.After(localTimestamp.Add(SyncBound))
	return nil
}

func UpdateStakerSet(
	currentStakerStates state.CurrentStakers,
	pendingStakerStates state.PendingStakers,
	currentSupply uint64,
	backend *Backend,
	proposedChainTime time.Time,
) (
	state.CurrentStakers,
...
PendingStakerStateLoop:
	...
	newlyPendingStakers := pendingStakerStates.DeleteStakers(numToRemoveFromPending)
...
CurrentStakerStateLoop:
	...
	newlyCurrentStaker, err := currentStakerStates.UpdateStakers(
	...
	return newlyCurrentStaker, newlyPendingStakers, currentSupply, nil
}
