...
func (e *StandardTxExecutor) CreateChainTx(tx *txs.CreateChainTx) error {
	...
	createBlockchainTxFee := builder.GetCreateBlockchainTxFee(*e.Cfg, timestamp)
	...
	// Produce the UTXOS
	utxos.ProduceOutputs(e.State, txID, e.Ctx.AVAXAssetID, tx.Outs)
	...
	e.OnAccept = func() error { return utils.CreateChain(*e.Cfg, e.Tx.Unsigned, e.Tx.ID()) }
	return nil
}
...
func (e *StandardTxExecutor) CreateSubnetTx(tx *txs.CreateSubnetTx) error {
	...
	// Verify the flowcheck
	timestamp := e.State.GetTimestamp()
	createSubnetTxFee := builder.GetCreateSubnetTxFee(*e.Cfg, timestamp)
	if err := e.SpendHandler.SemanticVerifySpend(
		e.State,
		tx,
		tx.Ins,
