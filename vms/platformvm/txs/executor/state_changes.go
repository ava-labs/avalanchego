...
func (s *stateChanges) Apply(onAccept state.Diff) {
	onAccept.SetCurrentSupply(s.supply)

	for _, currentValidatorToAdd := range s.currentValidatorsToAdd {
		onAccept.PutCurrentValidator(currentValidatorToAdd)
	}
	for _, pendingValidatorToRemove := range s.pendingValidatorsToRemove {
		onAccept.DeletePendingValidator(pendingValidatorToRemove)
	}
	for _, currentDelegatorToAdd := range s.currentDelegatorsToAdd {
		onAccept.PutCurrentDelegator(currentDelegatorToAdd)
	}
	for _, pendingDelegatorToRemove := range s.pendingDelegatorsToRemove {
		onAccept.DeletePendingDelegator(pendingDelegatorToRemove)
	}
