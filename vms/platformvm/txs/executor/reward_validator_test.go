...
func TestRewardAutoRenewedValidatorTxAbortPath(t *testing.T) {
	// Tests that the abort path correctly handles supply accounting in two scenarios:
	// 1. Cycle 1 abort: AccruedRewards=0, supply reduced by only PotentialReward.
	// 2. Cycle 2 abort: AccruedRewards>0 (from a previous successful commit),
	//    supply is still reduced by only the current cycle's PotentialReward —
	//    not by AccruedRewards, which were already minted in a prior cycle.
	tests := []struct {
		name                    string
		...
		{
			name:                    "cycle 1 abort - no accrued rewards",
			accruedRewards:          0,
			accruedDelegateeRewards: 0,
			delegateeReward:         0,
		},
		{
			name:                    "cycle 2 abort - with accrued rewards from previous commit",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			const potentialReward = 10_000_000
			...
			require.NoError(env.state.SetStakingInfo(staker.SubnetID, staker.NodeID, state.StakingInfo{
				DelegateeReward:          tt.delegateeReward,
				AccruedRewards:           tt.accruedRewards,
				AccruedDelegateeRewards:  tt.accruedDelegateeRewards,
				AutoCompoundRewardShares: validatorTx.AutoCompoundRewardShares,
				Period:                   duration, // Period > 0 means running (not graceful exit)
			}))
			...
			// On abort: supply must be reduced by only the current cycle's
			// PotentialReward. AccruedRewards from prior cycles must NOT be
			// subtracted — they were already minted into supply when those
			// cycles committed.
			abortSupply, err := onAbortState.GetCurrentSupply(constants.PrimaryNetworkID)
			require.NoError(err)
			require.Equal(currentSupply-potentialReward, abortSupply)

			// Validator must be removed from the staker set on abort.
			_, err = onAbortState.GetCurrentValidator(staker.SubnetID, staker.NodeID)
			require.ErrorIs(err, database.ErrNotFound)

			// Stake outputs must be returned on abort.
			for i, stakeOut := range validatorTx.StakeOuts {
				...
				require.NoError(err)
				require.Equal(stakeOut.Out.Amount(), utxo.Out.(*secp256k1fx.TransferOutput).Amt)
			}

			// Reward UTXOs on abort: AccruedRewards (if any), then total delegatee rewards (if any).
			outputIndexOffset := uint32(len(rewardTx.Unsigned.Outputs()))

			if tt.accruedRewards > 0 {
				utxoID := avax.UTXOID{TxID: rewardTx.ID(), OutputIndex: outputIndexOffset}
				utxo, err := onAbortState.GetUTXO(utxoID.InputID())
				require.NoError(err)
				require.Equal(tt.accruedRewards, utxo.Out.(*secp256k1fx.TransferOutput).Amount())
				require.True(validatorTx.ValidatorRewardsOwner.(*secp256k1fx.OutputOwners).Equals(
					&utxo.Out.(*secp256k1fx.TransferOutput).OutputOwners,
				))
				outputIndexOffset++
			}

			totalDelegateeRewards, err := math.Add(tt.delegateeReward, tt.accruedDelegateeRewards)
			require.NoError(err)
			if totalDelegateeRewards > 0 {
				utxoID := avax.UTXOID{TxID: rewardTx.ID(), OutputIndex: outputIndexOffset}
				utxo, err := onAbortState.GetUTXO(utxoID.InputID())
				require.NoError(err)
				require.Equal(totalDelegateeRewards, utxo.Out.(*secp256k1fx.TransferOutput).Amount())
				require.True(validatorTx.DelegatorRewardsOwner.(*secp256k1fx.OutputOwners).Equals(
					&utxo.Out.(*secp256k1fx.TransferOutput).OutputOwners,
				))
				outputIndexOffset++
			}

			// No further reward UTXOs on abort.
			nextUTXOID := avax.UTXOID{TxID: rewardTx.ID(), OutputIndex: outputIndexOffset}
			_, err = onAbortState.GetUTXO(nextUTXOID.InputID())
			require.ErrorIs(err, database.ErrNotFound)

			// On commit: validator must be re-inserted for the next cycle.
			renewedValidator, err := onCommitState.GetCurrentValidator(staker.SubnetID, staker.NodeID)
			require.NoError(err)

			// Commit weight must include restaked rewards from the current cycle.
			restakingRewards, _ := reward.Split(potentialReward, validatorTx.AutoCompoundRewardShares)
			restakingDelegateeRewards, _ := reward.Split(tt.delegateeReward, validatorTx.AutoCompoundRewardShares)
			expectedWeight, err := math.Add(staker.Weight, restakingRewards)
			require.NoError(err)
			expectedWeight, err = math.Add(expectedWeight, restakingDelegateeRewards)
			require.NoError(err)
			require.Equal(expectedWeight, renewedValidator.Weight)

			// Commit AccruedRewards must accumulate on top of prior AccruedRewards.
			updatedStakingInfo, err := onCommitState.GetStakingInfo(staker.SubnetID, staker.NodeID)
			require.NoError(err)
			require.Equal(tt.accruedRewards+restakingRewards, updatedStakingInfo.AccruedRewards)

			// Commit supply must include the next cycle's potential reward.
			commitSupply, err := onCommitState.GetCurrentSupply(constants.PrimaryNetworkID)
			require.NoError(err)
			require.Equal(currentSupply+renewedValidator.PotentialReward, commitSupply)
		})
	}
}
