...
// Following Blueberry activation AddValidatorTx, AddSubnetValidator and AddDelegator
// are moved from ProposalBlocks to Standand blocks.
// Utilities here collects common verifications carried out for both fork.
// Note that utilities do not change state with transactions execution.

// addValidatorValidation carries out validation for AddValidatorTx.
// It returns the whole list of tx outputs, to avoid recalculation it
// upon subsequent state update
func addValidatorValidation(
	backend *Backend,
...
func addSubnetValidatorValidation(
	backend *Backend,
	parentState state.Chain,
	sTx *txs.Tx,
...
// addDelegatorValidation carries out validation for AddDelegatorTx.
// It returns the whole list of tx outputs, to avoid recalculation it
// upon subsequent state update
func addDelegatorValidation(
	backend *Backend,
	parentState state.Chain,
	sTx *txs.Tx,
