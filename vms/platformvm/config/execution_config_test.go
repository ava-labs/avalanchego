	...
	t.Run("all values extracted from json", func(t *testing.T) {
		...
		b := []byte(`{
			"network": {
				...
				"push-gossip-discarded-cache-size": 3,
				"push-gossip-max-regossip-frequency": 4,
				"push-gossip-frequency": 5,
				"pull-gossip-poll-size": 6,
				"pull-gossip-frequency": 7,
				"pull-gossip-throttling-period": 8,
				"pull-gossip-throttling-limit": 9,
				"expected-bloom-filter-elements": 10,
				"expected-bloom-filter-false-positive-probability": 11,
				"max-bloom-filter-false-positive-probability": 12
			},
			"block-cache-size": 1,
			"tx-cache-size": 2,
		...
		expected := &ExecutionConfig{
			Network: network.Config{
				...
				TargetGossipSize:                            2,
				PushGossipDiscardedCacheSize:                3,
				PushGossipMaxRegossipFrequency:              4,
				PushGossipFrequency:                         5,
				PullGossipPollSize:                          6,
				PullGossipFrequency:                         7,
				PullGossipThrottlingPeriod:                  8,
				PullGossipThrottlingLimit:                   9,
				ExpectedBloomFilterElements:                 10,
				ExpectedBloomFilterFalsePositiveProbability: 11,
				MaxBloomFilterFalsePositiveProbability:      12,
			},
			BlockCacheSize:               1,
			TxCacheSize:                  2,
