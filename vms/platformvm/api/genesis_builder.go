...
// BuildGenesisParams are the arguments used to create
...
type BuildGenesisParams struct {
...
func BuildGenesis(params BuildGenesisParams) (string, error) {
	// Specify the UTXOs on the Platform chain that exist at genesis.
	utxos := make([]*genesis.UTXO, 0, len(params.UTXOs))
	for i, apiUTXO := range params.UTXOs {
		if apiUTXO.Amount == 0 {
		...
		utxo := avax.UTXO{
			...
			Asset: avax.Asset{ID: params.AvaxAssetID},
	...
	for _, vdr := range params.Validators {
		weight := uint64(0)
		...
		if uint64(vdr.EndTime) <= uint64(params.Time) {
		...
		var (
			baseTx = txs.BaseTx{BaseTx: avax.BaseTx{
				NetworkID:    uint32(params.NetworkID),
				BlockchainID: ids.Empty,
			}}
			validator = txs.Validator{
				NodeID: vdr.NodeID,
				Start:  uint64(params.Time),
	...
	for _, chain := range params.Chains {
		genesisBytes, err := formatting.Decode(params.Encoding, chain.GenesisData)
		if err != nil {
	...
	g := genesis.Genesis{
		...
		Chains:        chains,
		Timestamp:     uint64(params.Time),
		InitialSupply: uint64(params.InitialSupply),
	...
	encodedBytes, err := formatting.Encode(params.Encoding, bytes)
	if err != nil {
