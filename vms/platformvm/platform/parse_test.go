...
func TestStandardBlocks(t *testing.T) {
	...
	for _, cdc := range []codec.Manager{Codec, GenesisCodec} {
		...
		// parse block
		parsed, err := ParseBlock(cdc, apricotStandardBlk.Bytes())
		require.NoError(err)
		...
		// parse block
		parsed, err = ParseBlock(cdc, banffStandardBlk.Bytes())
		require.NoError(err)
...
func TestProposalBlocks(t *testing.T) {
	...
	for _, cdc := range []codec.Manager{Codec, GenesisCodec} {
		...
		// parse block
		parsed, err := ParseBlock(cdc, apricotProposalBlk.Bytes())
		require.NoError(err)
		...
		require.Equal([]*Tx{proposalTx}, parsedApricotProposalBlk.Txs())
		...
		// parse block
		parsed, err = ParseBlock(cdc, banffProposalBlk.Bytes())
		require.NoError(err)
		...
		require.Equal([]*Tx{proposalTx}, parsedBanffProposalBlk.Txs())
		...
		// parse block
		parsed, err = ParseBlock(cdc, banffProposalBlkWithDecisionTxs.Bytes())
		require.NoError(err)
...
func TestCommitBlock(t *testing.T) {
	...
	for _, cdc := range []codec.Manager{Codec, GenesisCodec} {
		...
		// parse block
		parsed, err := ParseBlock(cdc, apricotCommitBlk.Bytes())
		require.NoError(err)
		...
		// parse block
		parsed, err = ParseBlock(cdc, banffCommitBlk.Bytes())
		require.NoError(err)
...
func TestAbortBlock(t *testing.T) {
	...
	for _, cdc := range []codec.Manager{Codec, GenesisCodec} {
		...
		// parse block
		parsed, err := ParseBlock(cdc, apricotAbortBlk.Bytes())
		require.NoError(err)
		...
		// parse block
		parsed, err = ParseBlock(cdc, banffAbortBlk.Bytes())
		require.NoError(err)
...
func TestAtomicBlock(t *testing.T) {
	...
	for _, cdc := range []codec.Manager{Codec, GenesisCodec} {
		...
		// parse block
		parsed, err := ParseBlock(cdc, atomicBlk.Bytes())
		require.NoError(err)
		...
		require.Equal([]*Tx{atomicTx}, parsedAtomicBlk.Txs())
	}
}

func testAtomicTx() (*Tx, error) {
	utx := &ImportTx{
	...
	return NewSignedTx(utx, Codec, signers)
}

func testDecisionTxs() ([]*Tx, error) {
	countTxs := 2
	decisionTxs := make([]*Tx, 0, countTxs)
	for i := 0; i < countTxs; i++ {
		...
		signers := [][]*secp256k1.PrivateKey{{preFundedKeys[0]}}
		tx, err := NewSignedTx(utx, Codec, signers)
		if err != nil {
			return nil, err
		}
...
func testProposalTx() (*Tx, error) {
	utx := &RewardValidatorTx{
