...
func TestAddDelegatorTxSyntacticVerify(t *testing.T) {
	...
	if err := unsignedTx.Verify(vm.Ctx, vm.codec, vm.txFee, vm.Ctx.AVAXAssetID, vm.minValidatorStake); err == nil {
	...
	if err := tx.UnsignedTx.(*UnsignedAddDelegatorTx).Verify(vm.Ctx, vm.codec, vm.txFee, vm.Ctx.AVAXAssetID, vm.minValidatorStake); err == nil {
	...
	tx.UnsignedTx.(*UnsignedAddDelegatorTx).Validator.Wght = vm.minValidatorStake - 1
	// This tx was syntactically verified when it was created...pretend it wasn't so we don't use cache
	tx.UnsignedTx.(*UnsignedAddDelegatorTx).syntacticallyVerified = false
	if err := tx.UnsignedTx.(*UnsignedAddDelegatorTx).Verify(vm.Ctx, vm.codec, vm.txFee, vm.Ctx.AVAXAssetID, vm.minValidatorStake); err == nil {
	...
	if tx, err = vm.newAddDelegatorTx(
		...
		t.Fatal(err)
	} else if err := tx.UnsignedTx.(*UnsignedAddDelegatorTx).Verify(vm.Ctx, vm.codec, vm.txFee, vm.Ctx.AVAXAssetID, vm.minValidatorStake); err != nil {
		t.Fatal(err)
	}
}
