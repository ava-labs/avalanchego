...
// readOnlySchemes are the schemes the read-only guarantee must hold on. Both
// are needed because the backends differ under the wrapper, Firewood returning
// a nil storage trie copy where HashDB returns a real one.
