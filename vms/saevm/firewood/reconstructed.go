...
func NewReconstructedDatabase(db state.Database, tdb *TrieDB, recon *ffi.Reconstructed, root common.Hash) (state.Database, func()) {
	...
	session := &reconstructedSession{views: views}
	session.cleanup = runtime.AddCleanup(session, func(views *reconstructedViews) {
		views.close()
	}, views)
...
// until the session shared by the original state and every copy is unreachable.
type reconstructedSession struct {
	views   *reconstructedViews
	cleanup runtime.Cleanup
}

// reconstructedViews owns every native view created for one reconstructed
// database.
type reconstructedViews struct {
	lock  sync.Mutex
...
func (s *reconstructedSession) clone(view *ffi.Reconstructed) (*ffi.Reconstructed, error) {
	s.views.lock.Lock()
	defer s.views.lock.Unlock()
	if s.views.views == nil {
		return nil, errors.New("reconstructed session released")
	}
	clone, err := view.Clone()
	if err != nil {
		return nil, err
	}
	s.views.views = append(s.views.views, clone)
	return clone, nil
}
...
func (s *reconstructedViews) close() {
	s.lock.Lock()
	views := s.views
	s.views = nil
	s.lock.Unlock()
...
func (a *reconstructedAccountTrie) Hash() common.Hash {
	...
	if err := a.recon.Reconstruct(a.updateOps); err != nil {
		a.session.views.log.Error("hashing reconstructed account trie", zap.Error(err))
		return common.Hash{}
	}
	a.updateOps = nil
...
func (a *reconstructedAccountTrie) Copy() *reconstructedAccountTrie {
	...
	if err != nil {
		a.session.views.log.Error("copying reconstructed account trie", zap.Error(err))
		return nil
	}
