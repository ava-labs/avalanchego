...
// when the settler header carries a self-settling settlement marker — the
// shape of a header with no effective marker — instead of silently treating
// it as settling nothing.
...
func TestIsSynchronous(t *testing.T) {
	...
	// and so counts as no marker — synchronous, like the Helicon genesis. The
	// stub decodes a markerless header to the zero-value marker, which is
	// self-settling only at height 0; see [hookstest.Stub.SettledBy].
	for name, tt := range map[string]struct {
		settled       *hook.Settled // nil for a header carrying no marker
		headerNumber  int64
		isSynchronous bool
	}{
		"markerless_genesis_header": {
			settled:       nil,
			headerNumber:  0,
			isSynchronous: true,
		},
