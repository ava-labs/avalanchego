...
import (
	...
	_ "github.com/ava-labs/libevm/core/rawdb" // for comment linking
)
...
type ChainVM interface {
	...
	// AcceptBlock follows all the guarantees of [adaptor.ChainVM.AcceptBlock],
	// and additionally MUST enable the following database reads:
	// - [rawdb.ReadCanonicalHash]
	// - [rawdb.ReadHeaderNumber]
	// - [rawdb.ReadBlock]
	// - [rawdb.ReadHeadFastBlockHash] corresponding with the last accepted block
...
// shared contains all methods that route to the summary handler until the chain
// is built, then route to the chain. This allows the engine to startup without
// fully initializing the VM.
...
func (o *orchestrator[_]) activeHandler() shared {
