...
| `api-max-duration` | duration | Maximum duration of an `eth_call` (or `eth_callDetailed`) execution. Accepts a [Go duration string](https://pkg.go.dev/time#ParseDuration) (e.g. `"30s"`, `"2h45m"`); valid units are `ns`, `us`, `ms`, `s`, `m` and `h`. MUST be positive. | `"10s"` |
