...
func (nm *networkedMachine) deliverBlock(rt *rapid.T, n *modelNode, ab acceptedBlock) {
	...
	if ab.root != (common.Hash{}) {
		require.Equalf(rt, ab.root, blk.PostExecutionStateRoot(),
			"node %d post-execution state root at height %d", n.idx, ab.height)
	}
...
func (nm *networkedMachine) buildAndDistribute(rt *rapid.T) {
	...
	ab.root = blk.PostExecutionStateRoot() // builder executed above; safe to read
...
func (nm *networkedMachine) competingSiblings(rt *rapid.T) {
	...
	if blkA.ID() == blkB.ID() {
		...
		require.Equalf(rt, blkA.PostExecutionStateRoot(), blkB.PostExecutionStateRoot(),
			"identical siblings' post-execution state roots on builders %d and %d", j.idx, k.idx)
		ab.root = blkA.PostExecutionStateRoot()
	...
	// Every node executed its own winner handle above; all roots must agree.
	wantRoot := winner.PostExecutionStateRoot()
	for _, n := range nm.nodes {
		require.Equalf(rt, wantRoot, wins[n.idx].PostExecutionStateRoot(),
			"node %d winner-sibling post-execution state root", n.idx)
	}
	wb.root = wantRoot
