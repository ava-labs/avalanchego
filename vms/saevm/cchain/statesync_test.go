...
// its cross-chain tx.
func (s *SUT) produceBlocks(ctx context.Context, t *testing.T, w *wallet, ethW *saetest.Wallet, n int) (*blocks.Block, *tx.Tx) {
	t.Helper()
	...
	for range n {
		...
		blk = s.acceptBlockWith(ctx, t, stx, newEthTx(t, ethW))
	}
	return blk, stx
}
...
// and on the block ID at every height up to it. It deliberately compares IDs
// rather than full blocks: a state-synced node never executed blocks below the
// summary height, so it cannot serve their receipts.
func (s *SUT) assertChainsMatch(ctx context.Context, t *testing.T, other *SUT) {
	...
	for height := range s.lastAcceptedHeight(ctx, t) + 1 {
		want, err := other.GetBlockIDAtHeight(ctx, height)
		require.NoErrorf(t, err, "%T.GetBlockIDAtHeight(%d)", other.VM, height)
		got, err := s.GetBlockIDAtHeight(ctx, height)
		require.NoErrorf(t, err, "%T.GetBlockIDAtHeight(%d)", s.VM, height)
		require.Equalf(t, want, got, "%T.GetBlockIDAtHeight(%d)", s.VM, height)
	}
}
...
func TestStateSyncNewNodeJoins(t *testing.T) {
	...
	exportTx, export := w.newExportTx(
		t,
		snowtest.XChainID,
		txFee,
		txtest.NewTransferOutput(utxoAmount, key.Address()),
	)
	exportedUTXOs := txtest.ExportedUTXOs(exportTx.ID(), export)
	...
	lastBlk, lastTx := src.produceBlocks(srcCtx, t, w, ethW, commitInterval)

	ctx, dst := newSUT(t, append(sharedOpts, withState(snow.StateSyncing))...)
	saetest.ConnectTo(t, dst, src)

	dst.addUTXOs(t, dst.ctx.ChainID, snowtest.XChainID, seededUTXO)
	...
	// Check EVM state - bootstrapped state root is sufficient
	gotLastBlk := dst.blockAtHeight(ctx, t, dst.lastAcceptedHeight(ctx, t))
	require.Equal(t, lastBlk.PostExecutionStateRoot(), gotLastBlk.PostExecutionStateRoot(), "last post-execution state root")

	// Synced atomic state: the export's UTXOs reached dst's shared memory and
	// the import consumed the seeded UTXO.
	dst.assertUTXOsExist(t, snowtest.XChainID, dst.ctx.ChainID, exportedUTXOs...)
	dst.assertUTXOsMissing(t, dst.ctx.ChainID, snowtest.XChainID, seededUTXO)

	// Txs in blocks replayed during bootstrap were indexed and are served over
	// dst's own RPC client. Note that txs below the summary height (e.g.
	// exportTx and importTx) are NOT in dst's tx index.
	dst.assertTxAccepted(ctx, t, lastTx, lastBlk.Height())

	// dst keeps up with the network...
	blk, _ := src.produceBlocks(srcCtx, t, w, ethW, 1)
...
// engine.
func FuzzStateSyncDBFailure(f *testing.F) {
	for _, n := range []int{0, 1, 8, math.MaxInt} {
		f.Add(n)
	}
