...
func RegisterSyncHandler(n *p2p.Network, state *State) error {
	return hashdb.RegisterHandler(state.snowCtx.Log, n, state.trieDB, keyLength, nil, p2p.EVMAtomicLeafRequestHandlerID)
}

// Syncer is a [leaf.LeafSyncer] that can fetch and apply the atomic trie to a
// [State] and update shared memory.
type Syncer struct {
	fetcher *hashdb.LeafFetcher
...
func NewSyncer(n *p2p.Network, pt *p2p.PeerTracker, state *State, root common.Hash, height uint64) *Syncer {
	...
	return &Syncer{
		fetcher: hashdb.NewLeafFetcher(
			state.snowCtx.Log,
			hashdb.NewClient(n, pt, p2p.EVMAtomicLeafRequestHandlerID),
			tasks, 1,
		),
		targetRoot:   root,
		targetHeight: height,
...
func (s *Syncer) Sync(ctx context.Context) error {
	if err := s.fetcher.Sync(ctx); err != nil {
...
var _ hashdb.Task = (*syncTask)(nil)
...
// OnLeaves is called on each batch from the [hashdb.LeafFetcher]. All state is
// queued to be committed for each individual height. Any error returned will
// be treated as fatal.
func (t *syncTask) OnLeaves(_ context.Context, batch hashdb.LeafBatch) error {
...
// [syncTask.OnLeaves]. Any remaining leaves are pushed to disk, as the last
