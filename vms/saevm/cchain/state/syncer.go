...
func RegisterSyncHandler(n *p2p.Network, state *State) error {
	return evmstate.RegisterHandler(state.snowCtx.Log, n, p2p.EVMAtomicLeafRequestHandlerID, state.trieDB, keyLength)
}

// Syncer is a [leaf.Syncer] that can fetch and apply the atomic trie to a
...
func NewSyncer(n *p2p.Network, pt *p2p.PeerTracker, state *State, root common.Hash, height uint64) *Syncer {
	...
	return &Syncer{
		fetcher: leaf.NewSyncer(
			leafproto.NewClient(
				state.snowCtx.Log,
				evmstate.NewClient(n, p2p.EVMAtomicLeafRequestHandlerID, pt),
			),
			tasks,
		),
		targetRoot:   root,
...
// treated as fatal.
func (t *task) OnLeaves(_ context.Context, batch types.Leaves) error {
