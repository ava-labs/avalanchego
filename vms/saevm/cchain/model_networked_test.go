...
// atomic is nil when the block carries no cross-chain txs. root is the
// producer's PostExecutionStateRoot, filled by the producing action AFTER
// the block executed there (PostExecutionStateRoot blocks until execution);
// it is zero only for the in-flight ab of competingSiblings' identical-
// sibling branch, whose immediate deliveries predate resolution — the
// stored canonical entry is always rooted.
...
func newNetworkedMachine(t *testing.T, rt *rapid.T, cfg networkedRunConfig) *networkedMachine {
	...
	nm := &networkedMachine{
		modelCore: &modelCore{
			...
			vdrs:   warptest.NewValidators(tb, warptest.WithNodeIDs(vdrIDs...)),
		},
		cfg:          cfg,
		clock:        clock,
		timeOpt:      timeOpt,
