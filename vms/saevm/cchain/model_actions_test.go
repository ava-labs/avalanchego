...
func (c *modelCore) applyBlock(rt *rapid.T, ctx context.Context, sut *SUT, blk *blocks.Block) {
	...
	// Receipt-set completeness and worst-case block invariants (design
	// §1.2/§1.3). saexec builds receipts indexed 1:1 with Transactions()
	// (atomic txs live in BlockExtData and produce no receipts), so order
	// and length must match exactly. The ramp above has already advanced
	// m.price, so its Price() is this block's dynamic-minimum floor.
	txs := blk.Transactions()
	rs := blk.Receipts()
	require.Lenf(rt, rs, len(txs), "block %d receipts vs transactions", blk.NumberU64())
	floor := uint256.NewInt(uint64(m.price.Price()))
	wcBaseFee := blk.WorstCaseBaseFee()
	require.Truef(rt, wcBaseFee.Cmp(floor) >= 0,
		"block %d WorstCaseBaseFee %s below model price floor %s", blk.NumberU64(), wcBaseFee, floor)
	var cumulative uint64
	for i, r := range rs {
		ethTx := txs[i]
		require.Equalf(rt, ethTx.Hash(), r.TxHash, "block %d receipt %d pairs with tx %d", blk.NumberU64(), i, i)
		require.GreaterOrEqualf(rt, r.GasUsed, ethparams.TxGas, "block %d tx %d gasUsed below intrinsic", blk.NumberU64(), i)
		require.LessOrEqualf(rt, r.GasUsed, ethTx.Gas(), "block %d tx %d gasUsed above its gas limit", blk.NumberU64(), i)
		cumulative += r.GasUsed
		require.Equalf(rt, cumulative, r.CumulativeGasUsed, "block %d tx %d cumulative gas", blk.NumberU64(), i)

		egp, overflow := uint256.FromBig(r.EffectiveGasPrice)
		require.Falsef(rt, overflow, "block %d tx %d EffectiveGasPrice overflows uint256", blk.NumberU64(), i)
		feeCap, overflow := uint256.FromBig(ethTx.GasFeeCap())
		require.Falsef(rt, overflow, "block %d tx %d GasFeeCap overflows uint256", blk.NumberU64(), i)
		require.Truef(rt, egp.Cmp(feeCap) <= 0,
			"block %d tx %d effective price %s above fee cap %s", blk.NumberU64(), i, egp, feeCap)
		require.Truef(rt, egp.Cmp(floor) >= 0,
			"block %d tx %d effective price %s below model price floor %s", blk.NumberU64(), i, egp, floor)
		require.Truef(rt, feeCap.Cmp(wcBaseFee) >= 0,
			"block %d tx %d fee cap %s below WorstCaseBaseFee %s (builder inclusion rule)", blk.NumberU64(), i, feeCap, wcBaseFee)
	}
	require.LessOrEqualf(rt, cumulative, blk.WorstCaseGasUsed(),
		"block %d actual gas above worst-case gas", blk.NumberU64())
