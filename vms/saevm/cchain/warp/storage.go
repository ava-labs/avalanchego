...
type Storage struct {
	...
	// cache only ever holds messages backed by db; overrides are returned
...
// Has reports whether this chain emitted the message with the given ID.
// Unlike [Storage.Get], it never consults the node-local overrides:
// off-chain messages must not satisfy consensus checks.
