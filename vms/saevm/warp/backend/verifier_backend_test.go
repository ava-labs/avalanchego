...
type blockClientFunc func(ctx context.Context, blockID ids.ID) error

func (f blockClientFunc) IsAccepted(ctx context.Context, blockID ids.ID) error {
	return f(ctx, blockID)
}

func makeBlockClient(accepted ...ids.ID) blockClientFunc {
	return func(_ context.Context, blockID ids.ID) error {
...
func TestAddressedCallSignatures(t *testing.T) {
	...
	tests := map[string]struct {
		setup       func(backend precompileconfig.WarpMessageWriter) (request []byte, expectedResponse []byte)
		...
		"offchain message": {
			setup: func(_ precompileconfig.WarpMessageWriter) (request []byte, expectedResponse []byte) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			warpBackend, sigCache, err := New(snowCtx.NetworkID, snowCtx.ChainID, snowCtx.WarpSigner, makeBlockClient(), database, [][]byte{offchainMessage.Bytes()})
			require.NoError(t, err)
			...
			test.verifyStats(t, warpBackend.stats)

			if len(expectedResponse) == 0 {
				require.Empty(t, responseBytes, "expected response to be empty")
				return
			}
			require.NotZero(t, warpBackend.signatureCache.Len())
...
func TestBlockSignatures(t *testing.T) {
	...
	blockClient := makeBlockClient(knownBlkID)
