...
//
//  1. Discard (still present; see the design's deferred follow-up): recovery
//     boots from the last root whose trie is committed on disk (height 12 at
//     commitInterval = 4), while the persisted snapshot disk layer sits at
//     the root of libevm's single post-generation flush (Tree.Cap flushes
//     once more right after generation completes; later depth-1 flattens
//     buffer in memory at this state size, per [saedb.SnapshotCapLayers] =
//     1). The roots differ, libevm's loadSnapshot treats that as fatal, and
//     snapshot.New discards the snapshot and regenerates from key zero.
...
func TestSnapshotRegenerationAfterRecoveryCompletes(t *testing.T) {
	...
	settledRoots := make([]common.Hash, 0, numBlocks)
	for range numBlocks {
		b := transfer(src)
		require.NoErrorf(t, b.WaitUntilExecuted(ctx), "%T.WaitUntilExecuted()", b)
		settledRoots = append(settledRoots, b.SettledStateRoot())
	}

	// The healthy baseline: generation stayed complete, and the persisted
	// disk root differs from the boot root recovery will use — the
	// precondition for the discard-and-rebuild path under regression test
	// here. At the highest height h <= numBlocks with
	// saedb.ShouldCommitTrieDB(h, commitInterval), [saedb.Tracker.MaybeCommit]
	// commits height h's block's *settled* root (some earlier block's
	// post-execution root, per SAE's settlement lag) — not height h's own
	// post-execution root — and that is the only root recovery can find
	// resolvable on disk after an unclean crash, so it is what recovery boots
	// from. The persisted snapshot disk root, meanwhile, sits wherever
	// libevm's single post-generation flush (see the doc comment above) left
	// it. numBlocks must be >= commitInterval for bootHeight to be positive.
	gen, ok := readGenerator(src.db)
	...
	bootHeight := saedb.LastCommittedTrieDBHeight(numBlocks, commitInterval)
	require.Positive(t, bootHeight, "LastCommittedTrieDBHeight(%d, %d): numBlocks must reach at least one commitInterval boundary", numBlocks, commitInterval)
	bootRoot := settledRoots[bootHeight-1]
	persistedRoot := rawdb.ReadSnapshotRoot(src.db)
	require.NotEqual(t, bootRoot, persistedRoot, "persisted snapshot disk root vs recovery's boot root: they must differ for recovery to discard and rebuild the snapshot — the path under regression test")
	...
	// Discard (1): the boot root differed from the persisted disk root (see
	// the baseline assertion above), so recovery discarded the complete
	// snapshot and started regenerating from key zero. The gate guarantees it
