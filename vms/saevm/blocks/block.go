...
type Block struct {
	...
	//
	// INVARIANT: equal to [hook.IsSynchronous] for the block's header, under
	// the hooks with which the block was restored. It is written exactly
	// once, by [Block.restoreExecutionArtefacts]. Blocks constructed via
	// [New] with ancestry are never synchronous: the block builder rejects
	// self-settling markers and block verification rejects markerless
	// headers, so every header reaching them carries an effective settlement
	// marker.
	synchronous bool
