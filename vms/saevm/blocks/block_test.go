...
func newBlock(tb testing.TB, eth *types.Block, parent *Block) *Block {
	tb.Helper()
	b, err := New(eth, parent, hooks(), loggingtest.New(tb, logging.Warn))
...
func TestSetAncestors(t *testing.T) {
	...
	t.Run("incorrect_parent", func(t *testing.T) {
		_, err := New(child, lastSettled /*instead of parent*/, hooks(), loggingtest.New(t, logging.Warn))
		require.ErrorIs(t, err, errParentHashMismatch, "New() with inverted parent and last-settled blocks")
	})

	source := newBlock(t, child, parent)
	dest := newBlock(t, child, nil)
	...
	t.Run("not_incrementing_height", func(t *testing.T) {
		...
		_, err := New(ethB, parent, hooks(), logging.NoLog{})
