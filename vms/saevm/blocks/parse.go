...
// the universal invariants that every block MUST satisfy, applies the
// chain-specific checks via [hook.Points.VerifyBlockSyntax], and wraps the
// result in an unverified [Block]. It is the only way to turn bytes into a
// [Block], guaranteeing that no parse path can skip verification.
func Parse(buf []byte, hooks hook.Points, log logging.Logger) (*Block, error) {
