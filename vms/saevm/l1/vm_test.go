...
import (
	...
	"github.com/ava-labs/avalanchego/vms/saevm/l1/api/client"
...
type SUT struct {
	...
	// `*ethclient.Client`) and the L1-specific methods.
...
func newTestGenesis(fork upgradetest.Fork, keychain *saetest.KeyChain) *core.Genesis {
	chainConfig := legacyparams.Copy(paramstest.ForkToChainConfig[fork])
	return &core.Genesis{
		Config:     &chainConfig,
...
func (s *SUT) signTransferTx(t *testing.T, from int, to int, value *big.Int) *types.Transaction {
	...
	return s.ethWallet.SetNonceAndSign(t, from, &types.DynamicFeeTx{
		To:        &toAddress,
		Gas:       21_000,
		GasFeeCap: big.NewInt(225 * legacyparams.GWei),
		Value:     value,
	})
}
...
func (s *SUT) isPrecompileEnabledAtLatest(precompile common.Address) bool {
	...
	return legacyparams.GetExtra(chainConfig).IsPrecompileEnabled(precompile, s.vm.clock.Unix())
}
...
// `StartExecutingBlock` -> `legacycore.ApplyUpgrades`.
//
