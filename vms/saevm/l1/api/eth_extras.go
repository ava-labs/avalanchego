...
// EthExtrasAPI serves the L1-specific `eth_*` JSON-RPC methods, with wire
// shapes matching the legacy VM.
...
func (s *EthExtrasAPI) GetActiveRulesAt(_ context.Context, blockTimestamp *uint64) ActiveRulesResult {
	...
	rules := s.backend.ChainConfig().Rules(common.Big0, legacyparams.IsMergeTODO, timestamp)
	res := ActiveRulesResult{
		EthRules:          rules,
		AvalancheRules:    legacyparams.GetRulesExtra(rules).AvalancheRules,
		ActivePrecompiles: map[string]ActivePrecompilesResult{},
	}
	for _, cfg := range legacyparams.GetRulesExtra(rules).Precompiles {
