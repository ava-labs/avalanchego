...
func parseGenesis(ctx *snow.Context, genesisBytes []byte, upgradeBytes []byte) (*core.Genesis, error) {
	...
	// plugin.
	if g.Config == nil {
		g.Config = legacyparams.SubnetEVMDefaultChainConfig
	}

	// Populate the Avalanche config extras.
	configExtra := legacyparams.GetExtra(g.Config)
	...
	if configExtra.FeeConfig == commontype.EmptyFeeConfig {
		...
		configExtra.FeeConfig = legacyparams.DefaultFeeConfig
	}
	...
	if err := legacyparams.SetEthUpgrades(g.Config); err != nil {
