...
`l1` is the SAE-based EVM for Avalanche L1s. It uses the generic execution,
settlement, and Warp machinery in [`vms/saevm`](../), while reusing the
[legacy EVM implementation](../../../graft/subnet-evm) for compatible chain
configuration, header extensions, precompiles, and upgrade semantics. It runs
as an out-of-process `rpcchainvm` plugin.
...
```sh
go test ./vms/saevm/l1/...
task test-e2e-warp-l1
```
...
L1-specific code is concentrated here:

- [`genesis.go`](genesis.go) parses legacy-compatible genesis and upgrade bytes.
- [`hooks.go`](hooks.go) connects L1 rules and precompiles to SAE.
...
- [`warp`](warp) adds L1 Warp predicates and uptime verification.
...
The legacy `customtypes.HeaderExtra` type carries the SAE settlement marker,
...
effective tip and the base fee is burned. The legacy VM credited the full
