...
func TestParseGenesisNetworkUpgradeOverrides(t *testing.T) {
	...
	require.Equal(
		t,
		wantEtnaTimestamp,
		*legacyparams.GetExtra(got.Config).EtnaTimestamp,
...
func TestParseGenesisDefaultsFeeConfig(t *testing.T) {
	...
	legacyparams.GetExtra(genesis.Config).FeeConfig = commontype.EmptyFeeConfig
	genesisBytes, err := json.Marshal(genesis)
	require.NoError(t, err, "json.Marshal(genesis)")
	...
	require.Equal(
		t,
		legacyparams.DefaultFeeConfig,
		legacyparams.GetExtra(got.Config).FeeConfig,
