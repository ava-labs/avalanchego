...
const (
	canotoNumber_Summary__height    = 1
	canotoNumber_Summary__blockHash = 2

	canotoTag_Summary__height    = "\x08" // canoto.Tag(canotoNumber_Summary__height, canoto.Varint)
	canotoTag_Summary__blockHash = "\x12" // canoto.Tag(canotoNumber_Summary__blockHash, canoto.Len)
)
...
func (*Summary) CanotoSpec(...reflect.Type) *canoto.Spec {
	...
	s := &canoto.Spec{
		...
		Fields: []canoto.FieldType{
			{
				FieldNumber: canotoNumber_Summary__height,
				Name:        "height",
				OneOf:       "",
				TypeUint:    canoto.SizeOf(zero.height),
			},
			{
				FieldNumber:    canotoNumber_Summary__blockHash,
				Name:           "blockHash",
				OneOf:          "",
...
func (c *Summary) UnmarshalCanotoFrom(r canoto.Reader) error {
	...
	*c = Summary{}
	...
	for canoto.HasNext(&r) {
		...
		switch field {
		case canotoNumber_Summary__height:
		...
		case canotoNumber_Summary__blockHash:
...
func (c *Summary) CalculateCanotoCache() {
	var size uint64
	if !canoto.IsZero(c.height) {
		size += uint64(len(canotoTag_Summary__height)) + canoto.SizeUint(c.height)
	}
	if !canoto.IsZero(c.blockHash) {
		size += uint64(len(canotoTag_Summary__blockHash)) + canoto.SizeBytes((&c.blockHash)[:])
	}
...
func (c *Summary) MarshalCanotoInto(w canoto.Writer) canoto.Writer {
	if !canoto.IsZero(c.height) {
		canoto.Append(&w, canotoTag_Summary__height)
		canoto.AppendUint(&w, c.height)
	}
	if !canoto.IsZero(c.blockHash) {
		canoto.Append(&w, canotoTag_Summary__blockHash)
