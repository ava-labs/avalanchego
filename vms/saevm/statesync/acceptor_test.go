...
func TestStateSyncEnabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sut := newSUT(t, withoutInitialization(), withEnabled(tt.enabled))
			got, err := sut.StateSyncEnabled(t.Context())
...
// TestAcceptSummary checks that various configs and states will correctly
...
func TestAcceptSummary(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := <-done
			assert.ErrorIsf(t, r.err, wantErr, "%T.WaitForEvent()", sut.SummaryHandler)
