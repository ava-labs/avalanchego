...
var _ messengerpb.MessengerServer = (*Server)(nil)
...
func (s *Server) SubscribeToEvents(ctx context.Context) (common.Message, error) {
	...
	if stream == nil {
		return 0, errors.New("stream is nil, subscription attempted timed out or was cancelled")
	}

	if err := stream.Send(startRequest()); err != nil {
		s.logger.Error("failed to send subscription request", zap.Error(err))
		return 0, fmt.Errorf("failed to send subscription request: %w", err)
	}
	...
	case <-ctx.Done():
		if err := stream.Send(stopRequest()); err != nil {
			...
			return 0, fmt.Errorf("failed to send stop request: %w", err)
		}
		return 0, nil
	case event := <-s.events:
...
func startRequest() *messengerpb.EventRequest {
	return &messengerpb.EventRequest{Event: &messengerpb.EventRequest_Start{Start: true}}
}
