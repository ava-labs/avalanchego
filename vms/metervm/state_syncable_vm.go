...
func (vm *blockVM) StateSyncEnabled(ctx context.Context) (bool, error) {
	...
	vm.stateSyncEnabled.Observe(float64(end.Sub(start)))
	return enabled, err
}
...
func (vm *blockVM) GetOngoingSyncStateSummary(ctx context.Context) (block.StateSummary, error) {
	...
	vm.getOngoingSyncStateSummary.Observe(float64(end.Sub(start)))
	return summary, err
}
...
func (vm *blockVM) GetLastStateSummary(ctx context.Context) (block.StateSummary, error) {
	...
	vm.getLastStateSummary.Observe(float64(end.Sub(start)))
	return summary, err
}
...
func (vm *blockVM) ParseStateSummary(ctx context.Context, summaryBytes []byte) (block.StateSummary, error) {
	...
	if err != nil {
		vm.parseStateSummaryErr.Observe(duration)
		return nil, err
	}
	vm.parseStateSummary.Observe(duration)
	return summary, nil
}
...
func (vm *blockVM) GetStateSummary(ctx context.Context, height uint64) (block.StateSummary, error) {
	...
	duration := float64(end.Sub(start))
	if err != nil {
		vm.getStateSummaryErr.Observe(duration)
		return nil, err
	}
	vm.getStateSummary.Observe(duration)
	return summary, nil
}
