...
type StateReader interface {
	...
	// GetValidationIDs returns the set of validation IDs in the state
	GetValidationIDs() set.Set[ids.ID]
	// GetNodeIDs returns the set of validator node IDs in the state
...
type State interface {
	...
	// UpdateValidator updates an existing validator in the state
	UpdateValidator(vdr Validator) error
	// DeleteValidator removes a validator from the state
	DeleteValidator(vID ids.ID) error
	// WriteState writes the validator state to the disk
	WriteState() error
	// RegisterListener registers a callback listener for state changes
