...
func runStateSync(t *testing.T, ctx context.Context, f *synctest.StateFixture) ethdb.Database {
	...
	require.NoError(t, RegisterHandler(logging.NoLog{}, net, f.TrieDB, common.HashLength, nil))
	...
	codeSyncer := code.NewSyncer(logging.NoLog{}, code.NewClient(net, tracker), target, queue.CodeHashes())
