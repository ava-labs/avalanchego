...
// targetBlockHeightKey stores the head height captured at first run for ETA only.
...
func (m *migrator) run(ctx context.Context) error {
	var (
		...
		// under the header prefix that we are not migrating (e.g., canonical hash mappings).
	...
	for iter.Next() {
		...
		if err := m.migrateHeader(blockNum, hash); err != nil {
			return fmt.Errorf("failed to migrate header data: %w", err)
		}
		if err := m.migrateBody(blockNum, hash, iter.Value()); err != nil {
		...
		if processed-lastCompact >= compactionInterval {
			...
			iter = m.evmDB.NewIterator(evmBlockBodyPrefix, start)
...
func deleteBlock(db ethdb.KeyValueWriter, num uint64, hash common.Hash) error {
	// Avoid rawdb.DeleteHeader to preserve number/hash canonical mappings during migration.
...
func isMigratableKey(db ethdb.Reader, key []byte) bool {
	...
	// Skip genesis since all nodes have it and we won't benefit from
	// minHeight > 0 when state-syncing.
	if num == 0 {
		return false
	}
...
func minBlockHeightToMigrate(db ethdb.Database) (uint64, bool, error) {
	...
	for iter.Next() {
		...
		blockNum, _, err := parseBlockKey(key)
		if err != nil {
			continue
		}
