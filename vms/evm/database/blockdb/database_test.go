...
func TestDatabaseWriteDifferentBlocksAtSameHeight(t *testing.T) {
	...
	blocks2, receipts2 := createBlocksToUser(t, 2, to)
	b2 := blocks2[1]
	r2 := receipts2[1]
	...
	require.False(t, rawdb.HasHeader(db, b1.Hash(), b1.NumberU64()))
	require.False(t, rawdb.HasBody(db, b1.Hash(), b1.NumberU64()))
	require.False(t, rawdb.HasReceipts(db, b1.Hash(), b1.NumberU64()))
...
func TestDatabaseGenesisBlockHandling(t *testing.T) {
	...
	// Validate genesis block can be retrieved and its stored in evmDB.
