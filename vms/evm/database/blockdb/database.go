...
//
// The second return value is true if the block databases were initialized,
// and false if deferred.
func New(
	stateDB database.Database,
	...
	// and no blocks exist to migrate.
...
// InitBlockDBs initializes [database.HeightIndex] databases for blocks
// with the specified minimum height.
...
func databaseMinHeight(db database.KeyValueReader) (uint64, bool, error) {
	...
	height := binary.BigEndian.Uint64(minBytes)
...
// Returns true if the minimum block height key exists, which indicates the
// block database has been created and initialized before.
