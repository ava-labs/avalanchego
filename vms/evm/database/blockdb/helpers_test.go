...
// createBlocksToUser generates blocks with a single funded sender and a tx to the provided recipient.
func createBlocksToUser(t *testing.T, numBlocks int, to common.Address) ([]*types.Block, []types.Receipts) {
	t.Helper()
...
func createBlocks(t *testing.T, numBlocks int) ([]*types.Block, []types.Receipts) {
	...
	return createBlocksToUser(t, numBlocks, to)
}
...
func startMigration(t *testing.T, db *Database, waitForCompletion bool) {
	...
	if waitForCompletion {
		...
		require.True(t, waitMigratorDone(db.migrator, timeout), msg)
...
func waitMigratorDone(m *migrator, timeout time.Duration) bool {
