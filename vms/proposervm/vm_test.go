...
func TestExpiredBuildBlock(t *testing.T) {
	...
	coreVM.SubscribeToEventsF = func(context.Context) (common.Message, error) {
		select {
		case <-testEnd:
			return 0, nil
		case event := <-events:
...
func TestVMInnerBlkCache(t *testing.T) {
	...
	innerVM.EXPECT().SubscribeToEvents(gomock.Any()).Return(common.PendingTxs, nil).AnyTimes()
...
func TestVM_VerifyBlockWithContext(t *testing.T) {
	...
	// Create a VM
	innerVM := blockmock.NewChainVM(ctrl)
	innerVM.EXPECT().SubscribeToEvents(gomock.Any()).Return(common.Message(0), nil).AnyTimes()
...
func TestLocalParse(t *testing.T) {
	...
	innerVM.VM.SubscribeToEventsF = func(_ context.Context) (common.Message, error) {
