...
func TestBlockState(t *testing.T) {
	...
	db := memdb.New()
	bs := NewBlockState(db, nil, false, nil)
...
func TestMeteredBlockState(t *testing.T) {
	...
	db := memdb.New()
	bs, err := NewMeteredBlockState(db, "", prometheus.NewRegistry(), nil, false, nil)
	a.NoError(err)
...
func TestBlockStateDedup(t *testing.T) {
	...
	db := memdb.New()
	bs, err := NewMeteredBlockState(db, "", prometheus.NewRegistry(), getInnerBytes, false, logging.NoLog{})
	require.NoError(err)
