...
func NewBlockState(db database.Database, getInnerBytes func(ids.ID) ([]byte, error), millisecondTimestamps bool, log logging.Logger) BlockState {
	return &blockState{
...
func NewMeteredBlockState(db database.Database, namespace string, metrics prometheus.Registerer, getInnerBytes func(ids.ID) ([]byte, error), millisecondTimestamps bool, log logging.Logger) (BlockState, error) {
...
func (s *blockState) parseDedupBlock(blkID ids.ID, storedBytes []byte) (*blockWrapper, error) {
	...
	blk, err := block.ParseWithoutVerification(fullBytes, s.millisecondTimestamps)
	if err != nil {
		return nil, err
	}
