...
func TestInnerBlockMappingPersistence(t *testing.T) {
	assert := assert.New(t)
	activationTime := genesisTimestamp.Add(10 * time.Second)
	minPChainHeight := uint64(0)
	innerVM, _, proVM, innerGenBlk, DBManager := initTestProposerVM(t, activationTime, minPChainHeight) // enable ProBlks
	...
	// build a different VM on the same DB.
