...
func TestCoreVMNotRemote(t *testing.T) {
	...
	maxBlkNum := 10
	maxBlkSize := 10
	maxBlksRetrivaltime := time.Duration(10)
	_, errAncestors := proVM.GetAncestors(blkID, maxBlkNum, maxBlkSize, maxBlksRetrivaltime)
...
func TestGetAncestorsPreForkOnly(t *testing.T) {
	assert := assert.New(t)
	coreVM, proRemoteVM, coreGenBlk := initTestRemoteProposerVM(t, mockable.MaxTime, 0) // disable ProBlks
...
func TestGetAncestorsPostForkOnly(t *testing.T) {
	assert := assert.New(t)
	coreVM, proRemoteVM, coreGenBlk := initTestRemoteProposerVM(t, time.Time{}, 0) // enable ProBlks
	...
	maxBlocksNum := 1000                          // an high value to get all built blocks
	maxBlocksSize := 1000000                      // an high value to get all built blocks
	maxBlocksRetrivalTime := time.Duration(10000) // an high value to get all built blocks
	res, err := proRemoteVM.GetAncestors(reqBlkID, maxBlocksNum, maxBlocksSize, maxBlocksRetrivalTime)
...
func TestGetAncestorsAtSnomanPlusPlusFork(t *testing.T) {
	...
	coreVM, proRemoteVM, coreGenBlk := initTestRemoteProposerVM(t, forkTime, 0)
