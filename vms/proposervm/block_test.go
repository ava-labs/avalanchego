...
func TestPostDurangoBuildChildResetScheduler(t *testing.T) {
	...
	chainVM.EXPECT().SubscribeToEvents(gomock.Any(), gomock.Any()).Return(common.PendingTxs, uint64(0)).AnyTimes()
	...
	vm.subscriber = common.NewSubscriptionDelayer(vm.ChainVM.SubscribeToEvents)
