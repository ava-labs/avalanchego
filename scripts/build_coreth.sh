...
if [[ $# -eq 2 ]]; then
    ...
    if [[ ! -d "$coreth_path" ]]; then
        go get "github.com/chain4travel/coreth@$coreth_version"
    fi
else
...
cd "$coreth_path"
go build -ldflags "-X github.com/chain4travel/coreth/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$AVALANCHE_PATH"
