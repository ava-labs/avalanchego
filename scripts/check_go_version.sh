...
# Check MODULE.bazel
MODULE_VERSION=$(grep -oP 'go_sdk\.download\(version = "\K[0-9]+\.[0-9]+(\.[0-9]+)?' "$REPO_ROOT/MODULE.bazel" 2>/dev/null || echo "")
...
# Check nix/go/default.nix
NIX_VERSION=$(grep -oP 'goVersion = "\K[0-9]+\.[0-9]+(\.[0-9]+)?' "$REPO_ROOT/nix/go/default.nix" 2>/dev/null || echo "")
