...
BINARY_PATH="${AVALANCHEGO_BUILD_PATH:-${AVALANCHE_PATH}/build}/l1"

echo "Building L1 VM plugin @ GitCommit: ${git_commit} at ${BINARY_PATH}"
# shellcheck disable=SC2086
go build -ldflags "-X github.com/ava-labs/avalanchego/version.GitCommit=${git_commit} ${static_ld_flags}" \
  -o "${BINARY_PATH}" ./vms/saevm/l1/plugin/
