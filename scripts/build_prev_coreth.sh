...
PREV_CORETH_VER="v0.4.0-rc.7"
BINARY_PATH="$PLUGIN_DIR/evm-$PREV_CORETH_VER"

CORETH_PATH="$GOPATH/pkg/mod/github.com/ava-labs/coreth@$PREV_CORETH_VER"
...
if [[ $# -eq 2 ]]; then
    ...
    if [[ ! -d "$CORETH_PATH" ]]; then
        go get "github.com/ava-labs/coreth@$PREV_CORETH_VER"
    fi
else
