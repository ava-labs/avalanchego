...
# We exclude the graft/coreth directory to avoid linting files
# that should be run from a different location within the repo,
# as there are false positives.
#
# shellcheck disable=SC2035
find * -path 'graft/coreth' -prune -o -name '*.sh' -type f -print0 | xargs -0 shellcheck "${@}"
