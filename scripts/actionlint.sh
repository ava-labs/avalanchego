...
echo "Checking third-party action refs are pinned and versioned..."
...
# full commit SHA and a version comment so upgrades are explicit and reviewable.
FLOATING_ACTION_REFS=
if FLOATING_ACTION_REFS=$(rg --hidden -n -P '^\s*(?:-\s*)?uses:\s+(?!\.?/)(?!actions/)[^@\s]+@(?!(?:[0-9a-fA-F]{40})\s+#\s+v\S+\s*$)' "${AVALANCHE_PATH}/.github" -g '*.yml' -g '*.yaml'); then
  :
else
  RG_EXIT_CODE=$?
...
if [[ -n "${FLOATING_ACTION_REFS}" ]]; then
  ...
  echo "Pin every other third-party action to a full commit SHA with a # v<version> comment."
  exit 1
fi
