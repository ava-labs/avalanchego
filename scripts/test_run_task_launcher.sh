...
mkdir -p "${caller_dir}"
run_case_in_dir fallback-to-go "${stub_dir}" "${caller_dir}" hello world
assert_called go "tool -modfile=${repo_root}/tools/external/go.mod task hello world"
assert_pwd "${caller_dir}"
if [[ "$(<"${workdir}/go-calls")" != "tool -modfile=${repo_root}/tools/external/go.mod task hello world" ]]; then
  echo "expected go fallback to run task via tools/external/go.mod" >&2
  exit 1
fi
if [[ "$(<"${workdir}/go-gowork")" != "off" ]]; then
  echo "expected go fallback to disable workspace mode" >&2
  exit 1
fi
