//! Asset Virtual Machine (X-Chain) implementation.
//!
//! The AVM manages fungible and non-fungible assets on Avalanche.
//! It uses Avalanche consensus (DAG-based) for transaction finality.
//!
//! # Architecture
//!
//! - **Assets**: Fungible tokens and NFTs with customizable properties
//! - **Transactions**: Create, mint, transfer, burn operations
//! - **UTXOs**: Unspent transaction output model for asset tracking
//!
//! # Example
//!
//! ```ignore
...
impl AVM {
    ...
    /// Returns the AVM state.
    pub fn state(&self) -> Option<&AVMState> {
        self.state.as_ref()
    }
}
...
impl CommonVM for AVM {
    async fn initialize(
        ...
        // Initialize state
        let state = AVMState::new(db.clone(), &genesis)?;
...
impl ChainVM for AVM {
    ...
    async fn parse_block(&self, bytes: &[u8]) -> Result<Box<dyn Block>> {
        let block = block::AVMBlock::parse(bytes)?;
        Ok(Box::new(block))
    }
