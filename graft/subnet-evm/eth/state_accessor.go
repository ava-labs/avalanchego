...
//     on disk. For the Firewood scheme, it signals that the state should be served from
//     a freshly reconstructed, non-persistent revision rather than the live trie, so
//     callers that re-execute on top (e.g. debug_intermediateRoots) get a mutable view.
