...
func TestSimpleSyncCases(t *testing.T) {
	...
	tests := map[string]syncTest{
		...
		"accounts with code": {
			prepareForTest: func(t *testing.T, r *rand.Rand) (state.Database, state.Database, common.Hash) {
				serverDB := state.NewDatabase(rawdb.NewMemoryDatabase())
				root, _ := synctest.FillAccounts(t, r, serverDB, common.Hash{}, numAccounts, func(t *testing.T, index int, _ common.Address, account types.StateAccount, tr state.Trie) types.StateAccount {
					if index%3 == 0 {
