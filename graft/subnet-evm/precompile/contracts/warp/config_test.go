...
func TestVerify(t *testing.T) {
	tests := map[string]precompiletest.ConfigVerifyTest{
		"quorum numerator less than minimum": {
			Config:        NewConfig(new(uint64(3)), WarpQuorumNumeratorMinimum-1, false),
			ExpectedError: ErrInvalidQuorumRatio,
		},
		"quorum numerator greater than quorum denominator": {
			Config:        NewConfig(new(uint64(3)), WarpQuorumDenominator+1, false),
			ExpectedError: ErrInvalidQuorumRatio,
		},
		"default quorum numerator": {
			Config: NewDefaultConfig(new(uint64(3))),
		},
		...
		"invalid cannot activated before Durango activation": {
			Config: NewConfig(new(uint64(3)), 0, false),
...
func TestEqualWarpConfig(t *testing.T) {
	tests := map[string]precompiletest.ConfigEqualTest{
		...
		"different timestamp": {
			Config:   NewDefaultConfig(new(uint64(3))),
			Other:    NewDefaultConfig(new(uint64(4))),
			Expected: false,
		},

		"different quorum numerator": {
			Config:   NewConfig(new(uint64(3)), WarpQuorumNumeratorMinimum+1, false),
			Other:    NewConfig(new(uint64(3)), WarpQuorumNumeratorMinimum+2, false),
			Expected: false,
		},

		"same default config": {
			Config:   NewDefaultConfig(new(uint64(3))),
			Other:    NewDefaultConfig(new(uint64(3))),
			Expected: true,
		},

		"same non-default config": {
			Config:   NewConfig(new(uint64(3)), WarpQuorumNumeratorMinimum+5, false),
			Other:    NewConfig(new(uint64(3)), WarpQuorumNumeratorMinimum+5, false),
			Expected: true,
		},
	}
