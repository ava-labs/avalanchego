...
func TestVerify(t *testing.T) {
	...
	tests := map[string]precompiletest.ConfigVerifyTest{
		"nil initialGasPriceConfig": {
			Config: gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil, nil),
		},
		"valid initialGasPriceConfig": {
			Config: gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil, new(commontype.DefaultGasPriceConfig())),
		},
		"invalid initialGasPriceConfig": {
			Config: gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil, &commontype.GasPriceConfig{
...
func TestEqual(t *testing.T) {
	...
	tests := map[string]precompiletest.ConfigEqualTest{
		"non-nil config and nil other": {
			Config:   gaspricemanager.NewConfig(new(uint64(3)), admins, enableds, nil, nil),
			Other:    nil,
			Expected: false,
		},
		...
		"different timestamp": {
			Config:   gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil, nil),
			Other:    gaspricemanager.NewConfig(new(uint64(4)), admins, nil, nil, nil),
			Expected: false,
		},
		"non-nil initial config and nil initial config": {
			Config:   gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil, new(commontype.DefaultGasPriceConfig())),
			Other:    gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil, nil),
			Expected: false,
		},
		"different initial config": {
			Config: gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil, new(commontype.DefaultGasPriceConfig())),
			Other: gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil,
		...
		"same config": {
			Config:   gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil, new(commontype.DefaultGasPriceConfig())),
			Other:    gaspricemanager.NewConfig(new(uint64(3)), admins, nil, nil, new(commontype.DefaultGasPriceConfig())),
			Expected: true,
		},
	}
