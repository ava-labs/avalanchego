...
func TestVerify(t *testing.T) {
	...
	tests := map[string]precompiletest.ConfigVerifyTest{
		"valid config": {
			Config: nativeminter.NewConfig(new(uint64(3)), admins, enableds, managers, nil),
		...
		"invalid allow list config in native minter allowlisttest": {
			Config:        nativeminter.NewConfig(new(uint64(3)), admins, admins, nil, nil),
			ExpectedError: allowlist.ErrAdminAndEnabledAddress,
		},
		"duplicate admins in config in native minter allowlisttest": {
			Config:        nativeminter.NewConfig(new(uint64(3)), append(admins, admins[0]), enableds, managers, nil),
			ExpectedError: allowlist.ErrDuplicateAdminAddress,
		},
		"duplicate enableds in config in native minter allowlisttest": {
			Config:        nativeminter.NewConfig(new(uint64(3)), admins, append(enableds, enableds[0]), managers, nil),
			ExpectedError: allowlist.ErrDuplicateEnabledAddress,
		},
		"nil amount in native minter config": {
			Config: nativeminter.NewConfig(new(uint64(3)), admins, nil, nil,
...
func TestEqual(t *testing.T) {
	...
	tests := map[string]precompiletest.ConfigEqualTest{
		...
		"different timestamp": {
			Config:   nativeminter.NewConfig(new(uint64(3)), admins, nil, nil, nil),
			Other:    nativeminter.NewConfig(new(uint64(4)), admins, nil, nil, nil),
			Expected: false,
		},
		"different initial mint amounts": {
			...
			Other: nativeminter.NewConfig(new(uint64(3)), admins, nil, nil,
				map[common.Address]*math.HexOrDecimal256{
