...
var bindTests = []struct {
	...
	},
	{
		...
		`"github.com/stretchr/testify/require"
		 ...
		 "github.com/MetalBlockchain/metalgo/graft/subnet-evm/core/extstate"
...
func TestPrecompileBind(t *testing.T) {
	...
	pwd, _ := os.Getwd()
	replacer := exec.Command(gocmd, "mod", "edit", "-x", "-require", "github.com/MetalBlockchain/metalgo/graft/subnet-evm@v0.0.0", "-replace", "github.com/MetalBlockchain/metalgo/graft/subnet-evm="+filepath.Join(pwd, "..", "..", "..", "..")) // Repo root
	replacer.Dir = pkg
	out, err = replacer.CombinedOutput()
	require.NoError(t, err, "failed to replace binding test dependency to current source tree: %v\n%s", err, out)

	replacer = exec.Command(gocmd, "mod", "edit", "-x", "-require", "github.com/MetalBlockchain/metalgo/graft/evm@v0.0.0", "-replace", "github.com/MetalBlockchain/metalgo/graft/evm="+filepath.Join(pwd, "..", "..", "..", "..", "..", "evm"))
	replacer.Dir = pkg
	out, err = replacer.CombinedOutput()
	require.NoError(t, err, "failed to replace binding test dependency to current source tree: %v\n%s", err, out)

	replacer = exec.Command(gocmd, "mod", "edit", "-x", "-require", "github.com/MetalBlockchain/metalgo@v0.0.0", "-replace", "github.com/MetalBlockchain/metalgo="+filepath.Join(pwd, "..", "..", "..", "..", "..", ".."))
	replacer.Dir = pkg
	out, err = replacer.CombinedOutput()
	require.NoError(t, err, "failed to replace binding test dependency to current source tree: %v\n%s", err, out)
