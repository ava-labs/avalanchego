...
func TestPrecompileBind(t *testing.T) {
	...
	replacer = exec.Command(gocmd, "mod", "edit", "-x", "-require", "github.com/ava-labs/avalanchego/graft/evm-shared@v0.0.0", "-replace", "github.com/ava-labs/avalanchego/graft/evm-shared="+filepath.Join(pwd, "..", "..", "..", "..", "..", "evm-shared")) // Repo root
	replacer.Dir = pkg
	out, err = replacer.CombinedOutput()
	require.NoError(t, err, "failed to replace binding test dependency to current source tree: %v\n%s", err, out)
