...
go test -shuffle=on ${race:-} -timeout="${TIMEOUT:-900s}" -coverprofile=coverage.out -covermode=atomic "$@" -run ^TestCanonicalHashMarker$ github.com/ava-labs/avalanchego/graft/subnet-evm/core -count 500