...
func TestVerifyWithChainConfig(t *testing.T) {
	...
	config.GenesisPrecompiles = Precompiles{
		txallowlist.ConfigKey: txallowlist.NewConfig(new(uint64(2)), nil, nil, nil),
	}
	config.PrecompileUpgrades = []PrecompileUpgrade{
		{
			// disable TxAllowList at timestamp 4
			txallowlist.NewDisableConfig(new(uint64(4))),
		},
		{
			// re-enable TxAllowList at timestamp 5
			txallowlist.NewConfig(new(uint64(5)), admins, nil, nil),
		},
	}
	...
	badConfig.PrecompileUpgrades = append(
		badConfig.PrecompileUpgrades,
		PrecompileUpgrade{
			Config: txallowlist.NewConfig(new(uint64(5)), admins, nil, nil),
		},
	)
	err = badConfig.Verify()
...
func TestVerifyPrecompileUpgrades(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			upgrades: []PrecompileUpgrade{
				{
					Config: txallowlist.NewConfig(new(uint64(1)), admins, nil, nil),
				},
				{
					Config: txallowlist.NewDisableConfig(new(uint64(2))),
				},
			},
			expectedError: nil,
		},
		{
			...
			upgrades: []PrecompileUpgrade{
				...
				{
					Config: txallowlist.NewConfig(new(uint64(3)), admins, admins, admins),
				},
			},
		...
		},
		{
			...
			upgrades: []PrecompileUpgrade{
				{
					Config: feemanager.NewConfig(new(uint64(3)), admins, nil, nil,
		...
		},
		{
			...
			upgrades: []PrecompileUpgrade{
				...
				{
					Config: feemanager.NewConfig(new(uint64(1)), admins, nil, nil, nil),
				},
			},
			expectedError: nil,
		},
...
func TestVerifyPrecompiles(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "invalid allow list config in tx allowlist",
			precompiles: Precompiles{
				txallowlist.ConfigKey: txallowlist.NewConfig(new(uint64(3)), admins, admins, admins),
			},
			expectedError: allowlist.ErrAdminAndEnabledAddress,
		},
		{
			name: "invalid initial fee manager config",
			precompiles: Precompiles{
				feemanager.ConfigKey: feemanager.NewConfig(new(uint64(3)), admins, nil, nil,
					func() *commontype.FeeConfig {
						feeConfig := DefaultFeeConfig
						feeConfig.GasLimit = big.NewInt(-1)
						return &feeConfig
...
func TestVerifyRequiresSortedTimestamps(t *testing.T) {
	...
	config.PrecompileUpgrades = []PrecompileUpgrade{
		...
		{
			Config: deployerallowlist.NewConfig(new(uint64(1)), admins, nil, nil),
		},
	}
...
func TestGetPrecompileConfig(t *testing.T) {
	...
	config := &ChainConfig{}
	config.GenesisPrecompiles = Precompiles{
		deployerallowlist.ConfigKey: deployerallowlist.NewConfig(new(uint64(10)), nil, nil, nil),
	}
...
func TestPrecompileUpgradeUnmarshalJSON(t *testing.T) {
	...
	expectedNativeMinterConfig := nativeminter.NewConfig(new(uint64(1671543172)), nil, nil, nil, nil)
