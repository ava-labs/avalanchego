...
// MinDelayExponent calculates the minimum delay exponent based on the parent, current
// header and the desired min delay exponent.
// If the `desiredMinDelayExponent` is nil, the parent's delay exponent is used.
func MinDelayExponent(
	...
	// If the header is in Granite, calculate the min delay exponent.
...
func VerifyMinDelayExponent(
	...
	// not be equal.
	expectedDelayExponent, err := minDelayExponent(
		config,
		parent,
		remoteDelayExponent,
	)
...
// minDelayExponent takes the parent header and the desired min delay exponent
// and returns the min delay exponent for the child block.
// If the desired min delay exponent is specified, moves the min delay exponent as much
...
func minDelayExponent(
	...
	if config.IsGranite(parent.Time) {
		...
		// resulting min delay exponent from the parent block.
