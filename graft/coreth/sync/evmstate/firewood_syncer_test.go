...
func testFirewoodSync(t *testing.T, clientState, serverState state.Database, root common.Hash) {
	...
	fetcher, err := code.NewQueue(clientState.DiskDB().(ethdb.Database), make(chan struct{}))
	require.NoError(t, err, "NewCodeQueue()")

	// Create the consumer code syncer.
	codeSyncer, err := code.NewSyncer(mockClient, clientState.DiskDB().(ethdb.Database), fetcher.CodeHashes())
