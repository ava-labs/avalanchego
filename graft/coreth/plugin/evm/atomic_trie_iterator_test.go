...
func TestIteratorCanIterate(t *testing.T) {
	...
	atomicTrie1, err := newAtomicTrie(db, nil, nil, repo, codec, lastAcceptedHeight, 100)
	assert.NoError(t, err)
	...
	// iterator
	atomicTrie2, err := newAtomicTrie(db, nil, nil, repo, codec, lastAcceptedHeight, 100)
	assert.NoError(t, err)
...
func TestIteratorHandlesInvalidData(t *testing.T) {
	...
	// create an atomic trie
	// on create it will initialize all the transactions from the above atomic repository
	atomicTrie, err := newAtomicTrie(db, nil, nil, repo, codec, lastAcceptedHeight, 100)
	assert.NoError(t, err)
