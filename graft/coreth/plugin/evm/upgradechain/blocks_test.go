...
// are contiguous from height 1, so the tip height is the block count.
func (g *generator) tip() uint64 {
	return uint64(len(g.fixture.Blocks))
}
...
// milliEther is the quantum of the fixture's transfers: each moves a distinct
// multiple of it.
const milliEther = params.Ether / 1000

// transferTx returns a legacy transfer of n milliether to a fixed recipient.
func (g *generator) transferTx(t *testing.T, n int64) *types.Transaction {
	t.Helper()
	return g.signedTx(t, &types.LegacyTx{
		Nonce:    g.nonce(),
		To:       &transferRecipient,
		Value:    big.NewInt(n * milliEther),
		Gas:      ethparams.TxGas,
