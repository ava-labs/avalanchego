...
func loadAndParseJournal(db ethdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	if !bytes.Equal(blockHash.Bytes(), base.blockHash.Bytes()) {
		log.Warn("Loaded snapshot journal block hash mismatch. Ignoring snapshot journal.", "diskBlockHash", base.blockHash, "diffs", "unmatched")
		return base, generator, nil
	}
	if !bytes.Equal(root.Bytes(), base.root.Bytes()) {
		log.Warn("Loaded snapshot journal block root mismatch. Ignoring snapshot journal.", "diskroot", base.root, "diffs", "unmatched")
		return base, generator, nil
	}
