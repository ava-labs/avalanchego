...
usage() {
  cat >&2 <<EOF
Usage: $0 <version> <source-path> <target-path>
...
Arguments:
  ...
  source-path  Root of the local Git repository to merge from.
  target-path  Destination relative to this repository's root.

Paths are relative to this repository's root unless absolute. The target path
must not already exist. Roll back a previous subtree merge intentionally before
