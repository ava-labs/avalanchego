...
// After this call, the internal Reconstructed is mutated in place,
...
func (a *reconstructedAccountTrie) Hash() common.Hash {
	...
	if err := a.reconstructed.Reconstruct(a.updateOps); err != nil {
		log.Error("Failed to reconstruct trie", "error", err)
		return common.Hash{}
	}

	a.reader = &reconstructedReader{reconstructed: a.reconstructed}
	a.root = common.Hash(a.reconstructed.Root())
	a.updateOps = nil
	a.hasChanges = false
	return a.root
}
