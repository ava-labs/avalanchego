...
func TestStateHistoryDisabledCreatesNoStore(t *testing.T) {
	dir := t.TempDir()
	db, err := New(DefaultConfig(dir))
	require.NoError(t, err)
	require.Nil(t, db.HistoryStore())
	require.NoError(t, db.Close())
	require.NoFileExists(t, filepath.Join(dir, statehistory.Directory))
}
