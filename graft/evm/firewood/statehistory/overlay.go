...
// eth_getProof or the trie-iterating debug RPCs, which fail loud.
//
...
type Overlay struct {
	...
	// destructs memoizes, per hashed address, the latest destruction block at
	// or before target (ok=false: never destroyed), saving one seek per slot
	// read. An Overlay serves one request on one goroutine.
...
func NewOverlay(store *Store, frontier state.Database, target uint64, targetRoot common.Hash) *Overlay {
	return &Overlay{
		...
		destructs:  make(map[common.Hash]destructAt),
	}
}
...
func (t *overlayTrie) GetStorage(addr common.Address, key []byte) ([]byte, error) {
	...
	d, ok := t.o.destructs[hashedAddr]
	if !ok {
		block, found, err := t.o.store.DestructAt(hashedAddr, t.o.target)
		if err != nil {
			return nil, err
		}
		d = destructAt{block: block, ok: found}
		t.o.destructs[hashedAddr] = d
	}
	enc, err := t.o.store.storageAt(hashedAddr, crypto.Keccak256Hash(key), t.o.target, d.block, d.ok)
	if err != nil || enc == nil {
		return nil, err
	}
