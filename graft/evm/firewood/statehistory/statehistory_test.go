...
func TestFlushRejectsMalformedOps(t *testing.T) {
	...
	require.ErrorIs(s.Flush(0, []Op{{Kind: OpDestruct, Key: storageOpKey(testHashedAddr, testHashedSlot)}}), errBadOp)
	require.ErrorIs(s.Flush(0, []Op{{Kind: OpKind(0xff), Key: testHashedAddr[:]}}), errBadOp)
}
...
func TestFlushContiguity(t *testing.T) {
	require := require.New(t)
	s := New(memdb.New())

	require.ErrorIs(s.Flush(1, nil), ErrNoBaseline, "first flush must be genesis")
	require.ErrorIs(s.VerifyBaseline(1), ErrNoBaseline, "empty store on a chain past genesis")
	require.NoError(s.VerifyBaseline(0))

	require.NoError(s.Flush(0, nil))
	require.NoError(s.Flush(1, nil))
	require.ErrorIs(s.Flush(3, nil), ErrHole, "flush past head+1")
	require.NoError(s.Flush(1, nil), "replaying a captured block is idempotent")
	require.NoError(s.Flush(2, nil))

	require.NoError(s.VerifyBaseline(2))
	require.ErrorIs(s.VerifyBaseline(3), ErrNoBaseline, "head behind the chain")

	_, ok, err := s.StateAt(nil, 3, common.Hash{})
	require.NoError(err)
	require.False(ok, "above head is left to the live state")
}
