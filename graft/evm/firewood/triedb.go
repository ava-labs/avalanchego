...
type TrieDBConfig struct {
	...
	// StateHistoryEnabled opens a flat state history store (a separate pebble
...
func New(config TrieDBConfig) (*TrieDB, error) {
	...
	if config.StateHistoryEnabled {
		historyDB, err := pebbledb.New(filepath.Join(config.DatabaseDir, statehistory.Directory), nil, logging.NoLog{}, nil)
		if err != nil {
