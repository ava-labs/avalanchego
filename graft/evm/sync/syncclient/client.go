...
type Client interface {
	...
	GetBlocks(ctx context.Context, blockHash common.Hash, height uint64, parents uint16) ([]*evmtypes.Block, error)
...
type client struct {
	networkClient    types.SyncedNetworkClient
	codec            codec.Manager
...
type EthBlockParser interface {
	ParseEthBlock(b []byte) (*evmtypes.Block, error)
}
...
func (c *client) Codec() codec.Manager {
	return c.codec
}
...
func (c *client) GetBlocks(ctx context.Context, hash common.Hash, height uint64, parents uint16) ([]*evmtypes.Block, error) {
	...
	return data.(evmtypes.Blocks), nil
}
...
func (c *client) parseBlocks(codec codec.Manager, req message.Request, data []byte) (interface{}, int, error) {
	...
	blocks := make(evmtypes.Blocks, len(response.Blocks))
