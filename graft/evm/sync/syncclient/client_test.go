...
import (
	...
	clientstats "github.com/ava-labs/avalanchego/graft/evm/sync/syncclient/stats"
...
func TestGetCode(t *testing.T) {
	tests := map[string]struct {
		...
		"unexpected code bytes": {
			...
			expectedErr: syncclient.ErrHashMismatch,
		},
		"too many code elements returned": {
			...
			expectedErr: syncclient.ErrInvalidCodeResponseLen,
		},
		...
		"code size is too large": {
			...
			expectedErr: syncclient.ErrMaxCodeSizeExceeded,
		},
	}
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			testNetClient := &syncclient.TestNetwork{}
			stateSyncClient := syncclient.NewClient(&syncclient.ClientConfig{
				NetworkClient:    testNetClient,
				Codec:            message.Codec,
				Stats:            clientstats.NewNoOpStats(),
				StateSyncNodeIDs: nil,
				BlockParser:      syncclient.NewTestBlockParser(),
			})
...
func TestGetBlocks(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"gibberish response": {
			...
			expectedErr: syncclient.ErrUnmarshalResponse,
		},
		...
		"no blocks": {
			...
			expectedErr: syncclient.ErrEmptyResponse,
		},
		"more than requested blocks": {
			...
			expectedErr: syncclient.ErrTooManyBlocks,
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
...
func TestGetLeafs(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"too many leaves in response": {
			...
			expectedErr: syncclient.ErrTooManyLeaves,
		},
		...
		"removed first key in response": {
			...
			expectedErr: syncclient.ErrInvalidRangeProof,
		},
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			client := syncclient.NewClient(&syncclient.ClientConfig{
				NetworkClient:    &syncclient.TestNetwork{},
				Codec:            message.Codec,
				Stats:            clientstats.NewNoOpStats(),
				StateSyncNodeIDs: nil,
				BlockParser:      syncclient.NewTestBlockParser(),
			...
			response, _, err := syncclient.ParseLeafsResponse(client.Codec(), test.request, responseBytes)
			require.ErrorIs(t, err, test.expectedErr)
			if test.expectedErr != nil {
				return
			}
