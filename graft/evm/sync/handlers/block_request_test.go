...
func executeBlockRequestTest(t testing.TB, test blockRequestTest, blocks []*types.Block) {
	...
	blockRequestHandler := handlers.NewBlockRequestHandler(blockProvider, message.Codec, testHandlerStats)
...
func TestBlockRequestHandlerCtxExpires(t *testing.T) {
	...
	blockRequestHandler := handlers.NewBlockRequestHandler(blockProvider, message.Codec, stats.NewNoopHandlerStats())
