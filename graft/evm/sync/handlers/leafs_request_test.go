...
func TestLeafsRequestHandler_OnLeafsRequest(t *testing.T) {
	...
	snapshotProvider := &handlers.TestSnapshotProvider{}
	leafsHandler := handlers.NewLeafsRequestHandler(trieDB, message.StateTrieKeyLength, snapshotProvider, message.Codec, testHandlerStats)
	...
	tests := map[string]struct {
		...
		"max leaves overridden": {
			prepareTestFn: func() (context.Context, message.LeafsRequest) {
				return t.Context(), message.LeafsRequest{
					...
					Start:    bytes.Repeat([]byte{0x00}, common.HashLength),
					End:      bytes.Repeat([]byte{0xff}, common.HashLength),
					Limit:    handlers.MaxLeavesLimit * 10,
					NodeType: message.StateTrieNode,
				}
			},
			requireResponseFn: func(t *testing.T, _ message.LeafsRequest, response []byte, err error) {
				...
				_, err = message.Codec.Unmarshal(response, &leafsResponse)
				require.NoError(t, err)
				require.Len(t, leafsResponse.Keys, int(handlers.MaxLeavesLimit))
				require.Len(t, leafsResponse.Vals, int(handlers.MaxLeavesLimit))
		...
		"account data served from snapshot": {
			prepareTestFn: func() (context.Context, message.LeafsRequest) {
				...
				require.NoError(t, err)
				snapshotProvider.Snapshot = newSnapshotTreeAdapter(snap)
				return t.Context(), message.LeafsRequest{
		...
		"partial account data served from snapshot": {
			...
			requireResponseFn: func(t *testing.T, request message.LeafsRequest, response []byte, err error) {
				...
				numSegments := handlers.MaxLeavesLimit / handlers.SegmentLen
...
// snapshotTreeAdapter adapts coreth's *snapshot.Tree to the handlers.SnapshotTree interface.
type snapshotTreeAdapter struct {
	tree *snapshot.Tree
}

func newSnapshotTreeAdapter(tree *snapshot.Tree) handlers.SnapshotTree {
	return &snapshotTreeAdapter{tree: tree}
}

func (s *snapshotTreeAdapter) DiskAccountIterator(seek common.Hash) snapshot.AccountIterator {
	return s.tree.DiskAccountIterator(seek)
}

func (s *snapshotTreeAdapter) DiskStorageIterator(account common.Hash, seek common.Hash) snapshot.StorageIterator {
