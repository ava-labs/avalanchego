...
func TestCodeRequestHandler(t *testing.T) {
	...
	tests := map[string]struct {
		setup       func() (request message.CodeRequest, wantCodeResponse [][]byte)
	...
	messagetest.ForEachCodec(t, func(c codec.Manager, _ message.LeafsRequestType) {
		...
		for name, test := range tests {
			...
			t.Run(name, func(t *testing.T) {
				request, wantResponse := test.setup()
				...
				require.NoError(t, err)
				require.Len(t, response.Data, len(wantResponse))
				for i, code := range wantResponse {
