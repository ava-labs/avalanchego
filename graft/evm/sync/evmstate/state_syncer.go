...
// SnapshotIterable abstracts access to snapshot iteration.
// This allows the state syncer to work with different snapshot implementations.
type SnapshotIterable interface {
	// AccountIterator creates an account iterator starting at the given seek position.
	AccountIterator(seek common.Hash) libevmsnapshot.AccountIterator

	// StorageIterator creates a storage iterator for an account starting at the given seek position.
	StorageIterator(account common.Hash, seek common.Hash) (libevmsnapshot.StorageIterator, bool)
}

// SnapshotFactory creates a SnapshotIterable for a given database.
type SnapshotFactory func(db ethdb.Database) SnapshotIterable
...
type stateSync struct {
	...
	snapshot  SnapshotIterable               // used to access the database we are syncing as a snapshot.
...
func NewSyncer(client syncclient.Client, db ethdb.Database, root common.Hash, snapshotFactory SnapshotFactory, codeQueue *code.Queue, leafsRequestSize uint16, opts ...SyncerOption) (types.Syncer, error) {
