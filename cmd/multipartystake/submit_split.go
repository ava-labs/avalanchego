...
func submitSplitCmd() *cobra.Command {
	var (
		txFilePath  string
		sigFilesStr string
	)
	...
	cmd := &cobra.Command{
		Use:   "submit-split",
		Short: "Assemble signatures and submit the reward-split BaseTx",
		RunE: func(_ *cobra.Command, _ []string) error {
			sigFiles := strings.Split(sigFilesStr, ",")
			return runSubmitSplit(txFilePath, sigFiles)
		},
	}
	cmd.Flags().StringVar(&txFilePath, "tx-file", "", "Path to the prepare-split output JSON")
...
func runSubmitSplit(txFilePath string, sigFiles []string) error {
	...
	if err := json.Unmarshal(prepareBytes, &splitOutput); err != nil {
	...
	if !ok {
		return fmt.Errorf("expected BaseTx, got %T", utx)
	}
	...
	fmt.Printf("Submitted split tx %s\n", txID)
	return nil
}
