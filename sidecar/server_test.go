...
func validMsgBytes(t *testing.T) []byte {
	t.Helper()
	msg, err := oracle.NewOracleMessage("solana", "So11111111111111111111111111111111111111112", common.Address{1, 2, 3}, 100, 1, []byte("payload"))
...
func TestServer_Verify(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			verifier: &mockVerifier{err: fmt.Errorf("%w: rpc failed", oracle.ErrSourceUnavailable)},
			wantCode: codes.Unavailable,
		},
		{
			name:     "verifier rejection returns InvalidArgument",
			req:      &pb.VerifyRequest{MessageBytes: validMsgBytes(t), Justification: make([]byte, 64)},
			verifier: &mockVerifier{err: errors.New("slot mismatch: got 99, want 100")},
			wantCode: codes.InvalidArgument,
		},
		{
...
func TestServer_PayloadRoundtrip(t *testing.T) {
	want, err := oracle.NewOracleMessage("solana", "So11111111111111111111111111111111111111112", common.Address{}, 100, 1, []byte("payload"))
	require.NoError(t, err)

	srv := NewServer(&mockVerifier{})
