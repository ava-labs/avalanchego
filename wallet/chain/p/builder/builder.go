...
func (b *builder) spend(
	...
	result, err := Spend(b.backend, b.context, b.addrs, toBurn, toStake, excessAVAX, complexity, ownerOverride, options)
	if err != nil {
		return nil, nil, nil, err
	}
	return result.Inputs, result.ChangeOutputs, result.StakeOutputs, nil
}
...
func (b *builder) initCtx(tx txs.UnsignedTx) error {
	return initCtx(b.context, tx)
}

func initCtx(context *Context, tx txs.UnsignedTx) error {
	ctx, err := NewSnowContext(context.NetworkID, context.AVAXAssetID)
	if err != nil {
		return err
	}
