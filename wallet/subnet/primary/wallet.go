...
func MakeWallet(ctx context.Context, config *WalletConfig) (Wallet, error) {
	...
	pBuilder := pbackends.NewBuilder(avaxAddrs, pBackend)
	pSigner := pbackends.NewSigner(config.AVAXKeychain, pBackend)
	...
	xBuilder := xbackends.NewBuilder(avaxAddrs, xBackend)
	xSigner := xbackends.NewSigner(config.AVAXKeychain, xBackend)
