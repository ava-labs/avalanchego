...
func (a *App) Start() int {
	...
	case rocksdb.Name:
		...
		dbManager, err = manager.NewRocksDB(path, a.config.DatabaseConfig.Config, a.log, version.CurrentDatabase)
	case leveldb.Name:
		dbManager, err = manager.NewLevelDB(a.config.DatabaseConfig.Path, a.config.DatabaseConfig.Config, a.log, version.CurrentDatabase)
	case memdb.Name:
