...
func (a *app) Start() error {
	...
	if a.config.AttemptedNATTraversal && !a.config.Nat.SupportsNAT() {
		a.log.Warn("UPnP and NAT-PMP router attach failed, you may not be listening publicly. " +
	...
	go func() {
		...
		defer func() {
			...
			// If [p.node.Start()] panics, then we should log the panic and
