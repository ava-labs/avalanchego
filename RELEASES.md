...
- The C-Chain wallet (`wallet/chain/c`) and `client.AwaitTxAccepted` now wait for atomic txs with `avax.getAtomicTx` instead of the deprecated `avax.getAtomicTxStatus`, which public API endpoints no longer serve.
