...
func main() {
	...
	mapper.Map("TCP", Config.InternalStakingPort, Config.ExternalStakingIP.Port, stakingPortName)
	...
	if Config.HTTPHost != "127.0.0.1" && Config.HTTPHost != "localhost" {
		...
		mapper.Map("TCP", Config.InternalHTTPPort, Config.ExternalHTTPPort, httpPortName)
	}
