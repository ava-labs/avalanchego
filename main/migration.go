...
func (m *migrationManager) runMigration() error {
	prevVersionNode, err := m.binaryManager.runPreviousVersion(previousVersion, m.viperConfig)
	if err != nil {
		return fmt.Errorf("couldn't start old version during migration: %w", err)
	}
	defer func() {
		if err := m.binaryManager.kill(prevVersionNode.nodeID); err != nil {
	...
	defer func() {
		if err := m.binaryManager.kill(currentVersionNode.nodeID); err != nil {
