...
func Test_BigBitSet_Lsh(t *testing.T) {
	tests := []struct {
		name     string
		bits     []int
		n        uint
		...
		},
		{
			...
			bits:     []int{31, 15, 0},
			n:        10,
			expected: NewBigBitSet(31+10, 15+10, 0+10),
		},
		{
			name:     "multiple words",
			bits:     []int{255, 127, 63, 31, 0},
			n:        256,
			expected: NewBigBitSet(255+256, 127+256, 63+256, 31+256, 256),
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			r.EqualValues(test.expected, b)
		})
	}
}

func Test_BigBitSet_Rsh(t *testing.T) {
	tests := []struct {
		name     string
		bits     []int
		n        uint
		...
		},
		{
			...
			bits:     []int{63, 31, 0},
			n:        5,
			expected: NewBigBitSet(63-5, 31-5),
		},
		{
			name:     "multiple words",
			bits:     []int{255, 127, 63, 31, 0},
			n:        32,
			expected: NewBigBitSet(255-32, 127-32, 63-32),
		},
	}
