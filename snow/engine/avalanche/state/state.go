...
func (s *state) Status(id ids.ID) choice.Status {
	if statusIntf, found := s.dbCache.Get(id); found {
		status, _ := statusIntf.(choice.Status)
		return status
	}
	...
	s.dbCache.Put(id, choice.Unknown)
	return choice.Unknown
}

// SetStatus sets the status of the vertex and returns an error if it fails to write to the db
func (s *state) SetStatus(id ids.ID, status choice.Status) error {
	s.dbCache.Put(id, status)

	if status == choice.Unknown {
		return s.db.Delete(id[:])
	}
