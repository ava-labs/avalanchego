...
func TestEngineQuery(t *testing.T) {
	...
	sender.SendPushQueryF = func(_ context.Context, inVdrs ids.NodeIDSet, requestID uint32, vtxID ids.ID, vtx []byte) {
		if *queried {
			t.Fatalf("Asked multiple times")
		}
		*queried = true
...
func TestEngineScheduleRepoll(t *testing.T) {
	...
	sender.SendPushQueryF = func(_ context.Context, _ ids.NodeIDSet, reqID uint32, _ ids.ID, _ []byte) {
		*requestID = reqID
	}
...
func TestEngineGetVertex(t *testing.T) {
	...
	sender.SendPutF = func(_ context.Context, v ids.NodeID, _ uint32, vtxID ids.ID, vtx []byte) {
		if v != vdr.ID() {
	...
	if err := te.Get(context.Background(), vdr.ID(), 0, mVtx.ID()); err != nil {
		t.Fatal(err)
	}
}
...
func TestEngineInsufficientValidators(t *testing.T) {
	...
	queried := new(bool)
	sender.SendPushQueryF = func(_ context.Context, inVdrs ids.NodeIDSet, _ uint32, vtxID ids.ID, vtx []byte) {
		*queried = true
	}
...
func TestEngineBootstrappingIntoConsensus(t *testing.T) {
	...
	*requested = false
	...
	sender.SendPushQueryF = func(_ context.Context, vdrs ids.NodeIDSet, _ uint32, vtxID ids.ID, vtx []byte) {
		if vdrs.Len() != 1 {
			t.Fatalf("Should have requested from the validators")
		}
		if !vdrs.Contains(vdr) {
...
func TestEngineUndeclaredDependencyDeadlock(t *testing.T) {
	...
	sender.SendPushQueryF = func(context.Context, ids.NodeIDSet, uint32, ids.ID, []byte) {
...
func TestEnginePartiallyValidVertex(t *testing.T) {
	...
	sender.SendPushQueryF = func(_ context.Context, _ ids.NodeIDSet, _ uint32, vtxID ids.ID, _ []byte) {
...
func TestEngineGossip(t *testing.T) {
	...
	called := new(bool)
	sender.SendGossipF = func(_ context.Context, vtxID ids.ID, vtxBytes []byte) {
		*called = true
		if vtxID != gVtx.ID() {
			t.Fatal(errUnknownVertex)
		}
...
func TestEngineAggressivePolling(t *testing.T) {
	...
	sender.SendPushQueryF = func(context.Context, ids.NodeIDSet, uint32, ids.ID, []byte) { *numPushQueries++ }
...
func TestEngineDoubleChit(t *testing.T) {
	...
	reqID := new(uint32)
	sender.SendPushQueryF = func(_ context.Context, inVdrs ids.NodeIDSet, requestID uint32, vtxID ids.ID, _ []byte) {
		*reqID = requestID
		if inVdrs.Len() != 2 {
...
func TestEngineIssue(t *testing.T) {
	...
	sender.SendPushQueryF = func(_ context.Context, inVdrs ids.NodeIDSet, requestID uint32, vID ids.ID, vtx []byte) {
		vtxID = vID
		queryRequestID = requestID
	}
...
func TestHandleChitsV2(t *testing.T) {
	...
	require.NoError(te.ChitsV2(context.Background(), ids.GenerateTestNodeID(), 0, []ids.ID{ids.GenerateTestID()}, ids.Empty))
	require.True(*asked)
}
