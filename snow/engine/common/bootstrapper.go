...
func (b *bootstrapper) Accepted(ctx context.Context, nodeID ids.NodeID, requestID uint32, containerIDs []ids.ID) error {
	...
	if size == 0 && b.Beacons.Len() > 0 {
		...
		// bootstrap hoping for the network problems to go away; otherwise, we received
		// enough (>= b.Alpha) responses, but no frontier was supported by a majority
		// of validators (i.e. votes are split between minorities supporting different frontiers).
