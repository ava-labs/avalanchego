...
func (s *startup) OnValidatorAdded(nodeID ids.GenericNodeID, pk *bls.PublicKey, txID ids.ID, weight uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *startup) OnValidatorWeightChanged(nodeID ids.GenericNodeID, oldWeight, newWeight uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *startup) Connected(ctx context.Context, nodeID ids.GenericNodeID, nodeVersion *version.Application) error {
	s.lock.Lock()
	defer s.lock.Unlock()
