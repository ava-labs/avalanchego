...
func TestPushAndExecute(t *testing.T) {
	...
	count, err := jobs.ExecuteAll(context.Background(), snowtest.ConsensusContext(snowtest.Context(t, snowtest.CChainID)), &common.Halter{}, false)
	require.NoError(err)
...
func TestHandleJobWithMissingDependencyOnRunnableStack(t *testing.T) {
	...
	_, err = jobs.ExecuteAll(context.Background(), snowtest.ConsensusContext(snowtest.Context(t, snowtest.CChainID)), &common.Halter{}, false)
