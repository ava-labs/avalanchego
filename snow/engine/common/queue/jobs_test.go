...
func TestPushAndExecute(t *testing.T) {
	...
	count, err := jobs.ExecuteAll(context.Background(), snowtest.EmptyConsensusContext(), &common.Halter{}, false)
	require.NoError(err)
...
func TestHandleJobWithMissingDependencyOnRunnableStack(t *testing.T) {
	...
	_, err = jobs.ExecuteAll(context.Background(), snowtest.EmptyConsensusContext(), &common.Halter{}, false)
