...
func (nf *NotificationForwarder) run() {
	nf.Log.Info("Starting notification forwarder", zap.Bool("BootstrappingOrSyncing", nf.BootstrappingOrSyncing))
	for {
		select {
		case <-nf.closeChan:
			return
		...
		nf.Log.Info("Subscribing to notifications")
		...
		if performingConsensus && preferenceChanged {
			nf.Log.Info("Preference changed during subscription",
