...
// ChitsV2 provides a mock function with given fields: ctx, validatorID, requestID, containerIDs, containerID
func (_m *Engine) ChitsV2(ctx context.Context, validatorID ids.NodeID, requestID uint32, containerIDs []ids.ID, containerID ids.ID) error {
	ret := _m.Called(ctx, validatorID, requestID, containerIDs, containerID)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, ids.NodeID, uint32, []ids.ID, ids.ID) error); ok {
		r0 = rf(ctx, validatorID, requestID, containerIDs, containerID)
	} else {
		r0 = ret.Error(0)
	}
