...
type metrics struct {
	...
	numGetProcessingAncestorResults *prometheus.CounterVec
...
func newMetrics(namespace string, reg prometheus.Registerer) (*metrics, error) {
	...
	m := &metrics{
		...
		numGetProcessingAncestorResults: prometheus.NewCounterVec(prometheus.CounterOpts{
			Namespace: namespace,
			Name:      "num_get_processing_ancestor",
			Help:      "Number of votes that were attempted to be bubbled to a processing ancestor",
		}, []string{"result"}),
	...
	// Register the labels
	m.numGetProcessingAncestorResults.WithLabelValues(ancestorResult)
	m.numGetProcessingAncestorResults.WithLabelValues(selfResult)
	m.numGetProcessingAncestorResults.WithLabelValues(missingResult)
	m.numGetProcessingAncestorResults.WithLabelValues(decidedResult)
	...
	errs.Add(
		...
		reg.Register(m.numGetProcessingAncestorResults),
