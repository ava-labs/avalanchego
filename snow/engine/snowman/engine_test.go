...
// runForkNearAcceptFrontierConvergence drives a liveness regression test for the
// "only query the next preferred height" behavior, for a fork of length forkLen.
// If blockSize > 0, every block is inflated past that many bytes so it overflows
// the unverifiedBlockCache (proving convergence doesn't depend on that cache).
//
...
func runForkNearAcceptFrontierConvergence(t *testing.T, forkLen int, blockSize int) {
	require := require.New(t)
	...
	// When blocks are inflated past the cache size, SizedCache.put drops each one
	// (it flushes and returns without storing), so the cache is permanently empty
	// and cannot hold a single block on the bubble path.
	if blockSize > 0 {
...
func TestEngineForkNearAcceptFrontierAdvancesAcceptFrontier(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Every block is inflated past nonVerifiedCacheSize (64 MiB), so it
			// overflows the unverifiedBlockCache and the cache stays permanently
			// empty. This proves vote bubbling does not depend on the byte-bounded
			// cache: getProcessingAncestor bubbles votes through the unbounded
			// unverifiedIDToAncestor tree, which jumps straight from the high vote
			// target to the deepest processing ancestor regardless of what the
			// cache holds.
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			runForkNearAcceptFrontierConvergence(t, forkLen, test.blockSize)
		})
	}
}
