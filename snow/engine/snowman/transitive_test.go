...
func TestGetProcessingAncestor(t *testing.T) {
	var (
		...
		unissuedChain         = BuildChain(Genesis, 1)
	)
	...
	tests := []struct {
		...
		{
			name: "do not read block if it will be dropped",
			engine: &Transitive{
				Config: Config{
					VM: &block.TestVM{
						TestVM: common.TestVM{
				...
				nonVerifieds: ancestor.NewTree(),
				pending: map[ids.ID]snowman.Block{
					unissuedChain[0].ID(): unissuedChain[0],
				},
			},
			initialVote:      unissuedChain[0].ID(),
			expectedAncestor: ids.Empty,
			expectedFound:    false,
			expectedMetrics: map[string]float64{
				ancestorResult: 0,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			test.engine.Ctx = ctx
			test.engine.Consensus = c
			test.engine.metrics = metrics
			test.engine.nonVerifiedCache = &cache.Empty[ids.ID, snowman.Block]{}
			...
			for label, expectedValue := range test.expectedMetrics {
				metric := metrics.numGetProcessingAncestorResults.WithLabelValues(label)
				require.Equal(expectedValue, testutil.ToFloat64(metric), label)
			}
		})
	}
}
