...
func (*Context) CanotoSpec(types ...reflect.Type) *canoto.Spec {
	types = append(types, reflect.TypeOf(Context{}))
	var zero Context
	s := &canoto.Spec{
		Name: "Context",
...
func (c *Context) CalculateCanotoCache() {
	...
	if !canoto.IsZero(c.PChainHeight) {
		size += len(canoto__Context__PChainHeight__tag) + canoto.SizeUint(c.PChainHeight)
	}
