...
const (
	canotoNumber_Context__PChainHeight = 1

	canotoTag_Context__PChainHeight = "\x08" // canoto.Tag(canotoNumber_Context__PChainHeight, canoto.Varint)
)
...
func (*Context) CanotoSpec(...reflect.Type) *canoto.Spec {
	...
	s := &canoto.Spec{
		...
		Fields: []canoto.FieldType{
			{
				FieldNumber: canotoNumber_Context__PChainHeight,
				Name:        "PChainHeight",
				OneOf:       "",
...
func (c *Context) UnmarshalCanotoFrom(r canoto.Reader) error {
	...
	*c = Context{}
	...
	for canoto.HasNext(&r) {
		...
		switch field {
		case canotoNumber_Context__PChainHeight:
...
func (c *Context) CalculateCanotoCache() {
	var size uint64
	if !canoto.IsZero(c.PChainHeight) {
		size += uint64(len(canotoTag_Context__PChainHeight)) + canoto.SizeUint(c.PChainHeight)
	}
...
func (c *Context) MarshalCanotoInto(w canoto.Writer) canoto.Writer {
	if !canoto.IsZero(c.PChainHeight) {
		canoto.Append(&w, canotoTag_Context__PChainHeight)
