...
func TestDualAlphaOptimization(t *testing.T) {
	...
	singleAlphaNetwork := NewNetwork(FactoryInstance, params, numColors, source)

	params.AlphaPreference = params.K/2 + 1
	dualAlphaNetwork := NewNetwork(FactoryInstance, params, numColors, source)
...
func TestTreeConvergenceOptimization(t *testing.T) {
	...
	treeNetwork := NewNetwork(FactoryInstance, params, numColors, source)
	flatNetwork := NewNetwork(FactoryInstance, params, numColors, source)
