...
func TestPeerAppRequestFailed(t *testing.T) {
	require := require.New(t)

	self := ids.GenerateTestNodeID()
	router := ChainRouter{}
	require.NoError(router.Initialize(
		self,
		logging.NoLog{},
		&timeoutDisabled{},
		0,
	...
	testHandler := &testHandler{
		ctx:      snowCtx,
		messages: make([]handler.Message, 0),
	}

	router.AddChain(context.Background(), testHandler)
	require.Len(testHandler.messages, 1)

	want := handler.Message{
		InboundMessage: message.InternalConnected(self, version.CurrentApp),
		EngineType:     0,
	}

	require.Equal(want, testHandler.messages[0])
	...
	msg := message.InboundAppRequestFailed(
		nodeID,
		snowCtx.ChainID,
		requestID,
		errorCode,
	...
	require.Len(testHandler.messages, 2)
	...
	require.Equal(want, testHandler.messages[1])
}

type timeoutDisabled struct{}

func (timeoutDisabled) Dispatch() {}

func (timeoutDisabled) TimeoutDuration() time.Duration {
	return 0
}

func (timeoutDisabled) IsBenched(ids.NodeID, ids.ID) bool {
	return false
}

func (timeoutDisabled) RegisterChain(*snow.ConsensusContext) error {
	return nil
}

func (timeoutDisabled) RegisterRequest(ids.NodeID, ids.ID, bool, ids.RequestID, func()) {}

func (timeoutDisabled) RegisterRequestToUnreachableValidator() {}

func (timeoutDisabled) RegisterResponse(ids.NodeID, ids.ID, ids.RequestID, message.Op, time.Duration) {
}

func (timeoutDisabled) RemoveRequest(ids.RequestID) {}

func (t timeoutDisabled) Stop() {}
...
func (t *testHandler) RegisterTimeout(time.Duration) {}

func (*testHandler) HealthCheck(context.Context) (interface{}, error) {
	return nil, nil
}

func (t *testHandler) Context() *snow.ConsensusContext {
	return t.ctx
}

func (t *testHandler) ShouldHandle(ids.NodeID) bool {
	return true
}

func (t *testHandler) SetEngineManager(*handler.EngineManager) {}

func (t *testHandler) GetEngineManager() *handler.EngineManager {
	return nil
}

func (t *testHandler) SetOnStopped(func()) {}

func (*testHandler) Start(context.Context, bool) {}

func (t *testHandler) Push(_ context.Context, msg handler.Message) {
	t.messages = append(t.messages, msg)
}

func (t *testHandler) Len() int {
	return len(t.messages)
}

func (*testHandler) Stop(context.Context) {}

func (*testHandler) StopWithError(context.Context, error) {}

func (*testHandler) AwaitStopped(context.Context) (time.Duration, error) {
	return 0, nil
}
