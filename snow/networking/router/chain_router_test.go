...
func TestRouterCrossChainMessages(t *testing.T) {
	...
	mc := message.NewInternalBuilder()
	chainRouter := ChainRouter{}
	require.NoError(t, chainRouter.Initialize(nodeID, logging.NoLog{}, mc, tm, time.Millisecond, set.Set[ids.ID]{}, set.Set[ids.ID]{}, nil, HealthConfig{}, "", prometheus.NewRegistry()))
