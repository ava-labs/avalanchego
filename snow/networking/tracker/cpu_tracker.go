...
type TimeTracker interface {
	// Registers that the given node started using a CPU at the given time.
	IncCPU(ids.NodeID, time.Time)
	// Registers that the given node stopped using a CPU at the given time.
	DecCPU(ids.NodeID, time.Time)
