...
type ExternalSenderTest struct {
	...
	GossipF func(subnetID, chainID, containerID ids.ID, container []byte, validatorOnly bool)
}
...
// function shouldn't be called and testing was initialized, then testing will
// fail.
func (s *ExternalSenderTest) Gossip(subnetID, chainID ids.ID, containerID ids.ID, container []byte, validatorOnly bool) {
	switch {
	case s.GossipF != nil:
		s.GossipF(subnetID, chainID, containerID, container, validatorOnly)
