...
func (h *handler) dispatchChans(ctx context.Context) {
	...
	thisNodeID := ids.GenericNodeIDFromNodeID(h.ctx.NodeID)
	for {
		...
		case vmMSG := <-h.msgFromVMChan:
			msg = message.InternalVMMessage(thisNodeID, uint32(vmMSG))

		case <-gossiper.C:
			msg = message.InternalGossipRequest(thisNodeID)

		case <-h.timeouts:
			msg = message.InternalTimeout(thisNodeID)
		}
