...
func main() {
	...
	hardCodedGenesisBytes, _, err := genesis.FromConfig(&genesis.ManhattanConfig)
	if err != nil {
		log.Fatal(err)
	}
...
func makeAvaxGenesis() *genesis.UnparsedConfig {
...

	{
		...
		for scanner.Scan() {
			line := scanner.Text()
			c.InitialStakedFunds = append(c.InitialStakedFunds, line)
		}
		if err := scanner.Err(); err != nil {
			log.Fatal(err)
		}
	...
	initialStakeNodeIDs := []string(nil)
	{
		...
		for scanner.Scan() {
			line := scanner.Text()
			initialStakeNodeIDs = append(initialStakeNodeIDs, line)
		}
		if err := scanner.Err(); err != nil {
			log.Fatal(err)
		}
	...
	initialStakeRewardAddresses := []string(nil)
	{
		...
		for scanner.Scan() {
			line := scanner.Text()
			initialStakeRewardAddresses = append(initialStakeRewardAddresses, line)
		}
		if err := scanner.Err(); err != nil {
			log.Fatal(err)
		}
	}
	for i, nodeID := range initialStakeNodeIDs {
		rewardAddress := initialStakeRewardAddresses[i]
		c.InitialStakers = append(c.InitialStakers, genesis.UnparsedStaker{
			NodeID:        nodeID,
