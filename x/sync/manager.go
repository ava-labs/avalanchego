...
func (m *Manager) handleRangeProofResponse(
	...
	largestHandledKey, err := m.config.DB.CommitRangeProof(ctx, work.start, work.end, &rangeProof)
	if err != nil {
		m.setError(err)
		return nil
	}
...
func (m *Manager) handleChangeProofResponse(
	...
	case *pb.SyncGetChangeProofResponse_ChangeProof:
		...
		largestHandledKey, err := m.config.DB.CommitChangeProof(ctx, work.end, &changeProof)
		if err != nil {
			m.setError(err)
			return nil
		}
