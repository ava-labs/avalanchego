...
func findNextKey(
	...
	if len(keyChanges) > 0 {
		...
		if compareMaybeBytes(maybe.Some(lastReceivedKey), rangeEnd) == 1 {
			return maybe.Nothing[[]byte](), nil
		}
	}
...
func compareMaybeBytes(a, b maybe.Maybe[[]byte]) int {
...
func (m *Manager) completeWorkItem(work *workItem, nextKey maybe.Maybe[[]byte], rootID ids.ID) {
	if compareMaybeBytes(work.end, nextKey) == 1 && !nextKey.IsNothing() {
