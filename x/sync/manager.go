...
var _ DBClient = (*merkleDBSyncClient)(nil)
...
func (c *merkleDBSyncClient) setError(err error) {
	c.errorOnce.Do(func() {
		c.err.Set(err)
		_ = c.db.Clear() // only report the original error
	})
}

// FinalizeSync implements DBClient.
func (c *merkleDBSyncClient) FinalizeSync(ctx context.Context, root ids.ID) error {
	// If the root is empty, we can clear the database and finish immediately.
	if root == ids.Empty {
	...
	localRoot, err := c.db.GetMerkleRoot(ctx)
	if err != nil {
		c.setError(err)
		return fmt.Errorf("failed to get local root: %w", err)
	}
	if localRoot != root {
		return fmt.Errorf("%w: expected %s, got %s", ErrFinishedWithUnexpectedRoot, root, localRoot)
	}
	return nil
}
...
func NewManager(config ManagerConfig, registerer prometheus.Registerer) (*Manager, error) {
	switch {
	case config.DBClient == nil:
		return nil, ErrNoDBClientProvided
...
func (m *Manager) handleRangeProofResponse(
	...
	nextKey, err := m.config.DBClient.HandleRangeProofResponse(ctx, request, responseBytes)
	if err != nil {
		return err
	}
...
func (m *Manager) handleChangeProofResponse(
	...
	nextKey, err := m.config.DBClient.HandleChangeProofResponse(ctx, request, responseBytes)
	if err != nil {
		return err
	}
	m.completeWorkItem(work, nextKey, targetRootID)
