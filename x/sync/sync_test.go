...
func (client *mockClient) GetChangeProof(ctx context.Context, request *merkledbpb.ChangeProofRequest, _ SyncableDB) (*merkledb.ChangeProof, error) {
...
func Test_Sync_Error_During_Sync(t *testing.T) {
	...
	client.EXPECT().GetChangeProof(gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
		func(ctx context.Context, request *merkledbpb.ChangeProofRequest, _ SyncableDB) (*merkledb.ChangeProof, error) {
			startRoot, err := ids.ToID(request.StartRootHash)
			require.NoError(err)
			endRoot, err := ids.ToID(request.EndRootHash)
			require.NoError(err)
