...
func TestTrieViewGetPathTo(t *testing.T) {
	...
	nodePath, err := trie.getPathTo(NewPath(nil, BranchFactor16))
	require.NoError(err)
...
func Test_Trie_WriteToDB(t *testing.T) {
	...
	node, err := parseNode(NewPath(key, BranchFactor16), rawBytes)
	require.NoError(err)
...
func Test_Trie_HashCountOnBranch(t *testing.T) {
	...
	_, err = view2.getNode(NewPath(keyPrefix, BranchFactor16), false, false)
	require.NoError(err)
...
func Test_Trie_ChainDeletion(t *testing.T) {
	...
	root, err := newTrie.getNode(emptyPath(BranchFactor16), false, false)
	require.NoError(err)
...
func Test_Trie_NodeCollapse(t *testing.T) {
	...
	root, err := trie.getNode(emptyPath(BranchFactor16), false, false)
	require.NoError(err)
	require.Len(root.children, 1)
	...
	firstNode, err := trie.getNode(getSingleChildPath(root), true, false)
	require.NoError(err)
