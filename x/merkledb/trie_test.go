...
func getNodeValue(t ReadOnlyTrie, key string) ([]byte, error) {
	...
	if asDatabases, ok := t.(*merkleDB); ok {
		view, err := asDatabases.NewViewFromMap(context.Background(), nil, false)
		if err != nil {
			return nil, err
		}
		path := newPath([]byte(key))
...
func Test_GetValue_Safety(t *testing.T) {
	...
	trieView, err := db.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte{0}, Value: []byte{0}}}, true)
	require.NoError(err)
...
func TestTrieViewGetPathTo(t *testing.T) {
	...
	trieIntf, err := db.NewViewFromBatchOps(context.Background(), nil, true)
	require.NoError(err)
	...
	// Insert a key
	key1 := []byte{0}
	trieIntf, err = trie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: key1, Value: []byte("value")}}, true)
	require.NoError(err)
	require.IsType(&trieView{}, trieIntf)
	trie = trieIntf.(*trieView)
	...
	key2 := []byte{0, 1}
	trieIntf, err = trie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: key2, Value: []byte("value")}}, true)
	require.NoError(err)
	require.IsType(&trieView{}, trieIntf)
	trie = trieIntf.(*trieView)
	require.NoError(trie.calculateNodeIDs(context.Background()))
	...
	key3 := []byte{255}
	trieIntf, err = trie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: key3, Value: []byte("value")}}, true)
	require.NoError(err)
	require.IsType(&trieView{}, trieIntf)
	trie = trieIntf.(*trieView)
	require.NoError(trie.calculateNodeIDs(context.Background()))
...
func Test_Trie_ViewOnCommitedView(t *testing.T) {
	...
	committedTrie, err := dbTrie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte{0}, Value: []byte{0}}}, true)
	require.NoError(err)

	require.NoError(committedTrie.CommitToDB(context.Background()))

	newView, err := committedTrie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte{1}, Value: []byte{1}}}, true)
	require.NoError(err)
...
func Test_Trie_WriteToDB(t *testing.T) {
	...
	trieIntf1, err := dbTrie.NewViewFromBatchOps(context.Background(), nil, true)
	require.NoError(err)
	...
	trieIntf2, err := trie1.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte("key"), Value: []byte("value")}}, true)
	require.NoError(err)
...
func Test_Trie_Overwrite(t *testing.T) {
	...
	require.NoError(err)
	require.NotNil(dbTrie)
	trie, err := dbTrie.NewViewFromBatchOps(context.Background(), []database.BatchOp{
...
func Test_Trie_Delete(t *testing.T) {
	...
	trie, err := dbTrie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte("key"), Value: []byte("value0")}}, true)
	require.NoError(err)
	...
	trie, err = dbTrie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte("key"), Delete: true}}, true)
	require.NoError(err)
...
func Test_Trie_ExpandOnKeyPath(t *testing.T) {
	...
	require.NoError(err)
	require.NotNil(dbTrie)
	trieIntf, err := dbTrie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte("key"), Value: []byte("value0")}}, true)
	require.NoError(err)
	trie := trieIntf.(*trieView)
	...
	trieIntf, err = trie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte("key1"), Value: []byte("value1")}}, true)
	require.NoError(err)
	trie = trieIntf.(*trieView)
	...
	trieIntf, err = trie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte("key12"), Value: []byte("value12")}}, true)
	require.NoError(err)
	trie = trieIntf.(*trieView)
...
func Test_Trie_CompressedPaths(t *testing.T) {
	...
	require.NoError(err)
	require.NotNil(dbTrie)
	trieIntf, err := dbTrie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte("key12"), Value: []byte("value12")}}, true)
	require.NoError(err)
	trie := trieIntf.(*trieView)
	...
	trieIntf, err = trie.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte("key"), Value: []byte("value")}}, true)
	require.NoError(err)
	trie = trieIntf.(*trieView)
...
func Test_Trie_HashCountOnBranch(t *testing.T) {
	...
	require.NoError(err)
	require.NotNil(dbTrie)
	trieIntf, err := dbTrie.NewViewFromBatchOps(context.Background(), nil, true)
	require.NoError(err)
	trie := trieIntf.(*trieView)
...
func Test_Trie_HashCountOnDelete(t *testing.T) {
	...
	view, err := trie.NewViewFromBatchOps(context.Background(), []database.BatchOp{
...
func Test_Trie_ChainDeletion(t *testing.T) {
	...
	require.NoError(err)
	require.NotNil(trie)
	newTrie, err := trie.NewViewFromBatchOps(context.Background(), []database.BatchOp{
		{Key: []byte("k"), Value: []byte("value0")},
		{Key: []byte("ke"), Value: []byte("value1")},
		{Key: []byte("key"), Value: []byte("value2")},
		{Key: []byte("key1"), Value: []byte("value3")},
	...
	newTrie, err = newTrie.NewViewFromBatchOps(context.Background(), []database.BatchOp{
		{Key: []byte("k"), Delete: true},
		{Key: []byte("ke"), Delete: true},
		{Key: []byte("key"), Delete: true},
		{Key: []byte("key1"), Delete: true},
...
func Test_Trie_Invalidate_Siblings_On_Commit(t *testing.T) {
	...
	view1, err := dbTrie.NewViewFromBatchOps(context.Background(), nil, true)
	require.NoError(err)

	view2, err := view1.NewViewFromBatchOps(context.Background(), []database.BatchOp{{Key: []byte{0}, Value: []byte{0}}}, true)
	require.NoError(err)

	// Siblings of view2
	sibling1, err := view1.NewViewFromBatchOps(context.Background(), nil, true)
	require.NoError(err)
	sibling2, err := view1.NewViewFromBatchOps(context.Background(), nil, true)
	require.NoError(err)
...
func Test_Trie_NodeCollapse(t *testing.T) {
	...
	// delete the middle values
	trie, err = trie.NewViewFromBatchOps(context.Background(), []database.BatchOp{
		{Key: []byte("k"), Delete: true},
		{Key: []byte("ke"), Delete: true},
		{Key: []byte("key"), Delete: true},
	}, true)
...
func Test_Trie_MultipleStates(t *testing.T) {
	...
	for _, commitApproach := range []string{"never", "before", "after"} {
		t.Run(commitApproach, func(t *testing.T) {
			...
			root, err := db.NewViewFromBatchOps(context.Background(), ops, true)
			require.NoError(err)
			...
			for i := 0; i < 5; i++ {
				newState, err := root.NewViewFromBatchOps(context.Background(), nil, true)
				require.NoError(err)
			...
			for index, state := range concurrentStates {
				concurrentStates[index], err = state.NewViewFromBatchOps(context.Background(), concurrentOps[index], true)
				require.NoError(err)
			}
...
func TestNewViewOnCommittedView(t *testing.T) {
	...
	view2Intf, err := view1.NewViewFromBatchOps(context.Background(), nil, true)
	require.NoError(err)
	...
	// Make another view
	view3Intf, err := view2.NewViewFromBatchOps(context.Background(), nil, true)
	require.NoError(err)
...
func Test_TrieView_NewView(t *testing.T) {
	...
	_, err = invalidView.NewViewFromBatchOps(context.Background(), nil, true)
...
func Test_Trie_ConcurrentNewViewAndCommit(t *testing.T) {
	...
	newView, err := newTrie.NewViewFromBatchOps(context.Background(), nil, true)
	require.NoError(err)
...
func TestTrieCommitToDB(t *testing.T) {
	...
	tests := []test{
		{
			name: "invalid",
			trieFunc: func() TrieView {
				view, err := db.NewViewFromBatchOps(context.Background(), nil, true)
				r.NoError(err)
	...
	view, err := db.NewViewFromBatchOps(context.Background(), []database.BatchOp{
