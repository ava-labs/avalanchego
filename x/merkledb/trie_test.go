...
func getNodeValueWithBranchFactor(t ReadOnlyTrie, keyString string, bf BranchFactor) ([]byte, error) {
	if asTrieView, ok := t.(*trieView); ok {
		if err := asTrieView.calculateNodeIDs(context.Background()); err != nil {
			return nil, err
		}
		key := ConvertToKey([]byte(keyString), bf)
		nodePath, err := asTrieView.getPathTo(key)
		if err != nil {
			return nil, err
		}
		closestNode := nodePath[len(nodePath)-1]
		if closestNode.key != key || closestNode == nil {
	...
	if asDatabases, ok := t.(*merkleDB); ok {
		...
		key := ConvertToKey([]byte(keyString), bf)
		nodePath, err := view.(*trieView).getPathTo(key)
		if err != nil {
			return nil, err
		}
		closestNode := nodePath[len(nodePath)-1]
...
func TestTrieViewGetPathTo(t *testing.T) {
	...
	nodePath, err := trie.getPathTo(ConvertToKey(nil, BranchFactor16))
	require.NoError(err)
	...
	nodePath, err = trie.getPathTo(ConvertToKey(key1, BranchFactor16))
	require.NoError(err)

	// Root and 1 value
	require.Len(nodePath, 2)
	require.Equal(trie.root, nodePath[0])
	require.Equal(ConvertToKey(key1, BranchFactor16), nodePath[1].key)
	...
	nodePath, err = trie.getPathTo(ConvertToKey(key2, BranchFactor16))
	require.NoError(err)
	require.Len(nodePath, 3)
	require.Equal(trie.root, nodePath[0])
	require.Equal(ConvertToKey(key1, BranchFactor16), nodePath[1].key)
	require.Equal(ConvertToKey(key2, BranchFactor16), nodePath[2].key)
	...
	nodePath, err = trie.getPathTo(ConvertToKey(key3, BranchFactor16))
	require.NoError(err)
	require.Len(nodePath, 2)
	require.Equal(trie.root, nodePath[0])
	require.Equal(ConvertToKey(key3, BranchFactor16), nodePath[1].key)
	...
	key4 := []byte{0, 1, 2}
	nodePath, err = trie.getPathTo(ConvertToKey(key4, BranchFactor16))
	require.NoError(err)
	require.Len(nodePath, 3)
	require.Equal(trie.root, nodePath[0])
	require.Equal(ConvertToKey(key1, BranchFactor16), nodePath[1].key)
	...
	key5 := []byte{128}
	nodePath, err = trie.getPathTo(ConvertToKey(key5, BranchFactor16))
	require.NoError(err)
	require.Len(nodePath, 1)
	require.Equal(trie.root, nodePath[0])
}
...
func Test_Trie_WriteToDB(t *testing.T) {
	...
	node, err := parseNode(ConvertToKey(key, BranchFactor16), rawBytes)
	require.NoError(err)
...
func Test_Trie_HashCountOnBranch(t *testing.T) {
	...
	_, err = view2.getEditableNode(ConvertToKey(keyPrefix, BranchFactor16), false)
	require.NoError(err)
...
func getSingleChildKey(n *node) Key {
	for index, entry := range n.children {
		return n.key.Append(index).Extend(entry.compressedKey)
	}
	return Key{}
}
