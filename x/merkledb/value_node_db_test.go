...
func TestValueNodeDB(t *testing.T) {
	...
	// Put a key-node pair.
	node1 := &node{value: maybe.Some([]byte{0x01})}
	batch := db.NewBatch()
	batch.Put(key, node1)
	...
	node2 := &node{value: maybe.Some([]byte{0x02})}
	batch = db.NewBatch()
	batch.Put(key, node1)
	batch.Put(key, node2)
	require.NoError(batch.Write())
...
func TestValueNodeDBIterator(t *testing.T) {
	...
	for i := 0; i < cacheSize; i++ {
		...
		node := &node{value: maybe.Some([]byte{byte(i)})}
		batch := db.NewBatch()
		batch.Put(key, node)
		require.NoError(batch.Write())
	}
	...
	n := &node{value: maybe.Some([]byte{0xFF, 0x00})}
	batch := db.NewBatch()
	batch.Put(key, n)
	require.NoError(batch.Write())

	key = ToKey([]byte{0xFF, 0x01})
	n = &node{value: maybe.Some([]byte{0xFF, 0x01})}
	batch = db.NewBatch()
	batch.Put(key, n)
	require.NoError(batch.Write())
