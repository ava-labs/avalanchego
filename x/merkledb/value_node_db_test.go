...
func TestValueNodeDB(t *testing.T) {
	...
	key := ConvertToKey([]byte{0x01}, BranchFactor16)
	_, err := db.Get(key)
...
func TestValueNodeDBIterator(t *testing.T) {
	...
	for i := 0; i < cacheSize; i++ {
		key := ConvertToKey([]byte{byte(i)}, BranchFactor16)
		node := &node{
			dbNode: dbNode{
	...
	key := ConvertToKey([]byte{0xFF, 0x00}, BranchFactor16)
	n := &node{
		dbNode: dbNode{
	...
	key = ConvertToKey([]byte{0xFF, 0x01}, BranchFactor16)
	n = &node{
		dbNode: dbNode{
