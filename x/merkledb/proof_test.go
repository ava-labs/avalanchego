...
func writeBasicBatch(t *testing.T, db *Database) {
	batch := db.NewBatch()
	require.NoError(t, batch.Put(context.Background(), []byte{0}, []byte{0}))
	require.NoError(t, batch.Put(context.Background(), []byte{1}, []byte{1}))
	require.NoError(t, batch.Put(context.Background(), []byte{2}, []byte{2}))
	require.NoError(t, batch.Put(context.Background(), []byte{3}, []byte{3}))
	require.NoError(t, batch.Put(context.Background(), []byte{4}, []byte{4}))
...
func Test_Proof_MissingValue(t *testing.T) {
	...
	require.NoError(t, trie.Put(context.Background(), []byte{1}, []byte{0}))
	require.NoError(t, trie.Put(context.Background(), []byte{1, 2}, []byte{0}))
	require.NoError(t, trie.Put(context.Background(), []byte{1, 2, 4}, []byte{0}))
	require.NoError(t, trie.Put(context.Background(), []byte{1, 3}, []byte{0}))
...
func Test_RangeProof_Extra_Value(t *testing.T) {
	...
	val, err := db.Get(context.Background(), []byte{2})
	require.NoError(t, err)
...
func Test_RangeProof_NilStart(t *testing.T) {
	...
	require.NoError(t, err)
	batch := db.NewBatch()
	err = batch.Put(context.Background(), []byte("key1"), []byte("value1"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key2"), []byte("value2"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key3"), []byte("value3"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key4"), []byte("value4"))
	require.NoError(t, err)
	err = batch.Write()
	require.NoError(t, err)

	val, err := db.Get(context.Background(), []byte("key1"))
	require.NoError(t, err)
...
func Test_RangeProof_EmptyValues(t *testing.T) {
	...
	require.NoError(t, err)
	batch := db.NewBatch()
	err = batch.Put(context.Background(), []byte("key1"), nil)
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key12"), []byte("value1"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key2"), []byte{})
	require.NoError(t, err)
	err = batch.Write()
	require.NoError(t, err)

	val, err := db.Get(context.Background(), []byte("key12"))
	require.NoError(t, err)
	require.Equal(t, []byte("value1"), val)
...
func Test_RangeProof_Marshal_Nil(t *testing.T) {
	...
	val, err := db.Get(context.Background(), []byte{1})
	require.NoError(t, err)
...
func Test_ChangeProof_Marshal(t *testing.T) {
	...
	require.NoError(t, err)
	batch := db.NewBatch()
	err = batch.Put(context.Background(), []byte("key0"), []byte("value0"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key1"), []byte("value1"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key2"), []byte("value2"))
	...
	batch = db.NewBatch()
	err = batch.Put(context.Background(), []byte("key4"), []byte("value0"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key5"), []byte("value1"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key6"), []byte("value2"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key7"), []byte("value3"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key8"), []byte("value4"))
	require.NoError(t, err)
	err = batch.Write()
	require.NoError(t, err)

	batch = db.NewBatch()
	err = batch.Put(context.Background(), []byte("key9"), []byte("value0"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key10"), []byte("value1"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key11"), []byte("value2"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key12"), []byte("value3"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key13"), []byte("value4"))
	require.NoError(t, err)
	err = batch.Write()
	require.NoError(t, err)
...
func Test_ChangeProof_Marshal_Errors(t *testing.T) {
	...
	batch := db.NewBatch()
	require.NoError(t, batch.Put(context.Background(), []byte{5}, []byte{5}))
	require.NoError(t, batch.Put(context.Background(), []byte{6}, []byte{6}))
	require.NoError(t, batch.Put(context.Background(), []byte{7}, []byte{7}))
	require.NoError(t, batch.Put(context.Background(), []byte{8}, []byte{8}))
	require.NoError(t, batch.Delete(context.Background(), []byte{0}))
	require.NoError(t, batch.Write())

	batch = db.NewBatch()
	require.NoError(t, batch.Put(context.Background(), []byte{9}, []byte{9}))
	require.NoError(t, batch.Put(context.Background(), []byte{10}, []byte{10}))
	require.NoError(t, batch.Put(context.Background(), []byte{11}, []byte{11}))
	require.NoError(t, batch.Put(context.Background(), []byte{12}, []byte{12}))
	require.NoError(t, batch.Delete(context.Background(), []byte{1}))
	require.NoError(t, batch.Write())
	endroot, err := db.GetMerkleRoot(context.Background())
	require.NoError(t, err)
...
func Test_ChangeProof_BadBounds(t *testing.T) {
	...
	require.NoError(t, db.Put(context.Background(), []byte{0}, []byte{0}))
...
func Test_ChangeProof_Verify(t *testing.T) {
	...
	require.NoError(t, err)
	batch := db.NewBatch()
	err = batch.Put(context.Background(), []byte("key20"), []byte("value0"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key21"), []byte("value1"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key22"), []byte("value2"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key23"), []byte("value3"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key24"), []byte("value4"))
	require.NoError(t, err)
	err = batch.Write()
	require.NoError(t, err)
	startRoot, err := db.GetMerkleRoot(context.Background())
	...
	batch = dbClone.NewBatch()
	err = batch.Put(context.Background(), []byte("key31"), []byte("value1"))
	require.NoError(t, err)
	err = batch.Write()
	require.NoError(t, err)

	batch = db.NewBatch()
	err = batch.Put(context.Background(), []byte("key25"), []byte("value0"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key26"), []byte("value1"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key27"), []byte("value2"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key28"), []byte("value3"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key29"), []byte("value4"))
	require.NoError(t, err)
	err = batch.Write()
	require.NoError(t, err)

	batch = db.NewBatch()
	err = batch.Put(context.Background(), []byte("key30"), []byte("value0"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key31"), []byte("value1"))
	require.NoError(t, err)
	err = batch.Put(context.Background(), []byte("key32"), []byte("value2"))
	require.NoError(t, err)
	err = batch.Delete(context.Background(), []byte("key21"))
	require.NoError(t, err)
	err = batch.Delete(context.Background(), []byte("key22"))
	require.NoError(t, err)
	err = batch.Write()
	require.NoError(t, err)
