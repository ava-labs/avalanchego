...
func Test_Creation(t *testing.T) {
	...
	syncer, err := sync.NewSyncer(
		...
		sync.Config[*RangeProof, *ChangeProof]{
			RangeProofMarshaler:   rangeProofMarshaler,
			ChangeProofMarshaler:  changeProofMarshaler,
			ProofClient:           p2ptest.NewSelfClient(t, ctx, ids.EmptyNodeID, sync.NewGetProofHandler(db, rangeProofMarshaler, changeProofMarshaler)),
...
func Test_Sync_Result_Correct_Root(t *testing.T) {
		...
		},
		{
			...
			proofClient: func(db MerkleDB) *p2p.Client {
				return p2ptest.NewSelfClient(t, t.Context(), ids.EmptyNodeID, &flakyHandler{
					Handler: sync.NewGetProofHandler(db, rangeProofMarshaler, changeProofMarshaler),
					c:       &counter{m: 2},
				})
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			proofClient := p2ptest.NewSelfClient(t, ctx, ids.EmptyNodeID, sync.NewGetProofHandler(dbToSync, rangeProofMarshaler, changeProofMarshaler))
...
func Test_Sync_Result_Correct_Root_Update_Root_During(t *testing.T) {
	...
	actionHandler := synctest.NewCounterHandler(sync.NewGetProofHandler(dbToSync, rangeProofMarshaler, changeProofMarshaler), func() {
