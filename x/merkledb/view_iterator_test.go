...
func Test_TrieView_Iterator_Random(t *testing.T) {
	...
	view1, err := db.NewViewFromBatchOps(context.Background(), ops, true)
	require.NoError(err)
	...
	view2, err := view1.NewViewFromBatchOps(context.Background(), ops, true)
	require.NoError(err)
	...
	view3, err := view2.NewViewFromBatchOps(context.Background(), ops, true)
	require.NoError(err)
