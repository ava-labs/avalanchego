...
func Test_History_Simple(t *testing.T) {
	...
	origProof, err := db.GetRangeProof(context.Background(), Some([]byte("k")), Some([]byte("key3")), 10)
	require.NoError(err)
	require.NotNil(origProof)
	origRootID := db.root.id
	require.NoError(origProof.Verify(context.Background(), Some([]byte("k")), Some([]byte("key3")), origRootID))

	batch = db.NewBatch()
	require.NoError(batch.Put([]byte("key"), []byte("value0")))
	require.NoError(batch.Write())
	newProof, err := db.GetRangeProofAtRoot(context.Background(), origRootID, Some([]byte("k")), Some([]byte("key3")), 10)
	require.NoError(err)
	require.NotNil(newProof)
	require.NoError(newProof.Verify(context.Background(), Some([]byte("k")), Some([]byte("key3")), origRootID))
...
func Test_History_Large(t *testing.T) {
	...
	for i := 1; i < 10; i++ {
		...
		proof, err := db.GetRangeProofAtRoot(context.Background(), roots[0], Nothing[[]byte](), Nothing[[]byte](), 10)
		require.NoError(err)
		require.NotNil(proof)

		require.NoError(proof.Verify(context.Background(), Nothing[[]byte](), Nothing[[]byte](), roots[0]))
	}
}
...
func Test_History_Bad_GetValueChanges_Input(t *testing.T) {
	...
	_, err = db.history.getValueChanges(toBeDeletedRoot, endRoot, Nothing[[]byte](), Nothing[[]byte](), 1)
	require.NoError(err)
	_, err = db.history.getValueChanges(startRoot, endRoot, Nothing[[]byte](), Nothing[[]byte](), 1)
	require.NoError(err)

	_, err = db.history.getValueChanges(startRoot, endRoot, Nothing[[]byte](), Nothing[[]byte](), -1)
	require.ErrorIs(err, ErrInvalidMaxLength)

	_, err = db.history.getValueChanges(endRoot, startRoot, Nothing[[]byte](), Nothing[[]byte](), 1)
	...
	changes, err := db.history.getValueChanges(endRoot, endRoot, Nothing[[]byte](), Nothing[[]byte](), 10)
	require.NoError(err)
...
func Test_History_Trigger_History_Queue_Looping(t *testing.T) {
	...
	_, err = db.GetRangeProofAtRoot(context.Background(), origRootID, Some([]byte("k")), Some([]byte("key3")), 10)
...
func Test_History_Values_Lookup_Over_Queue_Break(t *testing.T) {
	...
	changes, err := db.history.getValueChanges(startRoot, endRoot, Nothing[[]byte](), Nothing[[]byte](), 10)
	require.NoError(err)
...
func Test_Change_List(t *testing.T) {
	...
	changes, err := db.history.getValueChanges(startRoot, endRoot, Nothing[[]byte](), Nothing[[]byte](), 8)
	require.NoError(err)
...
func TestHistoryGetChangesToRoot(t *testing.T) {
	...
	tests := []test{
		...
		},
		{
			...
			rootID: changes[maxHistoryLen-3].rootID,
			start:  Some([]byte{byte(maxHistoryLen - 1)}), // Omit values from second most recent change
			validateFunc: func(require *require.Assertions, got *changeSummary) {
				require.Len(got.nodes, 2)
				require.Len(got.values, 1)
				reversedChanges1 := changes[maxHistoryLen-1]
		...
		},
		{
			...
			rootID: changes[maxHistoryLen-3].rootID,
			end:    Some([]byte{byte(maxHistoryLen - 2)}), // Omit values from most recent change
			validateFunc: func(require *require.Assertions, got *changeSummary) {
				require.Len(got.nodes, 2)
				require.Len(got.values, 1)
				reversedChanges1 := changes[maxHistoryLen-1]
