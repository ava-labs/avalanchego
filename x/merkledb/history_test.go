...
func Test_History_Large(t *testing.T) {
	...
	for i := 1; i < 5; i++ {
		...
		for i := 0; i < numIters; i += numIters / 10 {
			...
			require.NoError(proof.Verify(context.Background(), maybe.Nothing[[]byte](), maybe.Nothing[[]byte](), roots[i], BranchFactorToTokenSize[config.BranchFactor], SHA256Hasher))
		}
	}
}
