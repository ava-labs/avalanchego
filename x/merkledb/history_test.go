...
func Test_History_Simple(t *testing.T) {
	...
	require.NoError(origProof.Verify(context.Background(), maybe.Some([]byte("k")), maybe.Some([]byte("key3")), origRootID, db.tokenSize, db.hasher, 10))
	...
	require.NoError(newProof.Verify(context.Background(), maybe.Some([]byte("k")), maybe.Some([]byte("key3")), origRootID, db.tokenSize, db.hasher, 10))
...
func Test_History_Large(t *testing.T) {
	...
	for i := 1; i < 5; i++ {
		...
		for i := 0; i < numIters; i += numIters / 10 {
			...
			require.NoError(proof.Verify(context.Background(), maybe.Nothing[[]byte](), maybe.Nothing[[]byte](), roots[i], BranchFactorToTokenSize[config.BranchFactor], config.Hasher, 10))
		}
	}
}
...
func Test_Change_List(t *testing.T) {
	...
	changes, err = db.history.getValueChanges(startRoot, endRoot, maybe.Some([]byte("key22")), maybe.Some([]byte("key31")), 8)
	require.NoError(err)
