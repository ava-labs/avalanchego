...
func Test_History_Simple(t *testing.T) {
	...
	require.NoError(db.VerifyRangeProof(context.Background(), origProof, maybe.Some([]byte("k")), maybe.Some([]byte("key3")), origRootID))
	...
	require.NoError(db.VerifyRangeProof(context.Background(), newProof, maybe.Some([]byte("k")), maybe.Some([]byte("key3")), origRootID))
...
func Test_History_Large(t *testing.T) {
	...
	for i := 1; i < 5; i++ {
		...
		for i := 0; i < numIters; i += numIters / 10 {
			...
			require.NoError(db.VerifyRangeProof(context.Background(), proof, maybe.Nothing[[]byte](), maybe.Nothing[[]byte](), roots[i]))
		}
	}
}
