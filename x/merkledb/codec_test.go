...
func FuzzCodecDBNodeCanonical(f *testing.F) {
	f.Fuzz(
		func(
			...
			require := require.New(t)
			node := &dbNode{}
			if err := decodeDBNode(DefaultHasher, b, node); err != nil {
				t.SkipNow()
			}
	...
	f.Fuzz(
		func(
			...
			for _, bf := range validBranchFactors {
				...
				var gotNode dbNode
				require.NoError(decodeDBNode(DefaultHasher, nodeBytes, &gotNode))
...
func TestCodecDecodeDBNode_TooShort(t *testing.T) {
	...
	err := decodeDBNode(DefaultHasher, tooShortBytes, &parsedDBNode)
...
func TestDecodeDBNode(t *testing.T) {
	for _, test := range encodeDBNodeTests {
		t.Run(test.name, func(t *testing.T) {
			...
			var n dbNode
			require.NoError(decodeDBNode(DefaultHasher, test.expectedBytes, &n))
...
func Benchmark_DecodeDBNode(b *testing.B) {
	for _, benchmark := range encodeDBNodeTests {
		b.Run(benchmark.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				var n dbNode
				err := decodeDBNode(DefaultHasher, benchmark.expectedBytes, &n)
