...
var (
	...
	ErrTooManyKeys = errors.New("too many keys in proof")
)
...
func (db *merkleDB) VerifyChangeProof(
	...
	if len(proof.KeyChanges) > maxKeys {
		return fmt.Errorf("%w: (%d) > %d", ErrTooManyKeys, len(proof.KeyChanges), maxKeys)
	}
...
func (db *merkleDB) VerifyRangeProof(
	...
	return proof.Verify(ctx, start, end, expectedEndRootID, db.tokenSize, db.hasher, maxKeys)
}
