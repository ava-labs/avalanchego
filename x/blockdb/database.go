...
func (s *Database) persistIndexHeaderInternal() error {
	...
	header.MaxHeight = s.getMaxBlockHeight()
...
func (s *Database) getMaxBlockHeight() BlockHeight {
...
func (s *Database) recoverUnindexedBlocks(startOffset, endOffset uint64) error {
	...
	if len(recoveredHeights) > 0 {
		s.updateRecoveredBlockHeights(recoveredHeights)
	}
...
func (s *Database) updateRecoveredBlockHeights(recoveredHeights []BlockHeight) {
	...
	currentMaxHeight := s.getMaxBlockHeight()
