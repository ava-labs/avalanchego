...
func TestDatabaseInspect(t *testing.T) {
	tempDir := t.TempDir()
	config := DefaultConfig().WithDir(tempDir).WithMinimumHeight(3).WithCompressBlocks(false)
	...
	testBlocks := []struct {
		...
		{3, []byte("header0:block0")},
		{4, []byte("header1:block1")},
		{5, []byte("header2:block2")},
		{6, []byte("header5:block5")},
		{7, []byte("header6:block6")},
		{9, []byte("header7:block7")},
		{10, []byte("header10:block10")},
		{11, []byte("header11:block11")},
	}
	...
	require.Contains(t, result, "Total Database Size: 508 bytes")
	require.Contains(t, result, "Index File Size: 208 bytes")
	// 6 blocks of 14 bytes + 2 blocks of 16 bytes each + 23 bytes for block entry header
	require.Contains(t, result, "Data Files Size: 300 bytes")
	require.Contains(t, result, "Number of Data Files: 1")
	require.Contains(t, result, "Average Block Size: 37 bytes")
}
