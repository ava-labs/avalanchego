...
var _ = ginkgo.Describe("[Staking Rewards]", func() {
	...
	ginkgo.It("should ensure that validator node uptime determines whether a staking reward is issued", func() {
		...
		tc.By("checking expected rewards against actual rewards", func() {
			...
			validatorRewardConfig, err := executor.GetHeliconRewardConfig(rewardConfig, *upgrades, alphaStartTime)
			require.NoError(err)
			delegatorRewardConfig, err := executor.GetHeliconRewardConfig(rewardConfig, *upgrades, gammaStartTime)
			require.NoError(err)
