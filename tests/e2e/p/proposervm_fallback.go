...
import (
	...
	_ "github.com/ava-labs/avalanchego/vms/example/xsvm/cmd/issue/transfer"
...
var _ = e2e.DescribePChain("fallback", func() {
	...
	ginkgo.It("creates and updates L1 validators", func() {
		...
		subnetRegisterNode := e2e.AddEphemeralNode(tc, env.GetNetwork(), tmpnet.NewEphemeralNode(tmpnet.FlagsMap{
			config.FallbackNonValidatorProposerMaxWaitTime: "5s",
			config.FallbackNonValidatorCanPropose:          "true",
			config.TrackSubnetsKey:                         subnetID.String(),
		}))
		...
		tc.By("issuing an XSVM transaction on the L1", func() {
			address := keychain.Keys[0].Address()

			status, err := transfer.Transfer(context.Background(), &transfer.Config{
				WaitForAcceptance: true,
				URI:               subnetRegisterNode.URI,
				ChainID:           chainID,
				AssetID:           chainID,
			...
			tc.By(fmt.Sprintf("transfer status: %+v", status))
		})
		...
		tc.By("increasing the weight of the validator", func() {
			setWeight(registerValidationID, genesisWeight*10)
		})
		...
		tc.By("advancing the proposervm P-chain height, again", advanceProposerVMPChainHeight)

		tc.By("Killing the genesis peer", func() {
			subnetGenesisNode.Stop(context.Background())
		})
