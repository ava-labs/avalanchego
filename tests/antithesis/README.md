...
**Note on subnet-evm**: The subnet-evm test setup has a hybrid structure to maintain architectural boundaries. The Dockerfiles are located in `tests/antithesis/subnet-evm/`, but the Go source code (`main.go` and `gencomposeconfig/main.go`) remains in `graft/subnet-evm/tests/antithesis/` because it needs to import from `graft/subnet-evm` packages, which are not allowed to be imported outside of `graft/subnet-evm` itself (except for `vms/evm/emulate`). The root-level build scripts orchestrate building the binaries from their graft location. Once the graft folder has been fully subsumed into `vms/evm`, we can move all the files properly.
// TODO(JonathanOppenheimer): do the above!
