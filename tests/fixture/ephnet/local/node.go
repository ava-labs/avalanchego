...
func (n *LocalNode) GetConfig() ephnet.NodeConfig {
	return n.NodeConfig
}
...
func (n *LocalNode) ReadConfig() error {
	...
	config := ephnet.NodeConfig{Flags: flags}
...
func (n *LocalNode) WriteConfig() error {
	...
	bytes, err := ephnet.DefaultJSONMarshal(n.Flags)
	if err != nil {
...
func (n *LocalNode) Stop() error {
	...
	ticker := time.NewTicker(ephnet.DefaultNodeTickerInterval)
	defer ticker.Stop()
...
func (n *LocalNode) IsHealthy(ctx context.Context) (bool, error) {
	...
	if proc == nil {
		return false, ephnet.ErrNotRunning
	}
