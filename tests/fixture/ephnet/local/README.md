...
allows for the `ephnetctl` cli and e2e test fixture to orchestrate
...
A local network can be managed by the `ephnetctl` cli tool:
...
# Stop the network
$ ./build/ephnetctl stop-network --network-dir=/path/to/network
```

Note the export of the path ending in `latest`. This is a symlink that
is set to the last network created by `ephnetctl start-network`. Setting
the `EPHNET_NETWORK_DIR` env var to this symlink ensures that
`ephnetctl` commands and e2e execution with
...
chosen. The ephnet fixture discovers the ports used by a given node
...
```
HOME
└── .ephnet                                              // Root path for the ephemeral network fixture
...
A shell script that sets the `EPHNET_NETWORK_DIR` env var to the
...
e2e and the `ephnetctl` cli to target the network path specified in
the env var.
