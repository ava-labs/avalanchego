...
func deployKubeCollector(
	...
	if err != nil {
		return fmt.Errorf("failed to get collector config for %s: %w", collectorConfig.name, err)
	}

	if err := createCollectorConfigSecret(ctx, log, clientset, collectorConfig.secretPrefix, config); err != nil {
		return fmt.Errorf("failed to create config secret for %s: %w", collectorConfig.name, err)
	}
