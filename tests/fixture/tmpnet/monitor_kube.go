...
func deployKubeCollector(
	...
	if err != nil {
		return fmt.Errorf("failed to get collector config for %s: %w", kubeConfig.name, err)
	}

	if err := createCollectorConfigSecret(ctx, log, clientset, kubeConfig.secretPrefix, config); err != nil {
		return fmt.Errorf("failed to create collector config secret for %s: %w", kubeConfig.name, err)
	}

	if err := applyManifest(ctx, log, dynamicClient, kubeConfig.manifest, monitoringNamespace); err != nil {
		return fmt.Errorf("failed to apply manifest for %s: %w", kubeConfig.name, err)
	}
	return nil
}
