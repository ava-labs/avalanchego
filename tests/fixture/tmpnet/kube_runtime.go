...
func (p *KubeRuntime) readState(ctx context.Context) error {
	...
	// TODO(marun) Implement 'waitUntilReady' which is the kube
	// equivalent of a node's process runtime file being written,
	// indicating that the node is accepting http requests. This isn't
	// part of `Start` to allow multiple nodes to be started and then
	// become ready in parallel.
...
func (p *KubeRuntime) getClientset() (*kubernetes.Clientset, error) {
	// TODO(marun) Implement support for kube config context
