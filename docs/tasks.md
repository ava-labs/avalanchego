...
[`tools/external/go.mod`](../tools/external/go.mod) must use the same version.  This
keeps the Nix development shell, the local `go tool` fallback, and the CI Task release
on one version.

Run `task check-task-version` to check the versions. This task does not change
repository files. The lint tasks run it. If this task reports a version mismatch after
a Nix update, run `task sync-task-version`. The sync task changes the Task bootstrap
dependency, its module sums, and the checked-in Task release checksums. A Task update
can also change indirect requirements shared with other tools. Review every change in
the tool module. Both tasks run in the Nix development shell.
