...
Each module has its own `go.mod` with `replace` directives pointing to sibling modules. Bazel handles cross-module imports via `go.work` and gazelle prefix directives.
...
**Preserving custom content**: Gazelle directives (`# gazelle:prefix`, `# gazelle:map_kind`) are automatically preserved. For other custom content (like `x_defs`), use `# keep` comments to prevent gazelle from removing them:
...
**Rule of thumb**: Don't delete BUILD.bazel files that contain gazelle directives or `# keep` comments.
...
Gazelle is declared as a `bazel_dep` in `MODULE.bazel`. It is **not** provided by Nix. When you run `task bazel-gazelle-generate`, it executes `bazel run //:gazelle` which downloads and runs Gazelle via Bazel.
...
The graft modules (coreth, evm, subnet-evm) have longer test timeouts than the root module. To handle this, custom test macros are defined in `.bazel/defs.bzl`:
...
Gazelle automatically uses these macros via `gazelle:map_kind` directives in each graft module's root BUILD.bazel:
...
This remaps all `go_test` targets in that subtree to use the custom macro with the longer timeout.
...
The `go.work` file aggregates all module dependencies (root + graft modules), so dependencies from nested modules are resolved automatically.
...
The repository uses pre-generated `.pb.go` files checked into source control rather than generating them at build time. This avoids proto toolchain complexity in Bazel while maintaining compatibility with the existing `go generate` workflow.
...
- **Shuffle**: ON
- **Race detection**: OFF (enable with `--config=race`)
- **Coverage**: Use `bazel coverage` instead of `bazel test`

```bash
# Run all unit tests (shuffle enabled, race off for faster iteration)
...
| Fast mode | - | `--config=fast` (no shuffle) |
...
# Fast mode (no shuffle, race already off)
bazel test --config=fast //...
```

#### Non-Unit Tests and the `manual` Tag

Tests that are not unit tests (e2e tests, integration tests, load tests) must have `tags = ["manual"]` in their BUILD.bazel file. This excludes them from `bazel test //...` which should only run unit tests.

This mirrors the behavior of `scripts/build_test.sh`, which excludes these directories via grep:
```bash
...
Check `.bazelrc` for required CGO flags. CGO is enabled via `--action_env=CGO_ENABLED=1`.
...
Some packages require specific CGO flags (e.g., BLST cryptography). These are configured in `.bazelrc` or handled via patches.
...
Currently only local disk cache is configured (`~/.cache/bazel-disk-cache`). Remote caching would enable:
...
- **Migrate CI to Bazel** - Update GitHub Actions to use Bazel for builds/tests where possible
- **Retain non-Bazel coverage** - Keep traditional `go build` CI jobs to support third-party consumers who don't use Bazel

### gnark-crypto Assembly Performance

The `purego` build tag is currently used for gnark-crypto (see `.bazelrc`), which disables assembly optimizations. This results in ~2-10x slower cryptographic operations.

Root cause: Go assembly files with relative `#include` paths break in Bazel sandboxes.

Potential solutions:
- Upstream fix to consensys/gnark-crypto
- Custom patches similar to blst handling
...
Graft modules already have custom test timeouts via `gazelle:map_kind` (see "Custom Test Macros" section). Consider further stratification:
