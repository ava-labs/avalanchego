...
1. `rust_static_library` — Compiles Rust FFI source into a static `.a`
   ...
   (unconditional in Cargo.toml, but jemalloc doesn't build on Windows).
...
**CI:** The FFI tests run as part of `bazel test //...` in the
`Bazel_Unit` CI job. Use `task bazel-firewood-ffi` to run them in
isolation locally.
