...
| Tool | Pin Mechanism | Rationale |
|------|---------------|-----------|
| Bazel | `.bazelversion` + bazelisk | Current LTS with native bzlmod support |
| Go | `go.mod` via `MODULE.bazel` `go_sdk.from_file()` | Single source of truth for Bazel Go toolchain version |
| Rust | `MODULE.bazel` rust.toolchain | Must match `firewood/Cargo.toml` rust-version |
| rules_go | `MODULE.bazel` | Go toolchain rules for Bazel builds |
| rules_rust | `MODULE.bazel` | Rust toolchain + crate_universe for firewood |
| gazelle | `MODULE.bazel` | BUILD file generation for Go/Bazel sources |
