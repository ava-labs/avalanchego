...
TESTING_CONTROLLER_IMAGE="$DOCKER_REPO/avalanche-testing_controller:0.9.1"
BYZANTINE_IMAGE="$DOCKER_REPO/gecko-byzantine:everest-hex"
...
if [[ ${#DOCKER_USERNAME} == 0 ]]; then
    ...
    ./scripts/run.sh --gecko-image-name="${GECKO_IMAGE}" --test-controller-image-name="${TESTING_CONTROLLER_IMAGE}"
else
    echo "$DOCKER_PASS" | docker login --username "$DOCKER_USERNAME" --password-stdin
    docker pull "${BYZANTINE_IMAGE}"
    ./scripts/run.sh --gecko-image-name="${GECKO_IMAGE}" --test-controller-image-name="${TESTING_CONTROLLER_IMAGE}" --byzantine-image-name="${BYZANTINE_IMAGE}"
fi
