---
name: draft-review
description: Use when the user wants to create, read, update, or delete a GitHub pending draft review body through the repo-local `gh-draft-review` tool as part of a human-in-the-loop PR review workflow. This skill is for top-level review body iteration only; do not use it for inline comment editing or for submitting reviews.
---

# Draft Review

Use the repo-local `gh-draft-review` tool to manipulate the authenticated
...
```bash
--config-dir "$HOME/.config/gh-draft-review"
--state-dir "$HOME/.local/state/gh-draft-review"
```
...
```bash
./bin/gh-draft-review create --pr <number> --body '<text>' --config-dir "$HOME/.config/gh-draft-review" --state-dir "$HOME/.local/state/gh-draft-review"
```
...
```bash
./bin/gh-draft-review get --pr <number> --config-dir "$HOME/.config/gh-draft-review" --state-dir "$HOME/.local/state/gh-draft-review"
```
...
```bash
./bin/gh-draft-review update-body --pr <number> --body '<new text>' --config-dir "$HOME/.config/gh-draft-review" --state-dir "$HOME/.local/state/gh-draft-review"
```
...
```bash
./bin/gh-draft-review delete --pr <number> --config-dir "$HOME/.config/gh-draft-review" --state-dir "$HOME/.local/state/gh-draft-review"
```
