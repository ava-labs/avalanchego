...
This is intentionally **not** a full fake CLI wrapper in the long term.
...
- the CLI/app supports a base URL override
...
- fake state -> GraphQL/REST responses consumed by `pendingreview`
...
1. Add backend override mechanism to pending-review for tests
2. Add a minimal fake backend server
3. Implement first real body-create scenario
4. Add read-body scenario
5. Add update-body scenario
6. Expand to comments
...
- test asserts:
  ...
  - command sequence includes the expected create path
...
- Prefer a fake backend built around the actual subset of GraphQL/REST calls the
  tool currently issues.
...
1. Inspecting how `tools/pendingreview` chooses its GitHub base URL and client
   boundary
2. Adding a test-only backend override path for the real CLI
3. Building the smallest fake backend needed for create/read/update-body
4. Replacing or supplementing the current smoke-style scenario with the first
