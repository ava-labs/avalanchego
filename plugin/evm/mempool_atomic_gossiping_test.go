...
func TestMempool_Add_Gossiped_CreateChainTx(t *testing.T) {
	...
	txBytes, err := encodeAtmTx(vm.codec, tx)
	if err != nil {
		t.Fatal("could not encode atomic tx")
	}
	reqID := vm.IssueID()
	vm.requestsAtmContent[reqID] = tx.ID()
...
func TestMempool_AtmTxs_AppResponseHandling_InvalidTx(t *testing.T) {
	...
	nodeID := ids.ShortID{'n', 'o', 'd', 'e'}
	reqID := vm.IssueID()
	illFormedTxBytes, err := encodeAtmTx(vm.codec, illFormedTx)
	if err != nil {
		t.Fatal("could not encode atomic tx")
	}

	vm.requestsAtmContent[reqID] = illFormedTx.ID()
...
func TestMempool_AtmTxs_AppGossipHandling(t *testing.T) {
	...
	if err := vm.AppGossip(nodeID, txIDBytes); err != nil {
		t.Fatal("error in reception of gossiped tx")
	}
	if !isTxRequested {
...
func TestMempool_AtmTxs_AppGossipHandling_InvalidTx(t *testing.T) {
	...
	nodeID := ids.ShortID{'n', 'o', 'd', 'e'}
	rejectedTxMsg, err := encodeAtmData(vm.codec, rejectedTx.ID())
	if err != nil {
		t.Fatal("could not encode atomic tx ID")
	}
	if err := vm.AppGossip(nodeID, rejectedTxMsg); err != nil {
		t.Fatal("error in reception of gossiped tx")
	}
	if isTxRequested {
...
func TestMempool_AtmTxs_AppRequestHandling_InvalidTx(t *testing.T) {
	...
	// create a tx
	rejectedTx := getTheValidTx(vm, sharedMemory, t)
	rejectedTxIDBytes, err := encodeAtmData(vm.codec, rejectedTx.ID())
	if err != nil {
		t.Fatal("could not encode atomic tx ID")
	}
	...
	nodeID := ids.ShortID{'n', 'o', 'd', 'e'}
	if err := vm.AppRequest(nodeID, vm.IssueID(), rejectedTxIDBytes); err != nil {
		t.Fatal("error in reception of gossiped tx")
	}
	if isResponseIssued {
		t.Fatal("there should be no response with unknown tx")
