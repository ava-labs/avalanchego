...
func NewService(log logging.Logger, chainManager chains.Manager, httpServer server.Server, ipcs *ipcs.ChainIPCs) (http.Handler, error) {
	ipcServer := &IPCServer{
		log:          log,
	...
	return newServer, newServer.RegisterService(ipcServer, "ipcs")
}
