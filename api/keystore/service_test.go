...
func TestMigrateKeystoreUser(t *testing.T) {
	...
	current := memdb.New()
	ks := &Keystore{}
	if err := ks.Initialize(logging.NoLog{}, current); err != nil {
		t.Fatal(err)
	}
	...
	upgradedDB := memdb.New()
	sdb := semanticdb.Create(upgradedDB, current)
	ksUpgraded := &Keystore{}
	if err := ksUpgraded.Initialize(logging.NoLog{}, sdb); err != nil {
		t.Fatal(err)
	}
	...
	secondUpgradedDB := memdb.New()
	sdb2 := semanticdb.Create(secondUpgradedDB, upgradedDB)

	ksSecondUpgrade := &Keystore{}
	if err := ksSecondUpgrade.Initialize(logging.NoLog{}, sdb2); err != nil {
		t.Fatal(err)
	}
