...
var (
	_               simplexcommon.Storage = (*Storage)(nil)
	genesisMetadata                       = simplexcommon.ProtocolMetadata{
		Version: 0,
		Epoch:   0,
		Round:   0,
		Seq:     0,
...
type Storage struct {
	...
	lastIndexedDigest simplexcommon.Digest
...
// If [seq] is not found, returns simplexcommon.ErrBlockNotFound.
func (s *Storage) Retrieve(seq uint64) (simplexcommon.VerifiedBlock, simplexcommon.Finalization, error) {
	// The genesis block doesn't have a finalization, so we need to handle it specifically.
	if seq == 0 {
		return s.genesisBlock, simplexcommon.Finalization{}, nil
	}

	block, err := getBlock(context.TODO(), s.vm, seq)
	if err != nil {
		if errors.Is(err, database.ErrNotFound) {
			return nil, simplexcommon.Finalization{}, simplexcommon.ErrBlockNotFound
		}
...
func (s *Storage) Index(ctx context.Context, block simplexcommon.VerifiedBlock, finalization simplexcommon.Finalization) error {
...
func getGenesisBlock(ctx context.Context, config *Config, blockTracker *blockTracker) (*Block, error) {
	...
	genesis := &Block{
		...
		blacklist:    simplexcommon.NewBlacklist(uint16(len(config.Params.InitialValidators))),
	}
...
func (s *Storage) retrieveFinalization(seq uint64) (simplexcommon.Finalization, error) {
	finalizationBytes, err := s.db.Get(finalizationKey(seq))
	if err != nil {
		if err == database.ErrNotFound {
			return simplexcommon.Finalization{}, simplexcommon.ErrBlockNotFound
		}
		s.log.Debug("Failed to retrieve finalization", zap.Uint64("seq", seq), zap.Error(err))
		return simplexcommon.Finalization{}, err
	}
...
func (s *Storage) retrieveBlacklist(seq uint64) (simplexcommon.Blacklist, error) {
	blacklistBytes, err := s.db.Get(blacklistKey(seq))
	if err != nil {
		if err == database.ErrNotFound {
			return simplexcommon.Blacklist{}, nil
		}
		s.log.Debug("Failed to retrieve blacklist", zap.Uint64("seq", seq), zap.Error(err))
		return simplexcommon.Blacklist{}, err
	}

	var blacklist simplexcommon.Blacklist
	if err := blacklist.FromBytes(blacklistBytes); err != nil {
		return simplexcommon.Blacklist{}, fmt.Errorf("failed to parse blacklist: %w", err)
	}
...
// finalizationToBytes serializes the simplexcommon.Finalization into bytes.
func finalizationToBytes(finalization simplexcommon.Finalization) []byte {
...
// finalizationFromBytes deserialized the bytes into a simplexcommon.Finalization.
func (c *canotoFinalization) toFinalization(d *QCDeserializer) (simplexcommon.Finalization, error) {
