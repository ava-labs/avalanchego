...
func NewComm(myNodeID simplex.NodeID,
	subnetID ids.ID,
	chainID ids.ID,
	logger simplex.Logger,
	validatorLister ValidatorLister,
	sender Sender,
	msgBuilder message.OutboundMsgBuilder) *Comm {
	var nodes []simplex.NodeID
	for _, id := range validatorLister.GetValidatorIDs(subnetID) {
	...
	for i, node := range sortedNodes {
		if node.Equals(myNodeID) {
			logger.Info("Found my node ID", zap.Int("index", i))
			found = true
		}
	}

	if !found {
		logger.Fatal("My node ID not found in validator list")
	}

	c := &Comm{subnetID: subnetID, nodes: sortedNodes, myNodeID: myNodeID, logger: logger, sender: sender, msgBuilder: msgBuilder, chainID: chainID}
	return c
}
