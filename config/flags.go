...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	fs.Uint64(SystemTrackerWarnThreshAvailDiskSpacePercentageKey, 10, fmt.Sprintf("Warning threshold for the percentage (between 0 and 50) of available disk space, under which the node will be considered unhealthy. Must be >= [%s]", SystemTrackerRequiredAvailableDiskSpacePercentageKey))
