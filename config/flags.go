...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	fs.Bool(PChainFollowOnlyKey, false, "If true, the P-chain follows the tip read-only without joining consensus, re-syncing from the peers listed in --bootstrap-ids. The node will always report the P-chain as not bootstrapped")
	// Uptime Requirement
