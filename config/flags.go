...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	fs.Bool(PChainFollowOnlyKey, false, "If true, the P-chain does not join consensus; it follows the tip read-only, re-syncing from the peers listed in --bootstrap-ids. Lets an isolated node track the P-chain through a single trusted peer/gateway. Only affects the P-chain")
	// Uptime Requirement
