...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	fs.Uint(NetworkLargeMessageSizeKey, constants.DefaultMaxMessageSize/units.KiB, fmt.Sprintf("Maximum P2P message size in KiB for peers listed in %s. Must be >= %d KiB (2 MiB)", NetworkLargeMessagePeerIDsKey, constants.DefaultMaxMessageSize/units.KiB))
	fs.StringSlice(NetworkLargeMessagePeerIDsKey, []string{}, fmt.Sprintf("Comma-separated list of peer node IDs allowed to send and receive messages up to %s (KiB). Set to '*' alone to apply this configuration to all peers", NetworkLargeMessageSizeKey))
	fs.Uint64(NetworkLargeMessageInboundAtLargeAllocSizeKey, 0, fmt.Sprintf("Size, in bytes, of at-large byte allocation in the inbound message throttler for peers listed in %s. When unset, derived from %s", NetworkLargeMessagePeerIDsKey, NetworkLargeMessageSizeKey))
	fs.Uint64(NetworkLargeMessageInboundValidatorAllocSizeKey, 0, fmt.Sprintf("Size, in bytes, of validator byte allocation in the inbound message throttler for peers listed in %s. When unset, derived from %s", NetworkLargeMessagePeerIDsKey, NetworkLargeMessageSizeKey))
	fs.Uint64(NetworkLargeMessageInboundNodeMaxAtLargeBytesKey, 0, fmt.Sprintf("Max number of bytes a peer listed in %s can take from the inbound message throttler's at-large allocation. When unset, derived from %s. Must be at least the max message size", NetworkLargeMessagePeerIDsKey, NetworkLargeMessageSizeKey))
	fs.Uint64(NetworkLargeMessageInboundMaxProcessingMsgsKey, 0, fmt.Sprintf("Max number of messages currently processing from a peer listed in %s. When unset, uses the default inbound throttler value", NetworkLargeMessagePeerIDsKey))
	fs.Uint64(NetworkLargeMessageInboundBandwidthRefillRateKey, 0, fmt.Sprintf("Max average inbound bandwidth usage of a peer listed in %s, in bytes per second. When unset, derived from %s", NetworkLargeMessagePeerIDsKey, NetworkLargeMessageSizeKey))
	fs.Uint64(NetworkLargeMessageInboundBandwidthMaxBurstSizeKey, 0, fmt.Sprintf("Max inbound bandwidth a peer listed in %s can use at once. When unset, derived from %s. Must be at least the max message size", NetworkLargeMessagePeerIDsKey, NetworkLargeMessageSizeKey))
	fs.Duration(NetworkLargeMessageInboundCPUMaxRecheckDelayKey, 0, fmt.Sprintf("In the CPU-based network throttler, check at least this often whether a peer listed in %s has fallen to an acceptable usage. When unset, uses the default inbound throttler value", NetworkLargeMessagePeerIDsKey))
	fs.Duration(NetworkLargeMessageInboundDiskMaxRecheckDelayKey, 0, fmt.Sprintf("In the disk-based network throttler, check at least this often whether a peer listed in %s has fallen to an acceptable usage. When unset, uses the default inbound throttler value", NetworkLargeMessagePeerIDsKey))
	fs.Uint64(NetworkLargeMessageOutboundAtLargeAllocSizeKey, 0, fmt.Sprintf("Size, in bytes, of at-large byte allocation in the outbound message throttler for peers listed in %s. When unset, derived from %s", NetworkLargeMessagePeerIDsKey, NetworkLargeMessageSizeKey))
	fs.Uint64(NetworkLargeMessageOutboundValidatorAllocSizeKey, 0, fmt.Sprintf("Size, in bytes, of validator byte allocation in the outbound message throttler for peers listed in %s. When unset, derived from %s", NetworkLargeMessagePeerIDsKey, NetworkLargeMessageSizeKey))
	fs.Uint64(NetworkLargeMessageOutboundNodeMaxAtLargeBytesKey, 0, fmt.Sprintf("Max number of bytes a peer listed in %s can take from the outbound message throttler's at-large allocation. When unset, derived from %s. Must be at least the max message size", NetworkLargeMessagePeerIDsKey, NetworkLargeMessageSizeKey))
