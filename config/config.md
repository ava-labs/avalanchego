...
| `--network-require-validator-to-connect` | `AVAGO_NETWORK_REQUIRE_VALIDATOR_TO_CONNECT` | boolean | `false` | If true, this node will only maintain a connection with another node if this node is a Primary Network validator, the other node is a Primary Network validator, the other node is a beacon, or the other node is a member of a Subnet this node tracks. A Subnet member is a validator of that Subnet, a node whose staking certificate chains to the Subnet's `memberCA`, or a node in its `allowedNodes`; see [the Subnet configuration reference](../subnets/config.md). |
