...
| `--chain-config-content` | `AVAGO_CHAIN_CONFIG_CONTENT` | string | - | As an alternative to `--chain-config-dir`, chains custom configurations can be loaded altogether from command line via `--chain-config-content` flag. Content must be base64 encoded. Example: First, encode the chain config: `cchainconfig="$(echo -n '{"log-level":"trace"}' | base64)"`. Then create the full config: `chainconfig="$(echo -n "{\"C\":{\"Config\":\"[cchainconfig_value]\",\"Upgrade\":null}}" | base64)"`. Finally, run: `avalanchego --chain-config-content "[chainconfig_value]"` |
