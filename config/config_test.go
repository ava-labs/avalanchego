...
func TestGetChainConfigsFromFiles(t *testing.T) {
	tests := map[string]struct {
		...
		expected map[string]platformvm.ChainConfig
	}{
		"no chain configs": {
			configs:  map[string]string{},
			upgrades: map[string]string{},
		...
		"valid chain-id": {
			...
			expected: func() map[string]platformvm.ChainConfig {
				m := map[string]platformvm.ChainConfig{}
				id1, err := ids.FromString("yH8D7ThNJkxmtkuv2jgBa4P1Rn3Qpr4pPr7QYNfcdoS6k6HWp")
				require.NoError(t, err)
				m[id1.String()] = platformvm.ChainConfig{Config: []byte("hello"), Upgrade: []byte("helloUpgrades")}

				id2, err := ids.FromString("2JVSBoinj9C2J33VntvzYtVJNZdN2NKiwwKjcumHUWEb5DbBrm")
				require.NoError(t, err)
				m[id2.String()] = platformvm.ChainConfig{Config: []byte("world"), Upgrade: []byte(nil)}
		...
		"valid alias": {
			...
			expected: func() map[string]platformvm.ChainConfig {
				m := map[string]platformvm.ChainConfig{}
				m["C"] = platformvm.ChainConfig{Config: []byte("hello"), Upgrade: []byte("upgradess")}
				m["X"] = platformvm.ChainConfig{Config: []byte("world"), Upgrade: []byte(nil)}
...
func TestGetChainConfigsDirNotExist(t *testing.T) {
	tests := map[string]struct {
		...
		"full structure": {
			...
			expectedErr: nil,
			expected:    map[string]platformvm.ChainConfig{"C": {Config: []byte("hello"), Upgrade: []byte(nil)}},
		},
	}
...
func TestSetChainConfigDefaultDir(t *testing.T) {
	...
	chainConfigs, err := getChainConfigs(v)
	require.NoError(err)
	expected := map[string]platformvm.ChainConfig{"C": {Config: []byte("helloworld"), Upgrade: []byte(nil)}}
	require.Equal(expected, chainConfigs)
}

func TestGetChainConfigsFromFlags(t *testing.T) {
	tests := map[string]struct {
		fullConfigs map[string]platformvm.ChainConfig
		expected    map[string]platformvm.ChainConfig
	}{
		"no chain configs": {
			fullConfigs: map[string]platformvm.ChainConfig{},
		...
		"valid chain-id": {
			fullConfigs: func() map[string]platformvm.ChainConfig {
				m := map[string]platformvm.ChainConfig{}
				id1, err := ids.FromString("yH8D7ThNJkxmtkuv2jgBa4P1Rn3Qpr4pPr7QYNfcdoS6k6HWp")
				require.NoError(t, err)
				m[id1.String()] = platformvm.ChainConfig{Config: []byte("hello"), Upgrade: []byte("helloUpgrades")}
