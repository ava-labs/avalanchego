...
For a quick unsigned local build (no GPG credentials required):

```bash
./scripts/run_task.sh --taskfile .github/packaging/Taskfile.yml build-tarballs
```

To exercise the full signing path - build, sign, verify, and smoke-test in
a fresh container - both `GPG_KEY_FILE` and `GPG_PASSPHRASE` must be set,
because `validate-tarballs` requires the `.sig` files and the exported
public key and will fail closed without them:

```bash
GPG_KEY_FILE=/path/to/key GPG_PASSPHRASE=secret \
...
```bash
task packaging:build-tarballs                  # unsigned
GPG_KEY_FILE=… GPG_PASSPHRASE=… \
  task packaging:test-build-tarballs           # full signed pipeline
```

Useful environment variables for build/validate:

- `PACKAGING_TAG` - tag/version label to embed, defaults to `v0.0.0`
- `GPG_KEY_FILE` - path to a real signing key (omit for unsigned local build)
- `GPG_PASSPHRASE` - passphrase for the signing key (required when
  `GPG_KEY_FILE` is set)
...
- `PACKAGING_TAG` - release tag, when invoking via the Taskfile
  (`task packaging:upload-tarballs`); the Taskfile re-exports it as `TAG`
  to the script, **overriding** any inbound `TAG` env var
- `TAG` - release tag, only when invoking `upload-tgz.sh` directly without
  the Taskfile
