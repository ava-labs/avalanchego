...
| `go work sync` | Updates `go.work.sum`; prefer `task go:sync-work` after dependency changes |
...
```bash
task tools:install-nix
```

If `task` is not yet available, use:

```bash
./scripts/run_task.sh tools:install-nix
```
...
- To **re-generate all mocks**, use the command below from the root of the project:
    ...
    ```sh
    ./scripts/run_task.sh avalanchego:generate-mocks
    ```
...
```sh
./scripts/run_task.sh avalanchego:test-unit-gomod
```

- Run the linter

```sh
./scripts/run_task.sh avalanchego:lint
```
